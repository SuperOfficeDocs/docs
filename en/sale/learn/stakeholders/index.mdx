---
uid: help-en-sale-stakeholders
title: Stakeholders
description: Stakeholders
keywords: ['sale', 'stakeholder']
author: Bergfrid Dias
date: 01.08.2025
so_version: 10.5
content_type: concept
license: salespremium
tier: growth
audience: person
audience_tooltip: SuperOffice CRM
related:
  - Create stakeholders: /en/sale/learn/stakeholders/create
  - Add stakeholders from section tabs: /en/project/learn/project-members/add#from-section-tab
  - /en/sale/learn/stakeholders/edit
  - Send email to stakeholders: /en/project/learn/project-members/send-email-to
  - Add or update sale types: /en/sale/admin/sale-type
  - Add or update stakeholder roles: /en/admin/lists/add-items
  - Add preference settings: /en/admin/preferences/update
language: en
---

![Stakeholders section tab on a sale -screenshot][img1]

A sale has a main contact, but partners and subcontractors may also be involved in the sale. These are referred to as stakeholders and can be entered in the **Stakeholders** section tab at the bottom of the Sale screen. Stakeholders can include individuals as well as organizations.

For each stakeholder, information about company, contact name, role in the sale, email address and telephone number is displayed.

## Sale - Stakeholder role {#role}

For each stakeholder, their role in the sale is shown. These sales roles are defined in the Lists screen in Settings and maintenance.

<Note>
For the **Stakeholders** section tab to show in the Sale screen, **This sale type has stakeholders** must be checked in the **Edit list item** dialog for the **Sale - Sale type (for stages)** list.
</Note>

You must also set the **Enable Stakeholders** preference in the **Sale** preference group.

[img1]: /media/loc/en/sale/stakeholders-section-tab.png
