...
import Workflow from "./includes/workflow.md";
...
|status|Status of the workflow definition|Enum [WorkflowDefinitionStatus](./enums/workflowdefinitionstatus)|&#x25CF;|
|associate\_id|Associate who owns this workflow|FK [associate](./associate)|&#x25CF;|
|group\_id|Group id of *original* owning associate, so that changes to owner&apos;s primary group do not influence group membership of this workflow.|FK [UserGroup](./usergroup)|&#x25CF;|
...
|tzLocationId|Timezone used for this workflows datetimes. If not set, behavior is best guess - databasetime, system time...|FK [TZLocation](./tzlocation)|&#x25CF;|
...
|exit\_flow\_id|On exit with no success criteria met, we attempt to add the participant to the given flow|FK [workflow](./workflow)|&#x25CF;|
|exit\_success\_flow\_id|On exit with success criteria met, we attempt to add the participant to the given flow|FK [workflow](./workflow)|&#x25CF;|
