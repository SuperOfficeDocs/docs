---
title: "hotlist table"
description: "The table is used by the hotlist to store the relationship between tickets and users"
generated: true
content_type: reference
envir: online
---

import Hotlist from "./includes/hotlist.mdx";

The table is used by the hotlist to store the relationship between tickets and users

Table number: 363

## Fields

| Name | Description | Type | Nullable |
|------|-------------|------|:----:|
|id|The primary key (auto-incremented)|PK|No|
|user\_id|The reference to the user.|FK [ejuser](./ejuser)|No|
|ticket\_id|The ticket the user what to have in his hotlist|FK [ticket](./ticket)|No|


![hotlist table relationship diagram](/media/loc/en/database/tables/hotlist.png)

<Hotlist />

## Indexes

| Fields | Types | Description |
|--------|-------|-------------|
|user\_id |FK |Index |
|ticket\_id |FK |Index |

## Relationships

| Table|  Description |
|------|-------------|
|[ejuser](./ejuser)  |This table contains entries for the users of the system. |
|[ticket](./ticket)  |This table contains the tickets (requests) of the system. Its purpose should be evident. |


## Replication Flags

* None

## Security Flags

* No access control via user's Role.

