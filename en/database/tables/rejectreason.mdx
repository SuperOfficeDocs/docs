---
title: "RejectReason table"
description: "Rejection reason (suggested) for bookings"
generated: true
content_type: reference
envir: online
---

import Rejectreason from "./includes/rejectreason.mdx";

RejectReason MDO list item table.
Rejection reason (suggested) for bookings

Table number: 222

## Fields

| Name | Description | Type | Nullable |
|------|-------------|------|:----:|
|RejectReason\_Id|Primary key|PK|No|
|name|The list item|String(239)|No|
|rank|Rank order|UShort|Yes|
|tooltip|Tooltip or other description|String(254)|Yes|
|deleted|0 -&gt; record is active 1 -&gt; record is &apos;deleted&apos; and should not be shown in lists|UShort|Yes|
|registered|Registered when|UtcDateTime|No|
|registered\_associate\_id|Registered by whom|FK [associate](./associate)|No|
|updated|Last updated when|UtcDateTime|No|
|updated\_associate\_id|Last updated by whom|FK [associate](./associate)|No|
|updatedCount|Number of updates made to this record|UShort|No|


![RejectReason table relationship diagram](/media/loc/en/database/tables/RejectReason.png)

<Rejectreason />

## Indexes

| Fields | Types | Description |
|--------|-------|-------------|
|name |String(239) |Unique |

## Relationships

| Table|  Description |
|------|-------------|
|[associate](./associate)  |Employees, resources and other users - except for External persons |
|[RejectReasonGroupLink](./rejectreasongrouplink)  |User group link table for Comptr, for MDO item hiding |
|[RejectReasonHeadingLink](./rejectreasonheadinglink)  |Heading link table for RejectReason, for MDO headers |


## Replication Flags

* Replicate changes DOWN from central to satellites and travellers.
* Replicate changes UP from satellites and travellers back to central.
* Copy to satellite and travel prototypes.

## Security Flags

* No access control via user's Role.

