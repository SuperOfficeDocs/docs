...
import Associate from "./includes/associate.mdx";
...
|group\_idx|Primary group membership, see UserGroupLink for secondary memberships|FK [UserGroup](./usergroup)|No|
|lastlogin|Last login date|DateTime|No|
|lastlogout|Last logout date|DateTime|No|
|person\_id|Owning person record|FK [person](./person)|No|
|checklistlimit|Diary checklist scan limit|DateTime|No|
|type|Associate type|Enum [AssociateType](./enums/associatetype)|Yes|
...
|tzLocationId|Default timezone location for this associate|FK [TZLocation](./tzlocation)|Yes|
|locationAddress|Address of location, if this is a resource that is a location|String(239)|Yes|
|ejuserId|ID of the ej user record corresponding to this associate; 0 for associates that are not ej users|FK [ejuser](./ejuser)|Yes|
...
![associate table relationship diagram](/media/loc/en/database/tables/associate.png)
