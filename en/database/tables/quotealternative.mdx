...
import Quotealternative from "./includes/quotealternative.mdx";
...
|------|-------------|------|:----:|
|quotealternative\_id|Primary key|PK|No|
|ERPQuoteAlternativeKey|Key that identifies this alternative in the ERP system, if it exists there.|String(254)|Yes|
|QuoteVersionId|The version that owns this alternative (the chain is Sale 1-&gt;1 Quote 1-&gt;+ QuoteVersion 1-&gt;+ QuoteAlternative.|FK [QuoteVersion](./quoteversion)|No|
|Name|Name of Alternative. Shown in tab in user interface, intentionally kept short|String(59)|Yes|
|Description|The tool-tip to use in the user interface (on the tab, for instance).|String(2047)|Yes|
|Status|If there was a problem with for instance calculation, this field is set to warning or error.|Enum [QuoteStatus](./enums/quotestatus)|Yes|
...
|VATInfo|Extra info about VAT that the connector might insert. This field has no business logic in the CRM code, but is available as a merge field in the quote documents.|String(254)|Yes|
|VAT|Tax/VAT - THIS IS AN AMOUNT, available as a merge field in the quote document. The SuperOffice quote connector will calculate this field based on the vat PERCENTAGES on the individual lines; other connectors may implement other algorithms at will.|Double|Yes|
|EarningPercent|The earning on this alternative, in percent of total. The Percentage is given in integer form, i.e. ‘12%’ is represented as ‘12’.|Double|Yes|
|EarningAmount|Earning on this alternative, as an absolute amount|Double|Yes|
|TotalPrice|Sum of the QuoteLines.TotalPrice - AlternativeDiscount or QuoteLines.TotalCost + Earning based on what, if anything, the user has entered last. Shall be calculated by the connector.|Double|Yes|
...
![QuoteAlternative table relationship diagram](/media/loc/en/database/tables/QuoteAlternative.png)
