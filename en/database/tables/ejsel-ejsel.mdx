---
title: "ejsel_ejsel table"
description: "This table defines sources for complex selections in a many/many relationship"
generated: true
content_type: reference
envir: online
---

import EjselEjsel from "./includes/ejsel-ejsel.mdx";

This table defines sources for complex selections in a many/many relationship

Table number: 392

## Fields

| Name | Description | Type | Nullable |
|------|-------------|------|:----:|
|id|Primary key|PK|No|
|parent|The id of the selection for which this item is a source|FK [ejselection](./ejselection)|No|
|child|The id of the child item selection|FK [ejselection](./ejselection)|No|
|operation|The operation to be performed when this source is added to the selection|Int|No|
|order\_pos|The order in which the operations are performed|Int|No|


![ejsel_ejsel table relationship diagram](/media/loc/en/database/tables/ejsel_ejsel.png)

<EjselEjsel />

## Indexes

| Fields | Types | Description |
|--------|-------|-------------|
|parent |FK |Index |
|child |FK |Index |
|operation |Int |Index |
|order\_pos |Int |Index |

## Relationships

| Table|  Description |
|------|-------------|
|[ejselection](./ejselection)  |This table stores selections, dynamic groups. |


## Replication Flags

* None

## Security Flags

* No access control via user's Role.

