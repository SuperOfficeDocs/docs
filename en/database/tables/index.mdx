...
| [AssociateHistory](./associatehistory) |Historical information about associates that have been deleted. Most references are NOT declared as foreign keys; this is a historical table that should not be updated when further changes occur in the database| 540 |
...
| [AutomatedCategoryUpdate](./automatedcategoryupdate) |Info on how to autoupdate category when changing sale or leadstatus on person| 551 |
| [autosave](./autosave) |Autosaved text clobs from ejscript and emarketeer messages| 402 |
| [available\_fonts](./available-fonts) |This table contains all fonts selected to be available for external usage| 552 |
...
| [board\_view\_settings](./board-view-settings) |Settings for selection board view| 505 |
...
| [CacheInvalidation](./cacheinvalidation) |Names and generation numbers for distributed invalidation of caches| 541 |
...
| [email\_flow](./email-flow) |A set of properties related to the email workflow.| 528 |
| [email\_flow\_content\_link](./email-flow-content-link) |Links content to an email workflow| 535 |
| [email\_folder](./email-folder) |Corresponds to an email folder on a mail server| 492 |
| [email\_item](./email-item) |Email data| 494 |
| [EntityCounts](./entitycounts) |Number of different entities an associate has created for usage statistics| 548 |
...
| [form\_field\_value](./form-field-value) |Form field values saved in a searchable format| 539 |
...
| [leadstatus](./leadstatus) |List items of lead status that is selectable for contacts and persons that have specific category| 550 |
...
| [mention](./mention) |A colleague tagged with @ in a request comment| 501 |
...
| [metering\_log](./metering-log) |Metering log table| 502 |
...
| [navigation\_history](./navigation-history) |Per-associate navigation state: Current (persisted soprotocol) and Recent navigation entries, keyed by client and key| 503 |
| [NewsFeedItem](./newsfeeditem) |Contains news feed items - published to one or more users, with one or more language descriptions| 507 |
| [NewsFeedItemCta](./newsfeeditemcta) |List of Calls-to-Action buttons to attach to a given news item. One item may have multiple CTAs (e.g. Approve + Reject).| 509 |
| [NewsFeedItemHtml](./newsfeeditemhtml) |Detailed description of the news item, in a specific language.| 508 |
| [NewsFeedItemRecipient](./newsfeeditemrecipient) |Recipients of the news feed item. One news feed item may be sent to multiple recipients (users).| 510 |
...
| [OnlineApp](./onlineapp) |Echo and track information about Online Apps and their usage| 511 |
...
| [PushNotificationService](./pushnotificationservice) |Used for storing handles to user devices that should receive push notifications| 512 |
...
| [QuoteApprReason](./quoteapprreason) |Predefined reasons for quote approval| 542 |
| [QuoteApprReasonGroupLink](./quoteapprreasongrouplink) |User group link table for QuoteApprReason, for MDO item hiding| 543 |
| [QuoteApprReasonHeadingLink](./quoteapprreasonheadinglink) |Heading link table for QuoteApprReason, for MDO headers| 544 |
...
| [QuoteDenyReason](./quotedenyreason) |Predefined reasons for quote denial| 545 |
| [QuoteDenyReasonGroupLink](./quotedenyreasongrouplink) |User group link table for QuoteDenyReason, for MDO item hiding| 546 |
| [QuoteDenyReasonHeadingLink](./quotedenyreasonheadinglink) |Heading link table for QuoteDenyReason, for MDO headers| 547 |
...
| [ShipmentType](./shipmenttype) |Shipment type list table. Classification of a mailing, allowing recipients to subscribe to lists| 513 |
| [ShipmentTypeReservation](./shipmenttypereservation) |ShipmentTypes a person has reserved against. Note that the absense of a record here implies acceptance of a mailings of this type| 514 |
...
| [target\_assignment\_info](./target-assignment-info) |Linking associate, company or user group target and other information with target values| 518 |
| [target\_assignment\_value](./target-assignment-value) |A set of values linked to assignment info and a period in a target group.| 519 |
| [target\_change](./target-change) |A single field change.| 522 |
| [target\_dimension](./target-dimension) |Defining target dimension| 523 |
| [target\_group](./target-group) |Info about a set of (sales, project, selection...) targets| 516 |
| [target\_period](./target-period) |A set of periods linked with target amounts for users/usergroups and the target groups/years.| 517 |
| [target\_revision](./target-revision) |One batch of changes made to targets| 521 |
| [target\_revision\_history](./target-revision-history) |Revision history info in case the original target group or assignment info was deleted| 520 |
...
| [temp\_db\_lock](./temp-db-lock) |Represents a temporary lock on a resource identified by table_id and key| 536 |
...
| [TemporaryKey](./temporarykey) |Temporary keys for lightweight authentications such as changing ones subscriptions| 515 |
...
| [ticket\_rel\_def\_ticket\_type](./ticket-rel-def-ticket-type) |Link table defining what Ticket types are involved in this relation. Will have M:N rows for a single relation definition| 557 |
| [ticket\_relation](./ticket-relation) |A relation between 2 tickets| 558 |
| [ticket\_relation\_def](./ticket-relation-def) |Ticket relation definition| 556 |
...
| [ticket\_type](./ticket-type) |A ticket (request) type| 553 |
| [ticket\_type\_priority](./ticket-type-priority) |Link table defining what Priorities are relevant to a particular Ticket type| 554 |
| [ticket\_type\_status](./ticket-type-status) |Link table defining what Statuses are relevant to a particular Ticket type| 555 |
| [time\_keeping](./time-keeping) |Time keeping for how much time a sale or project or other entity has spent in a given stage or other state| 506 |
...
| [TypicalSearch](./typicalsearch) |Owner of a set of predefined selection criteria| 504 |
...
| [utm\_parameters](./utm-parameters) |Table for utm parameters collected from visits and formsubmissions| 549 |
...
| [Webhook](./webhook) |Webhook URL to call when events occur in the client or in NetServer. Also tracks call+error statistics.| 524 |
| [Webhook\_notify](./webhook-notify) |Notification recipients for webhook failure events. Each row defines one recipient (associate or ad-hoc email) that should be notified when a webhook fails.| 526 |
| [Webhook\_usage](./webhook-usage) |Webhook usage statistics - tracks call+error statistics. Same primary key as the webhook.| 525 |
...
| [workflow](./workflow) |SuperOffice specific info about a workflow| 527 |
| [workflow\_goal](./workflow-goal) |A set of goals related to a workflow. Some goals will have search criterias associated with it to determine if the goal has been reached.| 530 |
| [workflow\_instance](./workflow-instance) |A set of properties related to the workflow instance of one participant going through the flow| 529 |
| [workflow\_root\_step\_link](./workflow-root-step-link) |Link root steps to the workflow| 538 |
| [workflow\_step](./workflow-step) |A set of steps related to a workflow.| 531 |
| [workflow\_step\_option](./workflow-step-option) |Some steps can have optional child &apos;flows&apos;, a new series of steps| 533 |
| [workflow\_step\_option\_link](./workflow-step-option-link) |Link steps to other steps through step options| 534 |
| [workflow\_trigger](./workflow-trigger) |A set of triggers related to a workflow.| 532 |
| [workflow\_wait\_for\_action](./workflow-wait-for-action) |Some steps wait for actions, this is info about those actions| 537 |
