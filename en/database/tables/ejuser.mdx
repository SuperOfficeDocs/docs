...
|usergroup|The group this user is member of |FK [hierarchy](./hierarchy)|No|
...
|default\_category|The default category for this user when posting tickets and use_last_category is false. |FK [ej_category](./ej-category)|No|
...
|last\_category|The last chosen category for this user when posting tickets.|FK [ej_category](./ej-category)|Yes|
|profile\_id|Reference to the profile connected to this ejuser|FK [profile](./profile)|Yes|
...
|picture\_id|The attachment.id this reference is connected to.|FK [attachment](./attachment)|Yes|
|not\_present|Comment to add for user when status is not present.|String(255)|Yes|
|default\_status\_new\_ticket|Default status on new requests.|FK [ticket_status](./ticket-status)|Yes|
|default\_status\_add\_message|Default status on add message.|FK [ticket_status](./ticket-status)|Yes|
...
|chat\_status|Various chat statuses for an agent|Enum [ChatStatus](./enums/chatstatus)|Yes|
