...
import Ejuser from "./includes/ejuser.md";
...
|usergroup|The group this user is member of |FK [hierarchy](./hierarchy)| |
...
|default\_category|The default category for this user when posting tickets and use_last_category is false. |FK [ej_category](./ej-category)| |
|textareasize|Size of textarea for each user, example &amp;apos;80x40&amp;apos;|String(31)|&#x25CF;|
|flags|A bitmask indicating flags for this entry.|Int|&#x25CF;|
|last\_category|The last chosen category for this user when posting tickets.|FK [ej_category](./ej-category)|&#x25CF;|
|profile\_id|Reference to the profile connected to this ejuser|FK [profile](./profile)|&#x25CF;|
...
|picture\_id|The attachment.id this reference is connected to.|FK [attachment](./attachment)|&#x25CF;|
|not\_present|Comment to add for user when status is not present.|String(255)|&#x25CF;|
|default\_status\_new\_ticket|Default status on new requests.|FK [ticket_status](./ticket-status)|&#x25CF;|
|default\_status\_add\_message|Default status on add message.|FK [ticket_status](./ticket-status)|&#x25CF;|
...
|chat\_status|Various chat statuses for an agent|Enum [ChatStatus](./enums/chatstatus)|&#x25CF;|
