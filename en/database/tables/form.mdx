...
|folder\_id|The folder which this form belongs to. -1 indicates that the shipment is on the root|FK [s_picture_folder](./s-picture-folder)|No|
|script\_id|The CRMScript Macro which will be run when the form is submitted.|FK [ejscript](./ejscript)|No|
|response\_shipment\_id|The id of the s_shipment that is used to send the response mails|FK [s_shipment](./s-shipment)|No|
...
|type|What kind of form is this? Indicates if this is a normal form or a template|Enum [FormType](./enums/formtype)|No|
|recipe|The JSON-formatted recipe of this form|Clob|Yes|
|group\_id|The group which this form belongs to.|FK [UserGroup](./usergroup)|No|
