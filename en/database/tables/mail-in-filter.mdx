...
import MailInFilter from "./includes/mail-in-filter.md";
...
|server\_type|Enum indicating the protocol to use (POP/IMAP).|Enum [MailboxType](./enums/mailboxtype)|&#x25CF;|
...
|category\_id|The id of the category this mailbox assigns tickets to (unless overridden by a sortfilter).|FK [ej_category](./ej-category)|&#x25CF;|
|priority|The id of the priority tickets are assigned when imported from this mailbox.|FK [ticket_priority](./ticket-priority)|&#x25CF;|
|reply\_template\_id|ID of the reply template used on this mail box.|FK [reply_template](./reply-template)| |
...
|language\_id|The id of the customer language entry this entry is connected to.|FK [cust_lang](./cust-lang)|&#x25CF;|
|autofaq|Whether to suggest FAQ entries for this email account or not|Bool|&#x25CF;|
|autofaq\_kb\_category|The root folder for the auto faq search.|FK [kb_category](./kb-category)|&#x25CF;|
