...
import Ticket from "./includes/ticket.md";
...
|connect\_id|If a ticket is connected to another ticket, this field is set to the id of the &amp;apos;master&amp;apos; ticket.|FK [ticket](./ticket)| |
...
|created\_by|The id of the user who created the ticket. 1 (system user) if the ticket was created externally|FK [ejuser](./ejuser)| |
|author|A string representing the author of the ticket (same as author of first message).|String(255)|&#x25CF;|
|owned\_by|The id of the user who owns the ticket. 1 (system user) if the ticket is unassigned.|FK [ejuser](./ejuser)| |
|category|The id of the category a ticket is in.|FK [ej_category](./ej-category)| |
|orig\_category|The id of the category the ticket was posted to.|FK [ej_category](./ej-category)|&#x25CF;|
|slevel|The securitylevel of the ticket.|Enum [TicketSecurityLevel](./enums/ticketsecuritylevel)|&#x25CF;|
|status|The status of the ticket. I.e. active/closed/postponed/deleted|Enum [TicketBaseStatus](./enums/ticketbasestatus)| |
|ticket\_status|User defined ticket status|FK [ticket_status](./ticket-status)| |
|cust\_id|The reference to the customer. NULL or -1 if ticket is not connected to customer.|FK [person](./person)| |
|priority|The reference to the ticket_priority table.|FK [ticket_priority](./ticket-priority)| |
|orig\_priority|The reference to the initial priority for the ticket.|FK [ticket_priority](./ticket-priority)|&#x25CF;|
...
|filter\_id|The reference to the mailbox from which the ticket was created. NULL or -1 if the ticket was not created by a mailbox.|FK [mail_in_filter](./mail-in-filter)| |
...
|read\_status|Whether the owner has read the ticket or not (red, yellow, green).|Enum [TicketReadStatus](./enums/ticketreadstatus)| |
...
|origin|What is the origin of this ticket|Enum [TicketOrigin](./enums/ticketorigin)|&#x25CF;|
...
|contact\_id|The company of the person in the cust_id field, if that person belongs to a company|FK [contact](./contact)|&#x25CF;|
...
|form\_submission\_id|If this ticket was created from a form submission, this field will point to that record|FK [form_submission](./form-submission)|&#x25CF;|
|created\_by\_workflow\_id|The workflow this request/ticket was created by|FK [workflow](./workflow)|&#x25CF;|
|suggestedCategory\_id|Suggestion for categorization, based on the text of the message (AI)|FK [ej_category](./ej-category)| |
|origHumanCategory\_id|Will contain the category id selected by the user, when having the choice of using the suggested category or manually selecting a category|FK [ej_category](./ej-category)| |
|sale\_id|Reference to sale table|FK [sale](./sale)|&#x25CF;|
|project\_id|Reference to project table|FK [project](./project)|&#x25CF;|
|time\_spent|The total time (minutes). Aggregated time spent from ticket&amp;apos;s messages. Read-only for external use.|Int| |
|ticket\_type|Tickettype of the ticket|FK [ticket_type](./ticket-type)| |
