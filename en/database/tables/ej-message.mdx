...
import EjMessage from "./includes/ej-message.md";
...
|ticket\_id|The ticket this message is a child of.|FK [ticket](./ticket)| |
...
|created\_by|The id of the user who posted the message. The value 1 (system user) for externally posted messages.|FK [ejuser](./ejuser)| |
|type|The type of the message (plaintext/html).|Enum [TicketMessageType](./enums/ticketmessagetype)|&#x25CF;|
|message\_category|Defines what kind of message this is.|Enum [TicketMessageCategory](./enums/ticketmessagecategory)| |
|author|A string representing the author of the message. Could be a user&amp;apos;s name, or a persons email address.|String(255)|&#x25CF;|
|slevel|The securitylevel of the message.|Enum [TicketSecurityLevel](./enums/ticketsecuritylevel)|&#x25CF;|
...
|customer\_id|If the message was created by email or from the customer center, this field will point to that customer. -1 if the message was created by an user|FK [person](./person)| |
|body\_attachment|If the body is too long for the database, it&amp;apos;s stored in this attachment instead.|FK [attachment](./attachment)| |
|html\_body\_attachment|If the htmlbody is too long for the database, it&amp;apos;s stored in this attachment instead.|FK [attachment](./attachment)| |
|html\_body\_inline\_converted|True if the html_body has converted inline image links.|Bool| |
|inbox|The id of the inbox entry which created this message. Note: inbox entries are regularily deleted.|FK [inbox](./inbox)|&#x25CF;|
...
|changed\_by|The id of the user who changed the message.|FK [ejuser](./ejuser)|&#x25CF;|
|badge|Badge to explicitly determine how a message was generated|Enum [BadgeType](./enums/badgetype)| |
