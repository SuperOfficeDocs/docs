...
import Suggestedappointment from "./includes/suggestedappointment.md";
...
|saleTypeStageLinkId|Link to saleTypeStageLink, the anchor for sale guide items. Either this OR projectAnchorId should be set, not both at the same time.|FK [SaleTypeStageLink](./saletypestagelink)|&#x25CF;|
|projectTypeStatusLinkId|Link to projectTypeStatusLink, the anchor for project guide items. Either this OR saleAnchorId should be set, not both at the same time.|FK [ProjectTypeStatusLink](./projecttypestatuslink)|&#x25CF;|
|task\_id|Type of the suggested appointment|FK [Task](./task)|&#x25CF;|
