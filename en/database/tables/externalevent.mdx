...
import Externalevent from "./includes/externalevent.md";
...
|signOnTaskId|If not 0, sign on should cause an Activity of this type to be registered|FK [Task](./task)| |
|signOnAssocId|If 0, the signOn Activity should go into the persons Our Contact; if not 0, this is the Associate whose diary should get the activity|FK [associate](./associate)| |
|signOnTextId|If not 0, the given text should be entered into the appointment|FK [text](./text)| |
|signOnConfTextId|If not 0, the given text should be shown as a confirmation text before accpting Sign On|FK [text](./text)| |
...
|signOffTaskId|If not 0, sign off should cause an Activity of this type to be registered|FK [Task](./task)| |
|signOffAssocId|If 0, the signOn Activity should go into the persons Our Contact; if not 0, this is the Associate whose diary should get the activity|FK [associate](./associate)| |
|signOffTextId|If not 0, the given text should be shown as a confirmation text before accpting Sign Off|FK [text](./text)| |
|signOffConfTextId|If not 0, the given text should be shown as a confirmation text before accpting Sign Off|FK [text](./text)| |
