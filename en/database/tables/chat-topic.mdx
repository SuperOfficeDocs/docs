...
import ChatTopic from "./includes/chat-topic.md";
...
|language\_id|The reference to the associated customer language. NULL or -1 if not set.|FK [cust_lang](./cust-lang)|&#x25CF;|
...
|flags|Flags for this topic: show popup alert?, collect consent first?, use custom message?|Enum [ChatTopicFlag](./enums/chattopicflag)|&#x25CF;|
...
|widget\_theme|Predefined themes for the chat widget|Enum [WidgetTheme](./enums/widgettheme)|&#x25CF;|
...
|category\_id|Category on ticket created from off-line request|FK [ej_category](./ej-category)|&#x25CF;|
|priority\_id|Priority on ticket created from off-line request|FK [ticket_priority](./ticket-priority)|&#x25CF;|
|widget\_offline\_fields|Required field when creating ticket in offline mode|Enum [WidgetRequiredFields](./enums/widgetrequiredfields)|&#x25CF;|
|widget\_auto\_faq|Enable automatic FAQ suggestions|Bool|&#x25CF;|
|kb\_category\_id|Root folder for FAQ suggestions|FK [kb_category](./kb-category)|&#x25CF;|
...
|widget\_required\_fields|Fields required to start a chat|Enum [WidgetRequiredFields](./enums/widgetrequiredfields)|&#x25CF;|
...
|tzLocationId|Timezone used for the start/stop times for this topic.|FK [TZLocation](./tzlocation)|&#x25CF;|
...
|bot\_register\_scriptid|Script id to register/unregister chatbot. 0 if there is no bot|FK [ejscript](./ejscript)|&#x25CF;|
|bot\_session\_created\_scriptid|Script id to run on new session.|FK [ejscript](./ejscript)|&#x25CF;|
|bot\_session\_changed\_scriptid|Script id to run when session state changes.|FK [ejscript](./ejscript)|&#x25CF;|
|bot\_message\_received\_scriptid|Script id to run when new messages received from customer.|FK [ejscript](./ejscript)|&#x25CF;|
