...
keywords: ['email', 'Exchange Online', 'IMAP', 'AccessGateway', 'MailKit']
author: fbhub2
date: 09.17.2026
so_version: 12
content_type: howto
...
## Step 0 - Verify server connectivity first {#step-0}
...
Success shows `235 2.7.0 Authentication successful`. A timeout means port 587 is blocked outbound. An authentication failure here despite correct credentials, rather than a timeout, is the signature of [SMTP AUTH being disabled on the mailbox](#smtp-auth-disabled).
...
| **SMTP AUTH enabled on the mailbox** | Disabled by default for some tenants/mailboxes. Must be enabled in Exchange Online, independent of whether the mailbox uses OAuth or password authentication, or outgoing mail will fail. See [Troubleshooting](#smtp-auth-disabled). |
...
## Token lifecycle and re-authentication {#tokens}
...
### Mail doesn't send even though IMAP/OAuth setup succeeds {#smtp-auth-disabled}

**Cause:** Exchange Online has authenticated SMTP submission ("SMTP AUTH") disabled for the mailbox, or for the tenant as a whole. This is a Microsoft-side setting that governs the SMTP protocol itself, not the credential type, so it applies whether the SuperOffice mailbox uses OAuth or password authentication. IMAP fetch (port 993) can succeed while SMTP send (port 587) fails for exactly this reason.

**Fix:**

* **Microsoft 365 admin center (per mailbox):** **Users** > **Active users** > select the user > **Mail** > **Manage email apps** > enable the **Authenticated SMTP** checkbox.
* **Exchange Online PowerShell (per mailbox):** `Set-CASMailbox -Identity <mailbox> -SmtpClientAuthenticationDisabled $false`
* **Exchange Online PowerShell (whole tenant):** `Set-TransportConfig -SmtpClientAuthenticationDisabled $false`, if SMTP AUTH is disabled at the organization level rather than per mailbox.

See Microsoft's [Enable or disable SMTP AUTH in Exchange Online][10] for the full admin reference.
...
[1]: ./index
[10]: https://learn.microsoft.com/en-us/exchange/clients-and-mobile-in-exchange-online/authenticated-client-smtp-submission
