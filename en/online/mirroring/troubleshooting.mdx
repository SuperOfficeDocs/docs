...
description: Diagnostic guidance for the Database Mirroring client - logs, service state, schema, and recovery.
keywords: ['replication', 'diagnostics', 'event log']
...
* The refresh token may have been revoked from the SuperOffice side - for example because the app was reset or the customer subscription changed. Run `reconnect` to re-authenticate. If that fails, run `provision` again.
...
* Confirm the Windows Service names are distinct. They are derived from the Replication ID, so two sessions with the same name on the same host will collide.
...
## Legacy (WCF-based) mirroring {#legacy}
...
* `iss` - issuer
* `aud` - audience
* `nonce` - needed for the signed response
...
[0]: /en/api/authentication/online/certificates/index
[1]: #legacy
[2]: ./setup-guide
[3]: ./force-resync#recover-from-extended-downtime-longer-than-7-days
[4]: ./setup-guide#logging-and-diagnostics
...
[img1]: /media/loc/en/online/database-mirror-unavailable.png
[img2]: /media/loc/en/online/database-mirror-service-iis-page.png
[img3]: /media/loc/en/online/database-mirror-404.png
[img4]: /media/loc/en/online/database-mirror-authenticate-jwt-claims.png
