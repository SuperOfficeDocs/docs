...
description: Database Mirroring service for SuperOffice CRM Online - outbound-only client architecture, near-real-time CDC, simplified security.
keywords: ['replication', 'CDC', 'Kafka', 'replica']
...
<Note>
Internally, the current implementation is sometimes called the *Replication* service. This is the same Database Mirroring feature. Literal product artifacts (the **Replication** tab in Operation Center, the `SuperOffice.Online.Replication.Client.exe` binary, the `_ReplicationState` table) keep that name and are shown as-is in the documentation.
</Note>
...
<Note>
An earlier WCF-based implementation inverted these roles: the cloud pushed data into a partner-hosted endpoint. That implementation is now legacy. New customers should always use the outbound-only client. Existing customers should plan a [migration][2].
</Note>
...
* [Requirements][7] - what you need on the host
* [Set up the client][6] - step-by-step provisioning
* [Migrate from legacy mirroring][2] - if you are already running the WCF service
* [Troubleshooting][8]

[2]: ./migrate
[3]: ./sql-server-schema
[4]: ./blocked-tables
[5]: ./order-database-mirroring
[6]: ./setup-guide
[7]: ./requirements
[8]: ./troubleshooting
