...
There is no GUI to inspect the replica, you must use **SQL Server Management Studio**. Generally, `SELECT * FROM <some table>` should return the same data on the source and the replica.

If a table is genuinely stale and the issue is not a transient connectivity problem, you can force a re-synchronization.

<Note>
This page covers the new outbound-only client. For the legacy WCF-based mirroring service, see [the legacy procedure](#legacy) at the bottom of this page.
</Note>
...
## Recover from extended downtime (longer than 7 days) {#recover-from-extended-downtime-longer-than-7-days}

Change events are streamed through Kafka topics with a default retention of **7 days**. If the client is offline longer than that window, change events will have been purged from the topics. The client cannot fill the gap by replaying older events, it has to be re-snapshotted.

You have three options, from least to most disruptive:

### Option A - Re-snapshot specific tables (recommended)
...
This is the **preferred** recovery path for most situations: it is targeted, fast, and does not disrupt other tables.
...
### Option C - Full reset (operations only)
...
[2]: ./setup-guide
[3]: ./setup-guide#command-reference
[4]: ./troubleshooting
[5]: ./migrate
