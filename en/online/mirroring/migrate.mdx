...
keywords: ['database mirroring', 'replication', 'migration', 'cutover']
...
Create an empty database alongside the existing one, with a clearly distinct name. The convention used here is `<existing_name>_new`, for example, if the legacy replica is `SuperOfficeReplica`, create `SuperOfficeReplica_new`.
...
5. **Start the Windows Service.** The session state lives in the `_ReplicationState` table inside the database itself, so it follows the database through the rename - no re-provisioning is required.
...
* **Windows Services and IIS application pools** should be restarted - the simplest and most reliable approach.
...
* **Confirm SQL Server logins, database users, and role memberships** on the renamed database are intact. A rename preserves principals, but it is worth a check - especially for service accounts that downstream applications depend on.
...
* Create a new version of the app with mirroring disabled and [request publishing][8]. See also [How to turn off mirroring][8].
...
This step is intentionally last and is only carried out once the new setup has stabilized - it makes a fast [rollback][3] possible.

## Schema alignment: `dbo` vs `crm7` {#schema-alignment}

Legacy customers chose the schema for their replica database: some standardized on `crm7`, others on `dbo`. The application code that consumes the replica is written against whichever was chosen, so the schema is part of the contract between the replica and its consumers.
...
When `TablePrefix` is empty or omitted (the default), the client preserves the source schema, typically `crm7`. When set to a value such as `"dbo"`, all replicated tables are created in that schema instead.
...
If schema alignment is not desirable (for example, you want to consolidate to a different schema as part of the migration), the alternatives are to bridge with `CREATE SYNONYM` in the new database, or to update the application queries, but treat that as a separate decision from the cutover, not as the default path.
...
* **Local schema customizations** in the existing replica (indexes, views, triggers, synonyms) must be re-applied to the new replica before cutover. The replicated tables are managed by the client; auxiliary objects are not.

* **Excluded tables.** The new client uses the same base list of [blocked tables][4] as the legacy service, with a few additions to keep the replica cleaner and faster, notably the `binaryobject` table and dictionary information tables.
...
The legacy service resumes populating the restored replica on its next cycle, which is precisely why turning off legacy mirroring on the SuperOffice side (step 7) is intentionally the last action, and only carried out once the new setup has stabilized.
...
* [SQL Server schema][6]
* [Blocked tables][4]
* [How to turn off mirroring for an app][8]
...
[8]: /en/developer-portal/faq/8-mirroring
