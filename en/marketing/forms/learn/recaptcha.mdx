...
description: Add Google reCAPTCHA to a SuperOffice Marketing form to protect it from spam.
keywords: ['reCAPTCHA profile', 'form', 'I', 'm not a robot', 'site key', 'secret key']
...
1. And then the "I'm not a robot" checkbox.
1. Add the domain. Under "domains", you must insert the domain that the Customer Center (customer.fcgi) is running on or a parent domain of that sub-domain.
