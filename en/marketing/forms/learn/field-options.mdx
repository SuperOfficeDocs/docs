...
* **Checkbox-type custom field:** A Boolean extra field (Yes/No only) that can be set to start checked or unchecked when the form loads. Find **Set checked by default** in the field's settings — it defaults to unchecked. If the field has a [related field][6] attached via a condition, the default-checked state is treated as if the visitor had set it, so the related field shows or stays hidden on load accordingly.
...
* **Conditional (related) fields:** Radio buttons, checkboxes, drop-downs, and rating fields can [trigger a related field][6] that only appears when a specific option or value is selected.
