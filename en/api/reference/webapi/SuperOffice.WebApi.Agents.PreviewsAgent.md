...
```csharp
public class PreviewsAgent : AgentBase, IPreviewsAgent, IAgentBase, IDisposable
```
...
using (PreviewsAgent agent = new PreviewsAgent(mySession))
{
...
```csharp
public PreviewsAgent(HttpClient httpClient)
```
...
```csharp
public PreviewsAgent(WebApiOptions options, HttpClient httpClient = null)
```
...
```csharp
public Task<string> GetPreviewAsync(string previewHint, RequestOptions requestOptions = null)
```
