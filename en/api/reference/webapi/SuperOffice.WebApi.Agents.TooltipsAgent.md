...
```csharp
public class TooltipsAgent : AgentBase, ITooltipsAgent, IAgentBase, IDisposable
```
...
using (TooltipsAgent agent = new TooltipsAgent(mySession))
\{
...
```csharp
public TooltipsAgent(HttpClient httpClient)
```
...
```csharp
public TooltipsAgent(WebApiOptions options, HttpClient httpClient = null)
```
...
```csharp
public Task<string> GetTooltipAsync(string tooltipHint, RequestOptions requestOptions = null)
```
