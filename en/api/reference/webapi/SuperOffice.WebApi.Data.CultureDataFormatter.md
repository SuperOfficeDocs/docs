...
### <a id="SuperOffice_WebApi_Data_CultureDataFormatter_GetAllMultiLanguageValues_System_String_"></a> GetAllMultiLanguageValues\(string\)

Extracts every language-specific value from a multi-language string, e.g.
<code>US:"Banana";NO:"Banan"</code> returns <code>["Banana", "Banan"]</code>. The two-letter language codes are
never returned, and any surrounding non-fragment text is discarded. If the input has no valid language
fragments (a plain string, or JSON), it is returned unchanged as the single value.

```csharp
public static IReadOnlyList<string> GetAllMultiLanguageValues(string multilangString)
```
...
The multi-language string to split.

#### Returns

 [IReadOnlyList](https://learn.microsoft.com/dotnet/api/system.collections.generic.ireadonlylist\-1)<[string](https://learn.microsoft.com/dotnet/api/system.string)\>

The distinct language values, or the original string when it is not multi-language.

#### Remarks

Shares the single-pass scanner with [ParseMultiLanguageString](/en/api/reference/webapi/SuperOffice.WebApi.Data.CultureDataFormatter#SuperOffice_WebApi_Data_CultureDataFormatter_ParseMultiLanguageString_System_String_System_String_), but returns all language
values rather than resolving one culture, so callers that index list-item text (e.g. the freetext
indexer) can cover every language without indexing the language codes.
