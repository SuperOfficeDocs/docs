# <a id="SuperOffice_WebApi_Data"></a> Namespace SuperOffice.WebApi.Data

### Classes

 [UserPreferenceStrings.AI](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.AI)

AI - Artificial Intelligence

 [AI\_AnalyzeBizCardImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_AnalyzeBizCardImageRequest)

 [AI\_AnalyzeBizCardTextRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_AnalyzeBizCardTextRequest)

 [AI\_AnswerAcceptedRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_AnswerAcceptedRequest)

 [AI\_AnswerFeedbackRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_AnswerFeedbackRequest)

 [AI\_AnswerRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_AnswerRequest)

 [AI\_AnswerUsingContextRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_AnswerUsingContextRequest)

 [AI\_CancelIndexingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_CancelIndexingRequest)

 [AI\_ClearChatbotTurnsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_ClearChatbotTurnsRequest)

 [AI\_CreateTextForAppointmentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_CreateTextForAppointmentRequest)

 [AI\_DetectLanguageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_DetectLanguageRequest)

 [AI\_DetectSentimentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_DetectSentimentRequest)

 [AI\_ExpandTextRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_ExpandTextRequest)

 [AI\_GetChatbotPromptSuggestionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_GetChatbotPromptSuggestionsRequest)

 [AI\_GetChatbotResponseRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_GetChatbotResponseRequest)

 [AI\_GetChatbotTurnsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_GetChatbotTurnsRequest)

 [AI\_GetFirstFormDesignCssFromImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_GetFirstFormDesignCssFromImageRequest)

 [AI\_GetFirstFormDesignCssFromUrlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_GetFirstFormDesignCssFromUrlRequest)

 [AI\_GetFormDesignCssFromImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_GetFormDesignCssFromImageRequest)

 [AI\_GetFormDesignCssFromUrlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_GetFormDesignCssFromUrlRequest)

 [AI\_GetFormDesignFromImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_GetFormDesignFromImageRequest)

 [AI\_GetFormDesignFromUrlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_GetFormDesignFromUrlRequest)

 [AI\_GetIndexingStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_GetIndexingStatusRequest)

 [AI\_GetNextFormDesignCssRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_GetNextFormDesignCssRequest)

 [AI\_GetSummarizeContactPromptRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_GetSummarizeContactPromptRequest)

 [AI\_GetSummarizeSalePromptRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_GetSummarizeSalePromptRequest)

 [AI\_GetSummarizeTicketPromptRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_GetSummarizeTicketPromptRequest)

 [AI\_GetTrainingStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_GetTrainingStatusRequest)

 [AI\_GuessCategoryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_GuessCategoryRequest)

 [AI\_ParseQueryIntoRestrictionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_ParseQueryIntoRestrictionsRequest)

 [AI\_RemovePhotoBackgroundRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_RemovePhotoBackgroundRequest)

 [AI\_RephraseTextRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_RephraseTextRequest)

 [AI\_SearchRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_SearchRequest)

 [AI\_StartRagIndexingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_StartRagIndexingRequest)

 [AI\_SummarizeContactRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_SummarizeContactRequest)

 [AI\_SummarizeSaleRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_SummarizeSaleRequest)

 [AI\_SummarizeTextRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_SummarizeTextRequest)

 [AI\_SummarizeTicketRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_SummarizeTicketRequest)

 [AI\_TrainCategoryGuesserRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_TrainCategoryGuesserRequest)

 [AI\_TranslateEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_TranslateEntityRequest)

 [AI\_TranslateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.AI_TranslateRequest)

 [AccessGatewayInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.AccessGatewayInfo)

Carrier object for AccessGatewayInfo.
Access gateway related data and operations

 [ActivityFilter](/en/api/reference/webapi/SuperOffice.WebApi.Data.ActivityFilter)

Carrier object for ActivityFilter.
Activity list filter information.

 [ActivityInformationListItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.ActivityInformationListItem)

Carrier object for ActivityInformationListItem.

 [ActivitySummary](/en/api/reference/webapi/SuperOffice.WebApi.Data.ActivitySummary)

Carrier object for ActivitySummary.
Activity information summary - number of free and busy activities.

 [ActivitySummaryItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.ActivitySummaryItem)

Carrier object for ActivitySummaryItem.
Summary details about appointments and documents.

 [Address](/en/api/reference/webapi/SuperOffice.WebApi.Data.Address)

Carrier object for Address.
Street and/or Postal address, in both formatted and structured forms. You only need to modify one of the two for the change to be registered.

 [AddressSyncData](/en/api/reference/webapi/SuperOffice.WebApi.Data.AddressSyncData)

Carrier object for AddressSyncData.

 [UserPreferenceStrings.AdminImportRules](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.AdminImportRules)

Preference keys for the mini panel.

 [AlarmData](/en/api/reference/webapi/SuperOffice.WebApi.Data.AlarmData)

Carrier object for AlarmData.

 [AmountClassEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.AmountClassEntity)

Carrier object for AmountClassEntity.

 [AnalyticsData](/en/api/reference/webapi/SuperOffice.WebApi.Data.AnalyticsData)

Carrier object for AnalyticsData.

 [Appointment](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment)

Carrier object for Appointment.
Simple read-only appointment data.

 [AppointmentEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.AppointmentEntity)

Carrier object for AppointmentEntity.
Appointments appear in the diary, and have links to a Contact/Person and possibly a Project or Sale. They have start and end time+date.

 [AppointmentInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.AppointmentInfo)

Carrier object for AppointmentInfo.
Info about an appointment

 [AppointmentMoveData](/en/api/reference/webapi/SuperOffice.WebApi.Data.AppointmentMoveData)

Carrier object for AppointmentMoveData.
Parameter data for moving an appointment.

 [AppointmentSaveData](/en/api/reference/webapi/SuperOffice.WebApi.Data.AppointmentSaveData)

Carrier object for AppointmentSaveData.
This class contains the updated appointment entity and a flag for if any emails were queued when saving

 [AppointmentSyncData](/en/api/reference/webapi/SuperOffice.WebApi.Data.AppointmentSyncData)

Carrier object for AppointmentSyncData.

 [Appointment\_AcceptRejectedRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_AcceptRejectedRequest)

 [Appointment\_AcceptRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_AcceptRequest)

 [Appointment\_AcceptWithEmailConfirmationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_AcceptWithEmailConfirmationRequest)

 [Appointment\_AcceptWithSmtpEmailConfirmationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_AcceptWithSmtpEmailConfirmationRequest)

 [Appointment\_AssignToRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_AssignToRequest)

 [Appointment\_CalculateDaysRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_CalculateDaysRequest)

 [Appointment\_CanAssignToProjectMemberRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_CanAssignToProjectMemberRequest)

 [Appointment\_CleanUpBookingDeletedRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_CleanUpBookingDeletedRequest)

 [Appointment\_CleanUpBookingDeletedWithUpdateModeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_CleanUpBookingDeletedWithUpdateModeRequest)

 [Appointment\_CleanUpRecurringBookingDeletedRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_CleanUpRecurringBookingDeletedRequest)

 [Appointment\_ConvertAppointmentToTaskRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_ConvertAppointmentToTaskRequest)

 [Appointment\_CreateAndAcceptRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_CreateAndAcceptRequest)

 [Appointment\_CreateAndAcceptWithEmailConfirmationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_CreateAndAcceptWithEmailConfirmationRequest)

 [Appointment\_CreateAppointmentEntityFromExistingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_CreateAppointmentEntityFromExistingRequest)

 [Appointment\_CreateAppointmentForUIDRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_CreateAppointmentForUIDRequest)

 [Appointment\_CreateDefaultAppointmentEntityByTypeAndAssociateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_CreateDefaultAppointmentEntityByTypeAndAssociateRequest)

 [Appointment\_CreateDefaultAppointmentEntityByTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_CreateDefaultAppointmentEntityByTypeRequest)

 [Appointment\_CreateDefaultAppointmentEntityFromProjectSuggestionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_CreateDefaultAppointmentEntityFromProjectSuggestionRequest)

 [Appointment\_CreateDefaultAppointmentEntityFromSaleSuggestionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_CreateDefaultAppointmentEntityFromSaleSuggestionRequest)

 [Appointment\_CreateDefaultReOpenAppointmentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_CreateDefaultReOpenAppointmentRequest)

 [Appointment\_CreateDefaultRecurrenceByDateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_CreateDefaultRecurrenceByDateRequest)

 [Appointment\_CreateDefaultRecurrenceRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_CreateDefaultRecurrenceRequest)

 [Appointment\_CreateVideoMeetingReservationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_CreateVideoMeetingReservationRequest)

 [Appointment\_DeclineInvitationFromEmailItemRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_DeclineInvitationFromEmailItemRequest)

 [Appointment\_DeleteRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_DeleteRequest)

 [Appointment\_DeleteVideoMeetingReservationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_DeleteVideoMeetingReservationRequest)

 [Appointment\_GenerateLeadRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GenerateLeadRequest)

 [Appointment\_GetActivityInformationListByDatesAndAssociateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetActivityInformationListByDatesAndAssociateRequest)

 [Appointment\_GetAlarmsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetAlarmsRequest)

 [Appointment\_GetAppointmentEmailInvitationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetAppointmentEmailInvitationRequest)

 [Appointment\_GetAppointmentFromUIDRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetAppointmentFromUIDRequest)

 [Appointment\_GetAppointmentHaveParticipantsWithEmailRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetAppointmentHaveParticipantsWithEmailRequest)

 [Appointment\_GetAppointmentRecordsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetAppointmentRecordsRequest)

 [Appointment\_GetAppointmentsByTaskHeadingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetAppointmentsByTaskHeadingRequest)

 [Appointment\_GetAssociateDiaryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetAssociateDiaryRequest)

 [Appointment\_GetAssociatesDiaryInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetAssociatesDiaryInfoRequest)

 [Appointment\_GetAssociatesDiaryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetAssociatesDiaryRequest)

 [Appointment\_GetCanInsertForAssociatesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetCanInsertForAssociatesRequest)

 [Appointment\_GetContactAppointmentsByTaskHeadingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetContactAppointmentsByTaskHeadingRequest)

 [Appointment\_GetContactAppointmentsByTaskRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetContactAppointmentsByTaskRequest)

 [Appointment\_GetContactAppointmentsByTasksRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetContactAppointmentsByTasksRequest)

 [Appointment\_GetContactAppointmentsByTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetContactAppointmentsByTypeRequest)

 [Appointment\_GetContactAppointmentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetContactAppointmentsRequest)

 [Appointment\_GetDayInformationListByDatesAndAssociateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetDayInformationListByDatesAndAssociateRequest)

 [Appointment\_GetDiaryByGroupRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetDiaryByGroupRequest)

 [Appointment\_GetMyAppointmentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetMyAppointmentsRequest)

 [Appointment\_GetMyDiaryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetMyDiaryRequest)

 [Appointment\_GetMyPublishedAppointmentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetMyPublishedAppointmentsRequest)

 [Appointment\_GetMySyncAppointmentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetMySyncAppointmentsRequest)

 [Appointment\_GetMyTasksRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetMyTasksRequest)

 [Appointment\_GetNextAvailableTimeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetNextAvailableTimeRequest)

 [Appointment\_GetNextSuggestedAppointmentBySaleRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetNextSuggestedAppointmentBySaleRequest)

 [Appointment\_GetOrganizerNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetOrganizerNameRequest)

 [Appointment\_GetPersonAppointmentsByTaskHeadingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetPersonAppointmentsByTaskHeadingRequest)

 [Appointment\_GetPersonAppointmentsByTaskRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetPersonAppointmentsByTaskRequest)

 [Appointment\_GetPersonAppointmentsByTasksRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetPersonAppointmentsByTasksRequest)

 [Appointment\_GetPersonAppointmentsByTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetPersonAppointmentsByTypeRequest)

 [Appointment\_GetPersonAppointmentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetPersonAppointmentsRequest)

 [Appointment\_GetPersonDiaryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetPersonDiaryRequest)

 [Appointment\_GetPersonTasksRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetPersonTasksRequest)

 [Appointment\_GetProjectAppointmentsByTaskHeadingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetProjectAppointmentsByTaskHeadingRequest)

 [Appointment\_GetProjectAppointmentsByTaskRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetProjectAppointmentsByTaskRequest)

 [Appointment\_GetProjectAppointmentsByTasksRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetProjectAppointmentsByTasksRequest)

 [Appointment\_GetProjectAppointmentsByTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetProjectAppointmentsByTypeRequest)

 [Appointment\_GetProjectAppointmentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetProjectAppointmentsRequest)

 [Appointment\_GetProjectMemberAppointmentsByTaskHeadingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetProjectMemberAppointmentsByTaskHeadingRequest)

 [Appointment\_GetProjectMemberAppointmentsByTaskRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetProjectMemberAppointmentsByTaskRequest)

 [Appointment\_GetProjectMemberAppointmentsByTasksRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetProjectMemberAppointmentsByTasksRequest)

 [Appointment\_GetProjectMemberAppointmentsByTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetProjectMemberAppointmentsByTypeRequest)

 [Appointment\_GetProjectMemberAppointmentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetProjectMemberAppointmentsRequest)

 [Appointment\_GetPublishedAppointmentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetPublishedAppointmentRequest)

 [Appointment\_GetPublishedAppointmentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetPublishedAppointmentsRequest)

 [Appointment\_GetPublishedProjectAppointmentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetPublishedProjectAppointmentsRequest)

 [Appointment\_GetRedLetterInformationListByDatesAndAssociateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetRedLetterInformationListByDatesAndAssociateRequest)

 [Appointment\_GetTaskListItemsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetTaskListItemsRequest)

 [Appointment\_GetUIDFromAppointmentIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetUIDFromAppointmentIdRequest)

 [Appointment\_GetVideoMeetingConnectedStateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_GetVideoMeetingConnectedStateRequest)

 [Appointment\_MoveExtendedRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_MoveExtendedRequest)

 [Appointment\_MoveRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_MoveRequest)

 [Appointment\_MoveWithEmailRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_MoveWithEmailRequest)

 [Appointment\_RejectRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_RejectRequest)

 [Appointment\_RejectWithEmailConfirmationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_RejectWithEmailConfirmationRequest)

 [Appointment\_RejectWithSmtpEmailConfirmationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_RejectWithSmtpEmailConfirmationRequest)

 [Appointment\_RequestForInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_RequestForInfoRequest)

 [Appointment\_SaveRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_SaveRequest)

 [Appointment\_SaveWithCustomInvitationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_SaveWithCustomInvitationRequest)

 [Appointment\_SetActivityStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_SetActivityStatusRequest)

 [Appointment\_SetSeenManyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_SetSeenManyRequest)

 [Appointment\_SetSeenRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_SetSeenRequest)

 [Appointment\_ToggleActivitiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_ToggleActivitiesRequest)

 [Appointment\_ToggleActivityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_ToggleActivityRequest)

 [Appointment\_ToggleAndSetActivitiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_ToggleAndSetActivitiesRequest)

 [Appointment\_ToggleAppointmentStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_ToggleAppointmentStatusRequest)

 [Appointment\_UpdateAppointmentFromIcsResponseRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_UpdateAppointmentFromIcsResponseRequest)

 [Appointment\_UpdateAppointmentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_UpdateAppointmentRequest)

 [Appointment\_UpdateAppointmentWithModeAndEmailRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_UpdateAppointmentWithModeAndEmailRequest)

 [Appointment\_UpdateAppointmentWithModeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_UpdateAppointmentWithModeRequest)

 [Appointment\_ValidateAppointmentEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_ValidateAppointmentEntityRequest)

 [Appointment\_ValidateDaysRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_ValidateDaysRequest)

 [Appointment\_WillSendEmailRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Appointment_WillSendEmailRequest)

 [ApproveRejectResponse](/en/api/reference/webapi/SuperOffice.WebApi.Data.ApproveRejectResponse)

Carrier object for ApproveRejectResponse.
ApproveRejectResponse contains a PluginResponse and an appointmentId if the operation created one.

 [ArchiveColumnData](/en/api/reference/webapi/SuperOffice.WebApi.Data.ArchiveColumnData)

Carrier object for ArchiveColumnData.
Data carrier class for an item (cell) in an archive. This class contains properties and members for three distinct values: The (visible) display value; the tooltip hint, and the link hint.

 [ArchiveColumnInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.ArchiveColumnInfo)

Carrier object for ArchiveColumnInfo.
This class carries information about the definition and properties of an archive display column. It is used by the archive MDO providers to construct column lists for the configuration dialogs, and by the archive control to know what type (and other properties) each column has. The class is serializable and used on both the application and Web server sides.

 [ArchiveConfiguration](/en/api/reference/webapi/SuperOffice.WebApi.Data.ArchiveConfiguration)

Carrier object for ArchiveConfiguration.
This service is used to store and fetch the configuration of an archive. The configuration consists of entities and columns; entities correspond to archive provider entities and the checkboxes you see in the bottom of standard archives. Column configuration includes both what columns should be visible, what order they are in (left to right), and whether the archive rows should be sorted by one or more columns.&lt;para/&gt;Note that desired columns can be set through the selectable mdo list called archiveColumns:&amp;lt;perovider name&amp;gt;, and entities can be set through the selectable mdo list archiveEntities:&amp;lt;provider name&amp;gt;. You can also use the SetChosenEntities and SetChosenColumns method of this service. The selectable mdo list implementation is suitable for data binding using mdo list controls.

 [ArchiveListItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.ArchiveListItem)

Carrier object for ArchiveListItem.

 [ArchiveListResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.ArchiveListResult)

Carrier object for ArchiveListResult.
Header/Detail variant of archive result, with a Row Count plus the actual rows. Future extensions may include other header data on the same level as the RowCount

 [ArchiveOrderByInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.ArchiveOrderByInfo)

Carrier object for ArchiveOrderByInfo.
This class carries information about an ordering criterion for an archive.

 [ArchiveRestrictionGroup](/en/api/reference/webapi/SuperOffice.WebApi.Data.ArchiveRestrictionGroup)

Carrier object for ArchiveRestrictionGroup.
Carrier for restriction group. It contains all the search restrictions for the group - that is, ArchiveRestrictionInfo - objects, including some info like rank and name

 [ArchiveRestrictionInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.ArchiveRestrictionInfo)

Carrier object for ArchiveRestrictionInfo.
This class carries information about a restriction on the query of an archive provider

 [Archive\_ExportArchiveRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_ExportArchiveRequest)

 [Archive\_GetActivityFilterRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetActivityFilterRequest)

 [Archive\_GetArchiveConfigurationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetArchiveConfigurationRequest)

 [Archive\_GetArchiveConfigurationV2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetArchiveConfigurationV2Request)

 [Archive\_GetArchiveConfigurationWithContextRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetArchiveConfigurationWithContextRequest)

 [Archive\_GetArchiveList2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetArchiveList2Request)

 [Archive\_GetArchiveListByColumns2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetArchiveListByColumns2Request)

 [Archive\_GetArchiveListByColumnsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetArchiveListByColumnsRequest)

 [Archive\_GetArchiveListByColumnsWithContext2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetArchiveListByColumnsWithContext2Request)

 [Archive\_GetArchiveListByColumnsWithContextRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetArchiveListByColumnsWithContextRequest)

 [Archive\_GetArchiveListByColumnsWithHeader2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetArchiveListByColumnsWithHeader2Request)

 [Archive\_GetArchiveListByColumnsWithHeaderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetArchiveListByColumnsWithHeaderRequest)

 [Archive\_GetArchiveListByColumnsWithHeaderWithContext2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetArchiveListByColumnsWithHeaderWithContext2Request)

 [Archive\_GetArchiveListByColumnsWithHeaderWithContextRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetArchiveListByColumnsWithHeaderWithContextRequest)

 [Archive\_GetArchiveListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetArchiveListRequest)

 [Archive\_GetArchiveListWithContext2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetArchiveListWithContext2Request)

 [Archive\_GetArchiveListWithContextRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetArchiveListWithContextRequest)

 [Archive\_GetAvailableColumnsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetAvailableColumnsRequest)

 [Archive\_GetAvailableEntitiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetAvailableEntitiesRequest)

 [Archive\_GetGroupAssociateIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetGroupAssociateIdsRequest)

 [Archive\_GetProviderNamesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetProviderNamesRequest)

 [Archive\_GetRelatedData2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetRelatedData2Request)

 [Archive\_GetRelatedDataRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_GetRelatedDataRequest)

 [Archive\_SetActivityFilterRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_SetActivityFilterRequest)

 [Archive\_SetChosenColumnsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_SetChosenColumnsRequest)

 [Archive\_SetChosenColumnsV2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_SetChosenColumnsV2Request)

 [Archive\_SetChosenEntitiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_SetChosenEntitiesRequest)

 [Archive\_SetChosenEntitiesV2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_SetChosenEntitiesV2Request)

 [Archive\_SetColumnWidthsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_SetColumnWidthsRequest)

 [Archive\_SetColumnWidthsV2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Archive_SetColumnWidthsV2Request)

 [UserPreferenceStrings.Archives](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Archives)

 [Associate](/en/api/reference/webapi/SuperOffice.WebApi.Data.Associate)

Carrier object for Associate.

 [Associate\_GetAssociateByPersonIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Associate_GetAssociateByPersonIdRequest)

 [Associate\_GetAssociatesByGroupRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Associate_GetAssociatesByGroupRequest)

 [Associate\_GetEncryptionKeyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Associate_GetEncryptionKeyRequest)

 [Associate\_GetNoteRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Associate_GetNoteRequest)

 [Associate\_LogOffWindowsUsersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Associate_LogOffWindowsUsersRequest)

 [Associate\_SaveNoteRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Associate_SaveNoteRequest)

 [UserPreferenceStrings.Associates](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Associates)

Associate preferences like which radiobutton did I select the last time I used this dialog/control
or which item did I select last time I used this dropdownlist

 [AttachmentEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.AttachmentEntity)

Carrier object for AttachmentEntity.
The Attachment carrier represent meta data for a Service attachment

 [AttachmentPreview](/en/api/reference/webapi/SuperOffice.WebApi.Data.AttachmentPreview)

Carrier object for AttachmentPreview.
Service attachment data intended for preview purposes

 [AttachmentValidationResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.AttachmentValidationResult)

Carrier object for AttachmentValidationResult.
Attachment validation info

 [AutomatedCategoryUpdate](/en/api/reference/webapi/SuperOffice.WebApi.Data.AutomatedCategoryUpdate)

Carrier object for AutomatedCategoryUpdate.
Carrier for an automated category update setting.

 [AvailableFont](/en/api/reference/webapi/SuperOffice.WebApi.Data.AvailableFont)

Carrier object for AvailableFont.
Carrier for a font that is available for external use.

 [BLOB\_ChangeContactImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_ChangeContactImageRequest)

 [BLOB\_ChangePersonImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_ChangePersonImageRequest)

 [BLOB\_ChangeProductImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_ChangeProductImageRequest)

 [BLOB\_ChangeProjectImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_ChangeProjectImageRequest)

 [BLOB\_CleanupBatchTaskRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_CleanupBatchTaskRequest)

 [BLOB\_GetBlobEntityOnContactRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_GetBlobEntityOnContactRequest)

 [BLOB\_GetBlobEntityOnCountryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_GetBlobEntityOnCountryRequest)

 [BLOB\_GetBlobEntityOnPersonRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_GetBlobEntityOnPersonRequest)

 [BLOB\_GetBlobEntityOnProductRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_GetBlobEntityOnProductRequest)

 [BLOB\_GetBlobEntityOnProjectRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_GetBlobEntityOnProjectRequest)

 [BLOB\_GetBlobStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_GetBlobStreamRequest)

 [BLOB\_GetChatImageWithSizeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_GetChatImageWithSizeRequest)

 [BLOB\_GetContactImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_GetContactImageRequest)

 [BLOB\_GetContactImageWithSizeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_GetContactImageWithSizeRequest)

 [BLOB\_GetOutputFromBatchRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_GetOutputFromBatchRequest)

 [BLOB\_GetPersonImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_GetPersonImageRequest)

 [BLOB\_GetPersonImageWithSizeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_GetPersonImageWithSizeRequest)

 [BLOB\_GetProductImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_GetProductImageRequest)

 [BLOB\_GetProductThumbnailRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_GetProductThumbnailRequest)

 [BLOB\_GetProjectImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_GetProjectImageRequest)

 [BLOB\_GetProjectImageWithSizeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_GetProjectImageWithSizeRequest)

 [BLOB\_GetQuoteLineImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_GetQuoteLineImageRequest)

 [BLOB\_SaveImageStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_SaveImageStreamRequest)

 [BLOB\_SaveProjectImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_SaveProjectImageRequest)

 [BLOB\_SetBlobStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_SetBlobStreamRequest)

 [BLOB\_SetContactImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_SetContactImageRequest)

 [BLOB\_SetPersonImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_SetPersonImageRequest)

 [BLOB\_SetProductImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_SetProductImageRequest)

 [BLOB\_SetProductThumbnailRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_SetProductThumbnailRequest)

 [BLOB\_SetProjectImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BLOB_SetProjectImageRequest)

 [Batch](/en/api/reference/webapi/SuperOffice.WebApi.Data.Batch)

Carrier object for Batch.
BatchTasks run in the background.

 [BatchTaskInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.BatchTaskInfo)

Carrier object for BatchTaskInfo.
BatchTaskInfo contains information about one batch task.

 [Batch\_DeleteBatchTaskRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Batch_DeleteBatchTaskRequest)

 [Batch\_DeleteBatchTasksRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Batch_DeleteBatchTasksRequest)

 [Batch\_GetAllBatchTaskInfosByNameAndStateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Batch_GetAllBatchTaskInfosByNameAndStateRequest)

 [Batch\_GetAllBatchTaskInfosByNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Batch_GetAllBatchTaskInfosByNameRequest)

 [Batch\_GetBatchTaskInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Batch_GetBatchTaskInfoRequest)

 [Batch\_GetBatchTaskInfosByAssociatesAndStateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Batch_GetBatchTaskInfosByAssociatesAndStateRequest)

 [Batch\_GetBatchTaskInfosByAssociatesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Batch_GetBatchTaskInfosByAssociatesRequest)

 [Batch\_GetBatchTaskInfosByNameAndAssociatesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Batch_GetBatchTaskInfosByNameAndAssociatesRequest)

 [Batch\_GetBatchTaskInfosByNameAndStateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Batch_GetBatchTaskInfosByNameAndStateRequest)

 [Batch\_GetBatchTaskInfosByStateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Batch_GetBatchTaskInfosByStateRequest)

 [Batch\_GetSystemBatchTaskInfosByNameAndStateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Batch_GetSystemBatchTaskInfosByNameAndStateRequest)

 [Batch\_GetSystemBatchTaskInfosByNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Batch_GetSystemBatchTaskInfosByNameRequest)

 [Batch\_StartBatchJobRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Batch_StartBatchJobRequest)

 [Batch\_StopBatchJobRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Batch_StopBatchJobRequest)

 [Batch\_UpdateBatchTaskRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Batch_UpdateBatchTaskRequest)

 [BizCard](/en/api/reference/webapi/SuperOffice.WebApi.Data.BizCard)

Carrier object for BizCard.
Contains a ContactEntity and a PersonEntity filled with information from a business card.

 [BlobEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.BlobEntity)

Carrier object for BlobEntity.

 [BoardViewSettingsBase](/en/api/reference/webapi/SuperOffice.WebApi.Data.BoardViewSettingsBase)

Carrier object for BoardViewSettingsBase.
Board view settings.

 [UserPreferenceStrings.BulkUpdate](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.BulkUpdate)

BulkUpdate - Misc

 [BulkUpdate\_ExecuteByEntityIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BulkUpdate_ExecuteByEntityIdsRequest)

 [BulkUpdate\_ExecuteBySelectionIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BulkUpdate_ExecuteBySelectionIdRequest)

 [BulkUpdate\_GetAvailableFieldsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BulkUpdate_GetAvailableFieldsRequest)

 [BulkUpdate\_GetJobResultAsJsonFromBinaryObjectIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BulkUpdate_GetJobResultAsJsonFromBinaryObjectIdRequest)

 [BulkUpdate\_GetJobResultsFromStorageAsJsonRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BulkUpdate_GetJobResultsFromStorageAsJsonRequest)

 [BulkUpdate\_GetStoredFieldsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BulkUpdate_GetStoredFieldsRequest)

 [BulkUpdate\_RemoveStoredFieldsAndGetDefaultFieldsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BulkUpdate_RemoveStoredFieldsAndGetDefaultFieldsRequest)

 [BulkUpdate\_RemoveStoredFieldsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.BulkUpdate_RemoveStoredFieldsRequest)

 [Business](/en/api/reference/webapi/SuperOffice.WebApi.Data.Business)

Carrier object for Business.

 [CRMScriptEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScriptEntity)

Carrier object for CRMScriptEntity.
CRM Scripts - stored in the ejScript table, including source code.

 [CRMScriptErrorInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScriptErrorInfo)

Carrier object for CRMScriptErrorInfo.
Object containing error information for a CRMScript

 [CRMScriptResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScriptResult)

Carrier object for CRMScriptResult.
Object for returning information when validating a CRMScript

 [CRMScriptRunResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScriptRunResult)

Carrier object for CRMScriptRunResult.
Object for returning result from a CRMScript

 [CRMScriptSourceMap](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScriptSourceMap)

Carrier object for CRMScriptSourceMap.
Object for returning source-map info when validating or executing a CRMScript

 [CRMScriptTraceRun](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScriptTraceRun)

Carrier object for CRMScriptTraceRun.
Object for returning Trace run result from a CRMScript

 [CRMScript\_DeleteCRMScriptByUniqueIdentifierRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_DeleteCRMScriptByUniqueIdentifierRequest)

 [CRMScript\_DeleteTriggerScriptByUniqueIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_DeleteTriggerScriptByUniqueIdRequest)

 [CRMScript\_ExecuteScriptAsEventByUniqueIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_ExecuteScriptAsEventByUniqueIdRequest)

 [CRMScript\_ExecuteScriptAsEventRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_ExecuteScriptAsEventRequest)

 [CRMScript\_ExecuteScriptByIncludeIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_ExecuteScriptByIncludeIdRequest)

 [CRMScript\_ExecuteScriptByStringRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_ExecuteScriptByStringRequest)

 [CRMScript\_ExecuteScriptByUniqueIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_ExecuteScriptByUniqueIdRequest)

 [CRMScript\_ExecuteScriptRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_ExecuteScriptRequest)

 [CRMScript\_GetCRMScriptByUniqueIdentifierRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_GetCRMScriptByUniqueIdentifierRequest)

 [CRMScript\_GetTriggerScriptByUniqueIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_GetTriggerScriptByUniqueIdRequest)

 [CRMScript\_ResolveIncludesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_ResolveIncludesRequest)

 [CRMScript\_SaveCRMScriptByUniqueIdentifierRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_SaveCRMScriptByUniqueIdentifierRequest)

 [CRMScript\_SaveCRMScriptEntityWithoutCompileRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_SaveCRMScriptEntityWithoutCompileRequest)

 [CRMScript\_SaveTriggerScriptByUniqueIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_SaveTriggerScriptByUniqueIdRequest)

 [CRMScript\_ValidateScriptByIncludeIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_ValidateScriptByIncludeIdRequest)

 [CRMScript\_ValidateScriptByStringRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_ValidateScriptByStringRequest)

 [CRMScript\_ValidateScriptByUniqueIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_ValidateScriptByUniqueIdRequest)

 [CRMScript\_ValidateScriptRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_ValidateScriptRequest)

 [CRMScript\_ValidateTriggerScriptByUniqueIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_ValidateTriggerScriptByUniqueIdRequest)

 [CRMScript\_ValidateTriggerScriptRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CRMScript_ValidateTriggerScriptRequest)

 [Cache](/en/api/reference/webapi/SuperOffice.WebApi.Data.Cache)

Carrier object for Cache.
Cache maintenance

 [CacheInvalidation](/en/api/reference/webapi/SuperOffice.WebApi.Data.CacheInvalidation)

Carrier object for CacheInvalidation.

 [CallerID](/en/api/reference/webapi/SuperOffice.WebApi.Data.CallerID)

Carrier object for CallerID.
Describes a caller ID for Mobile to lookup phone calls. Can be for a person, or for a company, in which case the personId field will be 0

 [CallerIDCollection](/en/api/reference/webapi/SuperOffice.WebApi.Data.CallerIDCollection)

Carrier object for CallerIDCollection.
Collection of caller ids with their corresponding hash

 [Carrier](/en/api/reference/webapi/SuperOffice.WebApi.Data.Carrier)

Carrier object for Carrier.
Base class for carrier objects Implementing common carrier properties

 [CategorizationModelDetails](/en/api/reference/webapi/SuperOffice.WebApi.Data.CategorizationModelDetails)

Carrier object for CategorizationModelDetails.
Details about a categorization model in HugoAI

 [CategorizationStatusResponse](/en/api/reference/webapi/SuperOffice.WebApi.Data.CategorizationStatusResponse)

Carrier object for CategorizationStatusResponse.
Carrier to describe the current categorization training status for a tenant in HugoAI

 [Category](/en/api/reference/webapi/SuperOffice.WebApi.Data.Category)

Carrier object for Category.

 [CategoryEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.CategoryEntity)

Carrier object for CategoryEntity.

 [CategoryGuess](/en/api/reference/webapi/SuperOffice.WebApi.Data.CategoryGuess)

Carrier object for CategoryGuess.
Category guess information

 [ChangedData](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChangedData)

Carrier object for ChangedData.
Generic carrier, used in return values to indicate what data NetServer has changed 'behind the scenes' so clients can know what to update

 [ChangedDataItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChangedDataItem)

Carrier object for ChangedDataItem.
Tablename/recordid pair used to refer to records

 [ChatBotSettings](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChatBotSettings)

Carrier object for ChatBotSettings.
Chat Topic bot settings. Trigger script ids to run on bot events

 [ChatMessage](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChatMessage)

Carrier object for ChatMessage.
A message in a chat session

 [ChatOpeningHours](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChatOpeningHours)

Carrier object for ChatOpeningHours.
Chat Topic Opening Hours

 [ChatPresence](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChatPresence)

Carrier object for ChatPresence.
Represents presence information for a chat user, bundled with some other useful information

 [ChatSession](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChatSession)

Carrier object for ChatSession.
Use the ChatSessionEntity on the Chat agent instead. Chat sessions belong to a chat topic, and contain messages to/from users

 [ChatSessionEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChatSessionEntity)

Carrier object for ChatSessionEntity.
Chat sessions belong to a chat topic, and contain messages to/from users

 [ChatSummaryItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChatSummaryItem)

Carrier object for ChatSummaryItem.
Chat session summary: id, title, created date

 [ChatTopicAgent](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChatTopicAgent)

Carrier object for ChatTopicAgent.
User agents within a chat topic. Which users are assigned and what roles they have within the chat topic.

 [ChatTopicEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChatTopicEntity)

Carrier object for ChatTopicEntity.
Chat topics define who is assigned, when the channel is open for business, and look of the chat widget.

 [ChatWidgetSettings](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChatWidgetSettings)

Carrier object for ChatWidgetSettings.
Defines the look of the chat widget.

 [Chat\_AcceptChatSessionTransferRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_AcceptChatSessionTransferRequest)

 [Chat\_AddChatMessageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_AddChatMessageRequest)

 [Chat\_AddChatTopicUserAgentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_AddChatTopicUserAgentRequest)

 [Chat\_ChatSessionsForUserRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_ChatSessionsForUserRequest)

 [Chat\_ChatTopicsForUserRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_ChatTopicsForUserRequest)

 [Chat\_CreateChatSessionForTopicRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_CreateChatSessionForTopicRequest)

 [Chat\_DeleteChatTopicUserAgentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_DeleteChatTopicUserAgentRequest)

 [Chat\_GetChatMessagesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_GetChatMessagesRequest)

 [Chat\_GetChatPresenceRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_GetChatPresenceRequest)

 [Chat\_GetChatTopicUserAgentListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_GetChatTopicUserAgentListRequest)

 [Chat\_GetChatTopicUserAgentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_GetChatTopicUserAgentRequest)

 [Chat\_GetChatTranscriptRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_GetChatTranscriptRequest)

 [Chat\_GetUserAgentListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_GetUserAgentListRequest)

 [Chat\_IsWithinOpeningHoursRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_IsWithinOpeningHoursRequest)

 [Chat\_PickUpChatSessionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_PickUpChatSessionRequest)

 [Chat\_PickUpFirstChatSessionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_PickUpFirstChatSessionRequest)

 [Chat\_RejectChatSessionTransferRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_RejectChatSessionTransferRequest)

 [Chat\_ResetChatSessionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_ResetChatSessionRequest)

 [Chat\_SaveChatPresenceRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_SaveChatPresenceRequest)

 [Chat\_TransferChatSessionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_TransferChatSessionRequest)

 [Chat\_UpdateChatTopicUserAgentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_UpdateChatTopicUserAgentRequest)

 [Chat\_UpdateChatTopicUserAgentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Chat_UpdateChatTopicUserAgentsRequest)

 [ChatbotTurn](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChatbotTurn)

Carrier object for ChatbotTurn.
Chat turn contains one user question, an optional display value for the user prompt, and one chatbot response.

 [CheckLicenseStatusResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.CheckLicenseStatusResult)

Carrier object for CheckLicenseStatusResult.
Result of CheckLicenseStatus

 [CheckoutInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.CheckoutInfo)

Carrier object for CheckoutInfo.
Information about the checked-out state of one document, describing whether it is checked out, and to whom.

 [UserPreferenceStrings.CloudServices](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.CloudServices)

Preferences for Cloud Services.

 [ColumnDataDictionary](/en/api/reference/webapi/SuperOffice.WebApi.Data.ColumnDataDictionary)

Serializable dictionary.
This class works as a regular dictionary. The only restriction is that the string is a value type

 [CommandInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.CommandInfo)

Carrier object for CommandInfo.
An instance of this class describes a custom command, declared and implemented by a document plugin

 [CommandResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.CommandResult)

Carrier object for CommandResult.
Generic carrier object for entity commands.

 [Competitor](/en/api/reference/webapi/SuperOffice.WebApi.Data.Competitor)

Carrier object for Competitor.

 [ConfigurableScreenDelta](/en/api/reference/webapi/SuperOffice.WebApi.Data.ConfigurableScreenDelta)

Carrier object for ConfigurableScreenDelta.
Configurable Screen Delta represents a list of changes to a SCIL recipe.

 [Configuration](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration)

Carrier object for Configuration.
This service supports fetching of client interface configuration data. Client configuration data include Application configuration, which defines how an application is composed out of pages; and page configuration that defines how a page is built out of panels, cards, views and controls. &lt;para/&gt; Separate access points also exist for fetching object mappings and filter lists.

 [Configuration\_ClearConfigurationCacheRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_ClearConfigurationCacheRequest)

 [Configuration\_DeleteWindowPosSizeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_DeleteWindowPosSizeRequest)

 [Configuration\_ExistsSystemEventRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_ExistsSystemEventRequest)

 [Configuration\_GetAllFeatureTogglesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetAllFeatureTogglesRequest)

 [Configuration\_GetAnyConfigurationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetAnyConfigurationRequest)

 [Configuration\_GetApplicationConfigurationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetApplicationConfigurationRequest)

 [Configuration\_GetCRMUrlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetCRMUrlRequest)

 [Configuration\_GetCSAuthUrlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetCSAuthUrlRequest)

 [Configuration\_GetCSRegistryValueRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetCSRegistryValueRequest)

 [Configuration\_GetCSWwwFolderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetCSWwwFolderRequest)

 [Configuration\_GetConfigurableScreenDeltasByDeltaRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetConfigurableScreenDeltasByDeltaRequest)

 [Configuration\_GetConfigurableScreenDeltasByQueryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetConfigurableScreenDeltasByQueryRequest)

 [Configuration\_GetCsCgiUrlInternalRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetCsCgiUrlInternalRequest)

 [Configuration\_GetCsCgiUrlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetCsCgiUrlRequest)

 [Configuration\_GetCsProgramUrlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetCsProgramUrlRequest)

 [Configuration\_GetCustomerUrlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetCustomerUrlRequest)

 [Configuration\_GetEMarketingUrlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetEMarketingUrlRequest)

 [Configuration\_GetEmailNumberOfDaysRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetEmailNumberOfDaysRequest)

 [Configuration\_GetFilterListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetFilterListRequest)

 [Configuration\_GetHelpDispatcherUrlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetHelpDispatcherUrlRequest)

 [Configuration\_GetLandingPageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetLandingPageRequest)

 [Configuration\_GetMyWindowPosSizesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetMyWindowPosSizesRequest)

 [Configuration\_GetObjectMappingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetObjectMappingRequest)

 [Configuration\_GetPageConfigurationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetPageConfigurationRequest)

 [Configuration\_GetRecipeAppliesToMappingsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetRecipeAppliesToMappingsRequest)

 [Configuration\_GetRefreshedPageConfigurationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetRefreshedPageConfigurationRequest)

 [Configuration\_GetScreenConfigurationDeltaRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetScreenConfigurationDeltaRequest)

 [Configuration\_GetScreenConfigurationDeltasRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetScreenConfigurationDeltasRequest)

 [Configuration\_GetSpfDomainRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetSpfDomainRequest)

 [Configuration\_GetWindowPosSizesOnAssociateIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetWindowPosSizesOnAssociateIdRequest)

 [Configuration\_GetWindowPosSizesOnPersonIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetWindowPosSizesOnPersonIdRequest)

 [Configuration\_GetWwwUrlForSMWebRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetWwwUrlForSMWebRequest)

 [Configuration\_GetWwwUrlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_GetWwwUrlRequest)

 [Configuration\_RebuildCustomObjectArchivePanelDeltasRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_RebuildCustomObjectArchivePanelDeltasRequest)

 [Configuration\_RebuildTaskMenuDeltasRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_RebuildTaskMenuDeltasRequest)

 [Configuration\_RebuildUdefDeltasRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_RebuildUdefDeltasRequest)

 [Configuration\_RebuildWebpanelDeltasRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_RebuildWebpanelDeltasRequest)

 [Configuration\_SaveAndPublishRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_SaveAndPublishRequest)

 [Configuration\_SaveLandingPageTabsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_SaveLandingPageTabsRequest)

 [Configuration\_SaveWindowPosSizeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_SaveWindowPosSizeRequest)

 [Configuration\_SaveWindowPosSizesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_SaveWindowPosSizesRequest)

 [Configuration\_SetAppliesToKeyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_SetAppliesToKeyRequest)

 [Configuration\_SetLandingPagePinForAssociateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Configuration_SetLandingPagePinForAssociateRequest)

 [ConsentInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.ConsentInfo)

Carrier object for ConsentInfo.
Consent information for a give person. Consents are for a particular Purpose, and may be withdrawn (see LegalBase).

 [ConsentPerson](/en/api/reference/webapi/SuperOffice.WebApi.Data.ConsentPerson)

Carrier object for ConsentPerson.

 [ConsentPurpose](/en/api/reference/webapi/SuperOffice.WebApi.Data.ConsentPurpose)

Carrier object for ConsentPurpose.

 [ConsentSource](/en/api/reference/webapi/SuperOffice.WebApi.Data.ConsentSource)

Carrier object for ConsentSource.

 [Contact](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact)

Carrier object for Contact.

 [ContactActivity](/en/api/reference/webapi/SuperOffice.WebApi.Data.ContactActivity)

Carrier object for ContactActivity.

 [ContactEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.ContactEntity)

Carrier object for ContactEntity.
The Contact Service. The service implements all services working with the Contact object

 [ContactOrPersonFromEmail](/en/api/reference/webapi/SuperOffice.WebApi.Data.ContactOrPersonFromEmail)

Carrier object for ContactOrPersonFromEmail.

 [ContactPersonBoardViewSettings](/en/api/reference/webapi/SuperOffice.WebApi.Data.ContactPersonBoardViewSettings)

Carrier object for ContactPersonBoardViewSettings.
Settings for contact/person selection board view.

 [ContactPersonId](/en/api/reference/webapi/SuperOffice.WebApi.Data.ContactPersonId)

Carrier object for ContactPersonId.
Class representing an ContactPersonId carrier used for storing either a person id or a contact id. If PersonId contains an id, ContactId will be 0, and the other way around.

 [ContactRelationEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.ContactRelationEntity)

Carrier object for ContactRelationEntity.
Relationship between a (contact/person) and another (contact/person), as described by the RelationDefinition.

 [ContactSummary](/en/api/reference/webapi/SuperOffice.WebApi.Data.ContactSummary)

Carrier object for ContactSummary.
Summary of contact with recent activities, chats, and requests included.

 [Contact\_AddPersonRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_AddPersonRequest)

 [Contact\_ChangeCountryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_ChangeCountryRequest)

 [Contact\_CopyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_CopyRequest)

 [Contact\_CreateNewEntryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_CreateNewEntryRequest)

 [Contact\_DeleteExpiredRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_DeleteExpiredRequest)

 [Contact\_EnrichContactEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_EnrichContactEntityRequest)

 [Contact\_GetAddressByCountryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_GetAddressByCountryRequest)

 [Contact\_GetAddressRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_GetAddressRequest)

 [Contact\_GetContactSummaryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_GetContactSummaryRequest)

 [Contact\_GetContactWithPersonsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_GetContactWithPersonsRequest)

 [Contact\_GetDomainDuplicatesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_GetDomainDuplicatesRequest)

 [Contact\_GetDuplicateRulesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_GetDuplicateRulesRequest)

 [Contact\_GetDuplicatesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_GetDuplicatesRequest)

 [Contact\_GetMyActiveContactsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_GetMyActiveContactsRequest)

 [Contact\_GetMyBizCardRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_GetMyBizCardRequest)

 [Contact\_GetMyContactRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_GetMyContactRequest)

 [Contact\_GetMyContactsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_GetMyContactsRequest)

 [Contact\_GetMyRecentContactsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_GetMyRecentContactsRequest)

 [Contact\_GetNameDepartmentDuplicatesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_GetNameDepartmentDuplicatesRequest)

 [Contact\_GetPersonsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_GetPersonsRequest)

 [Contact\_GetPreviewContactRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_GetPreviewContactRequest)

 [Contact\_GetPreviewEnrichContactRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_GetPreviewEnrichContactRequest)

 [Contact\_GetQuoteVersionAddressesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_GetQuoteVersionAddressesRequest)

 [Contact\_IsNumberValidRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_IsNumberValidRequest)

 [Contact\_MergeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_MergeRequest)

 [Contact\_SaveQuoteVersionAddressRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_SaveQuoteVersionAddressRequest)

 [Contact\_SetDuplicateRulesStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_SetDuplicateRulesStatusRequest)

 [Contact\_UndeleteRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_UndeleteRequest)

 [Contact\_UpdateInterestsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_UpdateInterestsRequest)

 [Contact\_ValidateContactEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Contact_ValidateContactEntityRequest)

 [ControlInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.ControlInfo)

Carrier object for ControlInfo.
This class carries information about the definition and properties of an field used in bulk update system. The class is serializable and used on both the application and Web server sides.

 [UserPreferenceStrings.Copilot](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Copilot)

Copilot Hugo AI

 [CopilotDataSourceEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.CopilotDataSourceEntity)

Carrier object for CopilotDataSourceEntity.
Copilot data source - contains a list of settings for this source

 [CopilotDataSourceSetting](/en/api/reference/webapi/SuperOffice.WebApi.Data.CopilotDataSourceSetting)

Carrier object for CopilotDataSourceSetting.
Setting for a Copilot data source (key + value)

 [CopilotEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.CopilotEntity)

Carrier object for CopilotEntity.
Entity for Copilot configuration

 [Country](/en/api/reference/webapi/SuperOffice.WebApi.Data.Country)

Carrier object for Country.

 [CreatePhotoBackground](/en/api/reference/webapi/SuperOffice.WebApi.Data.CreatePhotoBackground)

Carrier object for CreatePhotoBackground.
Photo background remover results.

 [Credential](/en/api/reference/webapi/SuperOffice.WebApi.Data.Credential)

Carrier object for Credential.
Credentials supported for authentication

 [CredentialType](/en/api/reference/webapi/SuperOffice.WebApi.Data.CredentialType)

Carrier object for CredentialType.
Description of credential type

 [CredentialUser](/en/api/reference/webapi/SuperOffice.WebApi.Data.CredentialUser)

Carrier object for CredentialUser.
Information about a users credentials in a foregin system that can be bound to a SuperOffice user (i.e. associate)

 [CredentialsGroup](/en/api/reference/webapi/SuperOffice.WebApi.Data.CredentialsGroup)

Carrier object for CredentialsGroup.
When the CredentialType's control is of type link, the user is selected from a source like active directory.  This will then be the organisational unit (e.g. user security group) in active directory.  Only one group is listed if the system does not support groups.

 [CredentialsGroupUsers](/en/api/reference/webapi/SuperOffice.WebApi.Data.CredentialsGroupUsers)

Carrier object for CredentialsGroupUsers.
This is the users within a user group.

 [Credited](/en/api/reference/webapi/SuperOffice.WebApi.Data.Credited)

Carrier object for Credited.

 [CriteriaInformation](/en/api/reference/webapi/SuperOffice.WebApi.Data.CriteriaInformation)

Carrier object for CriteriaInformation.
Carrier for criteria information. It contains all the search criteria - that is, ArchiveRestrictionInfo - objects, including a  column specification. In addition, it contains the same criteria expressed as an archive, with an array of ArchiveColumnInfo specifications and a set of ArchiveRow rows. The rows of the archive form a subset of the restriction array.

 [CsFeatureToggle](/en/api/reference/webapi/SuperOffice.WebApi.Data.CsFeatureToggle)

Carrier object for CsFeatureToggle.

 [CsSessionKey](/en/api/reference/webapi/SuperOffice.WebApi.Data.CsSessionKey)

Carrier object for CsSessionKey.

 [CultureDataFormatter](/en/api/reference/webapi/SuperOffice.WebApi.Data.CultureDataFormatter)

This is a helper class designed to encode, parse and reformat culturally sensitive data types between
an application server without culture knowledge, and a front-end with such knowledge.

 [Currency](/en/api/reference/webapi/SuperOffice.WebApi.Data.Currency)

Carrier object for Currency.

 [CurrencyEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.CurrencyEntity)

Carrier object for CurrencyEntity.

 [CustomCommand](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomCommand)

Carrier object for CustomCommand.
Generic carrier object for entity commands.

 [CustomObject](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomObject)

Carrier object for CustomObject.
A single row in a Custom Object table.

 [CustomObjectDefinition](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomObjectDefinition)

Carrier object for CustomObjectDefinition.
Schema definition for a Custom Object: the underlying table and its fields.

 [CustomObjectField](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomObjectField)

Carrier object for CustomObjectField.
Describes one field on a Custom Object Definition.

 [CustomObjectMetadata](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomObjectMetadata)

Carrier object for CustomObjectMetadata.
Bootstrap metadata for a Custom Object table.

 [CustomObjectRelation](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomObjectRelation)

Carrier object for CustomObjectRelation.
Describes a relation from a Custom Object to another entity.

 [CustomObject\_CreateDefaultCustomObjectRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomObject_CreateDefaultCustomObjectRequest)

 [CustomObject\_DeleteCustomObjectRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomObject_DeleteCustomObjectRequest)

 [CustomObject\_GetCustomObjectDefinitionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomObject_GetCustomObjectDefinitionRequest)

 [CustomObject\_GetCustomObjectRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomObject_GetCustomObjectRequest)

 [CustomObject\_GetCustomObjectsIconsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomObject_GetCustomObjectsIconsRequest)

 [CustomObject\_GetCustomObjectsMetadataRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomObject_GetCustomObjectsMetadataRequest)

 [CustomObject\_HasScreenChooserRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomObject_HasScreenChooserRequest)

 [CustomObject\_SaveCustomObjectRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomObject_SaveCustomObjectRequest)

 [CustomerCenterConfig](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerCenterConfig)

Carrier object for CustomerCenterConfig.
Contains configuration when displaying the Customer Center

 [CustomerLanguage](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerLanguage)

Carrier object for CustomerLanguage.
Customer languages that customer center supports.

 [CustomerServiceConfig](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerServiceConfig)

Carrier object for CustomerServiceConfig.
This contains different configuration methods regarding Service

 [CustomerServiceStartup](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerServiceStartup)

Carrier object for CustomerServiceStartup.
This carrier contains various data needed by Service for startup. Do not use this API, as it is for internal use and might change without notice

 [CustomerService\_AddMessageFromMailDataRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_AddMessageFromMailDataRequest)

 [CustomerService\_ChatSessionsForUserRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_ChatSessionsForUserRequest)

 [CustomerService\_CheckIfCustomizedTemplatesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_CheckIfCustomizedTemplatesRequest)

 [CustomerService\_CheckSymmetricEncryptionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_CheckSymmetricEncryptionRequest)

 [CustomerService\_CreateSessionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_CreateSessionRequest)

 [CustomerService\_CreateTicketFromMailDataRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_CreateTicketFromMailDataRequest)

 [CustomerService\_DeleteChatSessionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_DeleteChatSessionsRequest)

 [CustomerService\_EventHandlerExistsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_EventHandlerExistsRequest)

 [CustomerService\_EventHandlersExistsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_EventHandlersExistsRequest)

 [CustomerService\_ExecuteEventHandlersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_ExecuteEventHandlersRequest)

 [CustomerService\_FindTicketsByTitleOrIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_FindTicketsByTitleOrIdRequest)

 [CustomerService\_GetAllCustomerCenterConfigsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_GetAllCustomerCenterConfigsRequest)

 [CustomerService\_GetCustomerServiceStartupByOwnerRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_GetCustomerServiceStartupByOwnerRequest)

 [CustomerService\_GetCustomerServiceStartupRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_GetCustomerServiceStartupRequest)

 [CustomerService\_GetFaqForCustomerRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_GetFaqForCustomerRequest)

 [CustomerService\_GetMailboxesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_GetMailboxesRequest)

 [CustomerService\_GetParsedTemplateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_GetParsedTemplateRequest)

 [CustomerService\_GetPreviewFaqEntryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_GetPreviewFaqEntryRequest)

 [CustomerService\_GetPreviewQuickReplyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_GetPreviewQuickReplyRequest)

 [CustomerService\_GetPreviewReplyTemplateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_GetPreviewReplyTemplateRequest)

 [CustomerService\_GetProgramUrlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_GetProgramUrlRequest)

 [CustomerService\_GetSmsConfigRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_GetSmsConfigRequest)

 [CustomerService\_GetStatisticsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_GetStatisticsRequest)

 [CustomerService\_GetSystemTemplateSettingsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_GetSystemTemplateSettingsRequest)

 [CustomerService\_GetUnparsedTemplateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_GetUnparsedTemplateRequest)

 [CustomerService\_HasChatNotifyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_HasChatNotifyRequest)

 [CustomerService\_RemoveSessionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_RemoveSessionRequest)

 [CustomerService\_ReportSessionActiveRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_ReportSessionActiveRequest)

 [CustomerService\_SaveAllCustomerCenterConfigsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_SaveAllCustomerCenterConfigsRequest)

 [CustomerService\_SaveSystemTemplateSettingsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_SaveSystemTemplateSettingsRequest)

 [CustomerService\_SessionIsValidRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_SessionIsValidRequest)

 [CustomerService\_TestSmtpServerRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_TestSmtpServerRequest)

 [CustomerService\_UpdateFeatureTogglesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomerService_UpdateFeatureTogglesRequest)

 [Dash](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash)

Carrier object for Dash.
Dashboard configuration

 [DashCollection](/en/api/reference/webapi/SuperOffice.WebApi.Data.DashCollection)

Carrier object for DashCollection.
Collection of dashboards for an associate

 [DashTheme](/en/api/reference/webapi/SuperOffice.WebApi.Data.DashTheme)

Carrier object for DashTheme.
Dashboard theme configuration

 [DashTile](/en/api/reference/webapi/SuperOffice.WebApi.Data.DashTile)

Carrier object for DashTile.
Dashboard Tile configuration

 [DashTileDefinition](/en/api/reference/webapi/SuperOffice.WebApi.Data.DashTileDefinition)

Carrier object for DashTileDefinition.
Dashboard Tile definition

 [DashTileHtml](/en/api/reference/webapi/SuperOffice.WebApi.Data.DashTileHtml)

Carrier object for DashTileHtml.
HTML data for dash tiles with TileType = HTML

 [Dash\_AddNewTileToDashboardRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_AddNewTileToDashboardRequest)

 [Dash\_AddTileToDashboardRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_AddTileToDashboardRequest)

 [Dash\_CopyTileSelectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_CopyTileSelectionRequest)

 [Dash\_CreateDashTileSelectionFromEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_CreateDashTileSelectionFromEntityRequest)

 [Dash\_CreateDefaultTileDefinitionForEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_CreateDefaultTileDefinitionForEntityRequest)

 [Dash\_CreateDefaultTileForEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_CreateDefaultTileForEntityRequest)

 [Dash\_DuplicateDashboardRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_DuplicateDashboardRequest)

 [Dash\_DuplicateTileRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_DuplicateTileRequest)

 [Dash\_GetDashCollectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_GetDashCollectionRequest)

 [Dash\_GetDashListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_GetDashListRequest)

 [Dash\_GetDashQuickFiltersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_GetDashQuickFiltersRequest)

 [Dash\_GetDashTileHtmlListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_GetDashTileHtmlListRequest)

 [Dash\_GetDashTileHtmlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_GetDashTileHtmlRequest)

 [Dash\_GetDashTilesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_GetDashTilesRequest)

 [Dash\_GetDashboardFencingInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_GetDashboardFencingInfoRequest)

 [Dash\_GetPossibleDashQuickFiltersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_GetPossibleDashQuickFiltersRequest)

 [Dash\_GetPreviewDashRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_GetPreviewDashRequest)

 [Dash\_GetSecondaryTempTileSelectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_GetSecondaryTempTileSelectionRequest)

 [Dash\_GetTempTileSelectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_GetTempTileSelectionRequest)

 [Dash\_SaveDashQuickFiltersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_SaveDashQuickFiltersRequest)

 [Dash\_SaveDashTileHtmlListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_SaveDashTileHtmlListRequest)

 [Dash\_SaveDashTilesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dash_SaveDashTilesRequest)

 [UserPreferenceStrings.Dashboard](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Dashboard)

 [Dashboard](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dashboard)

Carrier object for Dashboard.
Dashboard configuration

 [DashboardTile](/en/api/reference/webapi/SuperOffice.WebApi.Data.DashboardTile)

Carrier object for DashboardTile.
Dashboard Tile configuration

 [DashboardTileOption](/en/api/reference/webapi/SuperOffice.WebApi.Data.DashboardTileOption)

Carrier object for DashboardTileOption.
Dashboard Tile option

 [Dashboard\_CreateNewFromTemplateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dashboard_CreateNewFromTemplateRequest)

 [Dashboard\_GetDashboardTilesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dashboard_GetDashboardTilesRequest)

 [Dashboard\_GetDashboardsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dashboard_GetDashboardsRequest)

 [Dashboard\_GetDataRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dashboard_GetDataRequest)

 [Dashboard\_GetDataWithSelectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dashboard_GetDataWithSelectionRequest)

 [Dashboard\_GetStandardDashboardTileFromChartIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dashboard_GetStandardDashboardTileFromChartIdRequest)

 [Dashboard\_SetTileRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Dashboard_SetTileRequest)

 [UserPreferenceStrings.DataCollection](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.DataCollection)

Data collection (usage statistics) - used for status storage and locking, not intended for user interaction

 [DataRight](/en/api/reference/webapi/SuperOffice.WebApi.Data.DataRight)

Carrier object for DataRight.

 [DataRights](/en/api/reference/webapi/SuperOffice.WebApi.Data.DataRights)

Carrier object for DataRights.

 [DatabaseTable\_DeleteRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.DatabaseTable_DeleteRequest)

 [DatabaseTable\_DeleteRowRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.DatabaseTable_DeleteRowRequest)

 [DatabaseTable\_InsertRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.DatabaseTable_InsertRequest)

 [DatabaseTable\_InsertRowRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.DatabaseTable_InsertRowRequest)

 [DatabaseTable\_ReadRowRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.DatabaseTable_ReadRowRequest)

 [DatabaseTable\_TruncateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.DatabaseTable_TruncateRequest)

 [DatabaseTable\_UpdateRowRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.DatabaseTable_UpdateRowRequest)

 [DatabaseTable\_UpsertRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.DatabaseTable_UpsertRequest)

 [Database\_GetStepsFromNetServerCodeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Database_GetStepsFromNetServerCodeRequest)

 [Database\_GetStepsInDatabaseRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Database_GetStepsInDatabaseRequest)

 [DayInformationListItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.DayInformationListItem)

Carrier object for DayInformationListItem.
Summary information about the activites and holidaysfor one day

 [UserPreferenceStrings.DbOptimizations](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.DbOptimizations)

Boss mod preferences for database-dependent special optimizations

 [UserPreferenceStrings.Debug](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Debug)

 [UserPreferenceStrings.Defaults](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Defaults)

Default values

 [DeliveryTerm](/en/api/reference/webapi/SuperOffice.WebApi.Data.DeliveryTerm)

Carrier object for DeliveryTerm.
The built-in delivery terms list. Used by the SuperOffice Quote Connector in quote versions.

 [DeliveryType](/en/api/reference/webapi/SuperOffice.WebApi.Data.DeliveryType)

Carrier object for DeliveryType.
The built-in delivery types list. Used by the SuperOffice Quote Connector in quote versions.

 [DeltaQuery](/en/api/reference/webapi/SuperOffice.WebApi.Data.DeltaQuery)

Carrier object for DeltaQuery.
Query object to use to fetch configruablescreendeltas matching this object

 [Department](/en/api/reference/webapi/SuperOffice.WebApi.Data.Department)

Carrier object for Department.
a.k.a. User Group

 [Diagnostics\_AddWebAppUsageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_AddWebAppUsageRequest)

 [Diagnostics\_ChangeLogSettingsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_ChangeLogSettingsRequest)

 [Diagnostics\_CollectDataAdditionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_CollectDataAdditionsRequest)

 [Diagnostics\_CollectTableSizesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_CollectTableSizesRequest)

 [Diagnostics\_CollectWebUsageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_CollectWebUsageRequest)

 [Diagnostics\_CollectWinUsageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_CollectWinUsageRequest)

 [Diagnostics\_FlushCachesByNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_FlushCachesByNameRequest)

 [Diagnostics\_FlushCachesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_FlushCachesRequest)

 [Diagnostics\_GetAnalyticsDataRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_GetAnalyticsDataRequest)

 [Diagnostics\_GetCacheInvalidationGenerationsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_GetCacheInvalidationGenerationsRequest)

 [Diagnostics\_GetCacheNamesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_GetCacheNamesRequest)

 [Diagnostics\_GetCacheStateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_GetCacheStateRequest)

 [Diagnostics\_GetEntityCountsForAllUsersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_GetEntityCountsForAllUsersRequest)

 [Diagnostics\_GetEntityCountsForCurrentUserRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_GetEntityCountsForCurrentUserRequest)

 [Diagnostics\_GetNextTableNumberRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_GetNextTableNumberRequest)

 [Diagnostics\_GetSystemInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_GetSystemInfoRequest)

 [Diagnostics\_GetSystemMessagesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_GetSystemMessagesRequest)

 [Diagnostics\_GetWebAppUsagesForPeriodRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_GetWebAppUsagesForPeriodRequest)

 [Diagnostics\_LogViewStateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_LogViewStateRequest)

 [Diagnostics\_PerformTasksAfterCustomObjectTableCreatedRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_PerformTasksAfterCustomObjectTableCreatedRequest)

 [Diagnostics\_PerformTasksAfterCustomObjectTableDeletedRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_PerformTasksAfterCustomObjectTableDeletedRequest)

 [Diagnostics\_PerformTasksAfterUpgradeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_PerformTasksAfterUpgradeRequest)

 [Diagnostics\_RegisterClosedTicketsUsageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_RegisterClosedTicketsUsageRequest)

 [Diagnostics\_RemoveExpiredSystemMessagesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_RemoveExpiredSystemMessagesRequest)

 [Diagnostics\_RemoveSystemMessageForAssociateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_RemoveSystemMessageForAssociateRequest)

 [Diagnostics\_RemoveSystemMessageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_RemoveSystemMessageRequest)

 [Diagnostics\_ResyncUsersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_ResyncUsersRequest)

 [Diagnostics\_SendSystemMessageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_SendSystemMessageRequest)

 [Diagnostics\_TrackEventRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_TrackEventRequest)

 [Diagnostics\_TrackUserRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_TrackUserRequest)

 [Diagnostics\_WebAppUsageExistsInPeriodRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Diagnostics_WebAppUsageExistsInPeriodRequest)

 [UserPreferenceStrings.DiaryView](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.DiaryView)

Diary

 [DiaryViewEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.DiaryViewEntity)

Carrier object for DiaryViewEntity.
The DiaryView Service. The service implements all services working with the DiaryView object.

 [DictionaryStepInformation](/en/api/reference/webapi/SuperOffice.WebApi.Data.DictionaryStepInformation)

Carrier object for DictionaryStepInformation.
Information about a dictionary step

 [UserPreferenceStrings.DocPluginNames](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.DocPluginNames)

Doc Plugin Addons: [DocPluginNames] plugin-id = prog.id.  The prog.id is requred and
matches COM ProgId property of the document plugin class.

 [UserPreferenceStrings.DocTemplateTypeMap](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.DocTemplateTypeMap)

DocTemplate Type Mapping: [DocTemplateTypeMap] .xyz = .docx
<p></p>Maps a template's unknown file type to a know file type before tag-substitution logic
is run on the template

 [Document](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document)

Carrier object for Document.

 [DocumentEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocumentEntity)

Carrier object for DocumentEntity.

 [DocumentMigrationAssociate](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocumentMigrationAssociate)

Carrier object for DocumentMigrationAssociate.
Class used to map relationship between AssociateId and the user's email.

 [DocumentMigrationItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocumentMigrationItem)

Carrier object for DocumentMigrationItem.
Class used to summarize all document-properties relevant for migrating documents.

 [DocumentMigrationItemList](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocumentMigrationItemList)

Carrier object for DocumentMigrationItemList.

 [DocumentMigration\_GetForAllDocumentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocumentMigration_GetForAllDocumentsRequest)

 [DocumentMigration\_GetForDateRangeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocumentMigration_GetForDateRangeRequest)

 [DocumentMigration\_GetForSelectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocumentMigration_GetForSelectionRequest)

 [DocumentMigration\_GetForTemplatesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocumentMigration_GetForTemplatesRequest)

 [DocumentMigration\_GetNumberOfConfidentialDocumentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocumentMigration_GetNumberOfConfidentialDocumentsRequest)

 [DocumentMigration\_HasDocumentsToMigrateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocumentMigration_HasDocumentsToMigrateRequest)

 [DocumentMigration\_MigrateDocumentToNewDocPluginRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocumentMigration_MigrateDocumentToNewDocPluginRequest)

 [DocumentMigration\_MigrateTemplateToNewDocPluginRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocumentMigration_MigrateTemplateToNewDocPluginRequest)

 [DocumentPreview](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocumentPreview)

Carrier object for DocumentPreview.
Document data intended for preview purposes

 [DocumentTemplate](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocumentTemplate)

Carrier object for DocumentTemplate.
The template a document is based on is stored in the SO_arc\template folder on the server. This will return the name as displayed in the GUI, and not the physical document name, of the template the document object is based on.

 [DocumentTemplateEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocumentTemplateEntity)

Carrier object for DocumentTemplateEntity.
The template a document is based on is stored in the SO_arc\template folder on the server. This will return the name as displayed in the GUI, and not the physical document name, of the template the document object is based on.

 [DocumentTemplateMigrationList](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocumentTemplateMigrationList)

Carrier object for DocumentTemplateMigrationList.

 [Document\_CheckinDocumentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_CheckinDocumentRequest)

 [Document\_CheckoutDocumentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_CheckoutDocumentRequest)

 [Document\_CopyDocumentToCsAttachmentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_CopyDocumentToCsAttachmentRequest)

 [Document\_CreateDefaultDocumentEntityFromSuggestionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_CreateDefaultDocumentEntityFromSuggestionRequest)

 [Document\_CreateDocumentStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_CreateDocumentStreamRequest)

 [Document\_CreateNewPhysicalDocumentFromTemplateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_CreateNewPhysicalDocumentFromTemplateRequest)

 [Document\_CreateNewPhysicalDocumentFromTemplateWithCustomTags2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_CreateNewPhysicalDocumentFromTemplateWithCustomTags2Request)

 [Document\_CreateNewPhysicalDocumentFromTemplateWithCustomTagsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_CreateNewPhysicalDocumentFromTemplateWithCustomTagsRequest)

 [Document\_CreateNewPhysicalMailMergeDocumentFromTemplateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_CreateNewPhysicalMailMergeDocumentFromTemplateRequest)

 [Document\_CreateTempFileRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_CreateTempFileRequest)

 [Document\_DeletePhysicalDocumentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_DeletePhysicalDocumentRequest)

 [Document\_DeleteTempFileRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_DeleteTempFileRequest)

 [Document\_ExecuteDocumentCommandRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_ExecuteDocumentCommandRequest)

 [Document\_GetAppointmentDocumentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetAppointmentDocumentsRequest)

 [Document\_GetCheckoutStateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetCheckoutStateRequest)

 [Document\_GetContactDocumentsByTemplateHeadingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetContactDocumentsByTemplateHeadingRequest)

 [Document\_GetContactDocumentsByTemplateTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetContactDocumentsByTemplateTypeRequest)

 [Document\_GetContactDocumentsByTemplateTypesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetContactDocumentsByTemplateTypesRequest)

 [Document\_GetContactDocumentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetContactDocumentsRequest)

 [Document\_GetDocumentCommandsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetDocumentCommandsRequest)

 [Document\_GetDocumentLengthRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetDocumentLengthRequest)

 [Document\_GetDocumentPropertiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetDocumentPropertiesRequest)

 [Document\_GetDocumentStreamFromEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetDocumentStreamFromEntityRequest)

 [Document\_GetDocumentStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetDocumentStreamRequest)

 [Document\_GetDocumentUrlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetDocumentUrlRequest)

 [Document\_GetDocumentsByTemplateHeadingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetDocumentsByTemplateHeadingRequest)

 [Document\_GetMyPublishedDocumentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetMyPublishedDocumentsRequest)

 [Document\_GetPersonDocumentsByDateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetPersonDocumentsByDateRequest)

 [Document\_GetPersonDocumentsByTemplateHeadingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetPersonDocumentsByTemplateHeadingRequest)

 [Document\_GetPersonDocumentsByTemplateTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetPersonDocumentsByTemplateTypeRequest)

 [Document\_GetPersonDocumentsByTemplateTypesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetPersonDocumentsByTemplateTypesRequest)

 [Document\_GetPersonDocumentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetPersonDocumentsRequest)

 [Document\_GetPluginCapabilitiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetPluginCapabilitiesRequest)

 [Document\_GetPluginListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetPluginListRequest)

 [Document\_GetPreviewDocumentStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetPreviewDocumentStreamRequest)

 [Document\_GetProjectDocumentsByTemplateHeadingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetProjectDocumentsByTemplateHeadingRequest)

 [Document\_GetProjectDocumentsByTemplateTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetProjectDocumentsByTemplateTypeRequest)

 [Document\_GetProjectDocumentsByTemplateTypesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetProjectDocumentsByTemplateTypesRequest)

 [Document\_GetProjectDocumentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetProjectDocumentsRequest)

 [Document\_GetProjectMemberDocumentsByTemplateHeadingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetProjectMemberDocumentsByTemplateHeadingRequest)

 [Document\_GetProjectMemberDocumentsByTemplateTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetProjectMemberDocumentsByTemplateTypeRequest)

 [Document\_GetProjectMemberDocumentsByTemplateTypesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetProjectMemberDocumentsByTemplateTypesRequest)

 [Document\_GetProjectMemberDocumentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetProjectMemberDocumentsRequest)

 [Document\_GetPublishedDocumentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetPublishedDocumentRequest)

 [Document\_GetPublishedDocumentsByDateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetPublishedDocumentsByDateRequest)

 [Document\_GetPublishedDocumentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetPublishedDocumentsRequest)

 [Document\_GetPublishedPersonDocumentsByDateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetPublishedPersonDocumentsByDateRequest)

 [Document\_GetPublishedProjectDocumentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetPublishedProjectDocumentsRequest)

 [Document\_GetSaleDocumentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetSaleDocumentsRequest)

 [Document\_GetSanitizedDocumentStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetSanitizedDocumentStreamRequest)

 [Document\_GetSanitizedTemplateStreamFromIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetSanitizedTemplateStreamFromIdRequest)

 [Document\_GetSanitizedTemplateStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetSanitizedTemplateStreamRequest)

 [Document\_GetTempFileRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetTempFileRequest)

 [Document\_GetTemplateStreamFromIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetTemplateStreamFromIdRequest)

 [Document\_GetTemplateStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetTemplateStreamRequest)

 [Document\_GetUnsanitizedPreviewDocumentStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetUnsanitizedPreviewDocumentStreamRequest)

 [Document\_GetVersionListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_GetVersionListRequest)

 [Document\_RenameDocumentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_RenameDocumentRequest)

 [Document\_SaveDocumentTemplateStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_SaveDocumentTemplateStreamRequest)

 [Document\_SavePrivacyReportPdfRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_SavePrivacyReportPdfRequest)

 [Document\_SavePrivacyReportRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_SavePrivacyReportRequest)

 [Document\_SetDocumentStreamFromIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_SetDocumentStreamFromIdRequest)

 [Document\_SetDocumentStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_SetDocumentStreamRequest)

 [Document\_SetTemplateStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_SetTemplateStreamRequest)

 [Document\_SubstituteMergeDocumentTemplateVariables2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_SubstituteMergeDocumentTemplateVariables2Request)

 [Document\_SubstituteMergeDocumentTemplateVariablesEx2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_SubstituteMergeDocumentTemplateVariablesEx2Request)

 [Document\_SubstituteMergeDocumentTemplateVariablesExRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_SubstituteMergeDocumentTemplateVariablesExRequest)

 [Document\_SubstituteMergeDocumentTemplateVariablesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_SubstituteMergeDocumentTemplateVariablesRequest)

 [Document\_SubstituteTemplateVariablesExRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_SubstituteTemplateVariablesExRequest)

 [Document\_SubstituteTemplateVariablesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_SubstituteTemplateVariablesRequest)

 [Document\_SubstituteTemplateVariablesWithCustomTags2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_SubstituteTemplateVariablesWithCustomTags2Request)

 [Document\_SubstituteTemplateVariablesWithCustomTagsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_SubstituteTemplateVariablesWithCustomTagsRequest)

 [Document\_UndoCheckoutDocumentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_UndoCheckoutDocumentRequest)

 [Document\_ValidateDocumentEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_ValidateDocumentEntityRequest)

 [Document\_VerifyGetDocumentStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Document_VerifyGetDocumentStreamRequest)

 [DuplicateEntry](/en/api/reference/webapi/SuperOffice.WebApi.Data.DuplicateEntry)

Carrier object for DuplicateEntry.
A duplicate entry

 [DuplicateRule](/en/api/reference/webapi/SuperOffice.WebApi.Data.DuplicateRule)

Carrier object for DuplicateRule.
Describes one duplicate rule to be used to find similar entries

 [UserPreferenceStrings.DuplicateSystem](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.DuplicateSystem)

Duplicate system settings

 [EMailAccount](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMailAccount)

Carrier object for EMailAccount.
Properties for an email account

 [EMailAddress](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMailAddress)

Carrier object for EMailAddress.

 [EMailAppointment](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMailAppointment)

Carrier object for EMailAppointment.
Information about an email appointment received as iCal data

 [EMailAttachment](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMailAttachment)

Carrier object for EMailAttachment.
Information about an attachment

 [EMailConnectionInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMailConnectionInfo)

Carrier object for EMailConnectionInfo.
All information needed to connect to a mailserver

 [EMailConnectionInfoExtended](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMailConnectionInfoExtended)

Carrier object for EMailConnectionInfoExtended.

 [EMailCustomHeader](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMailCustomHeader)

Carrier object for EMailCustomHeader.

 [EMailEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMailEntity)

Carrier object for EMailEntity.
All information about an e-mail

 [EMailEnvelope](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMailEnvelope)

Carrier object for EMailEnvelope.
Limited information about one e-mail.

 [EMailFolder](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMailFolder)

Carrier object for EMailFolder.
Properties for a mail folder

 [EMailSOInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMailSOInfo)

Carrier object for EMailSOInfo.
Glue between SuperOffice data and an e-mail.

 [EMail\_AuthenticateIncomingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_AuthenticateIncomingRequest)

 [EMail\_AuthenticateOutgoingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_AuthenticateOutgoingRequest)

 [EMail\_AuthenticateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_AuthenticateRequest)

 [EMail\_CacheFolderListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_CacheFolderListRequest)

 [EMail\_CreateEMailFromDocumentEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_CreateEMailFromDocumentEntityRequest)

 [EMail\_CreateEMailFromMimeMessageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_CreateEMailFromMimeMessageRequest)

 [EMail\_CreateFolderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_CreateFolderRequest)

 [EMail\_CreateForwardEmailRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_CreateForwardEmailRequest)

 [EMail\_CreateInlineHtmlHeaderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_CreateInlineHtmlHeaderRequest)

 [EMail\_CreateNewPhysicalDocumentFromEmailAttachmentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_CreateNewPhysicalDocumentFromEmailAttachmentRequest)

 [EMail\_CreateNewPhysicalDocumentFromEmailRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_CreateNewPhysicalDocumentFromEmailRequest)

 [EMail\_DeleteEmailsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_DeleteEmailsRequest)

 [EMail\_DeleteExtendedRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_DeleteExtendedRequest)

 [EMail\_DeleteFolderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_DeleteFolderRequest)

 [EMail\_DeleteFromServerIdentifierRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_DeleteFromServerIdentifierRequest)

 [EMail\_DeleteRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_DeleteRequest)

 [EMail\_EmptyFolderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_EmptyFolderRequest)

 [EMail\_FindAddressRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_FindAddressRequest)

 [EMail\_FindContactOrPersonByEmailNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_FindContactOrPersonByEmailNameRequest)

 [EMail\_FindContactOrPersonByEmailNameWithLimitRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_FindContactOrPersonByEmailNameWithLimitRequest)

 [EMail\_GetAttachmentFromIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetAttachmentFromIdRequest)

 [EMail\_GetAttachmentFromServerIdentifierRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetAttachmentFromServerIdentifierRequest)

 [EMail\_GetAttachmentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetAttachmentRequest)

 [EMail\_GetCurrentAccountIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetCurrentAccountIdRequest)

 [EMail\_GetCurrentAccountRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetCurrentAccountRequest)

 [EMail\_GetEMailAccountFromEMailAddressRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEMailAccountFromEMailAddressRequest)

 [EMail\_GetEMailAddressesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEMailAddressesRequest)

 [EMail\_GetEMailAsEmlStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEMailAsEmlStreamRequest)

 [EMail\_GetEMailAsStreamFromServerIdentifierRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEMailAsStreamFromServerIdentifierRequest)

 [EMail\_GetEMailAsStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEMailAsStreamRequest)

 [EMail\_GetEMailEnvelopesFromServerIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEMailEnvelopesFromServerIdsRequest)

 [EMail\_GetEMailEnvelopesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEMailEnvelopesRequest)

 [EMail\_GetEMailFromAttachmentIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEMailFromAttachmentIdRequest)

 [EMail\_GetEMailFromDocumentAttachmentIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEMailFromDocumentAttachmentIdRequest)

 [EMail\_GetEMailFromDocumentIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEMailFromDocumentIdRequest)

 [EMail\_GetEMailFromDocumentIdWithoutAttachmentStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEMailFromDocumentIdWithoutAttachmentStreamRequest)

 [EMail\_GetEMailFromIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEMailFromIdRequest)

 [EMail\_GetEMailFromServerIdentifierRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEMailFromServerIdentifierRequest)

 [EMail\_GetEMailFromTempRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEMailFromTempRequest)

 [EMail\_GetEMailRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEMailRequest)

 [EMail\_GetEmailAppointmentRecurrenceRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEmailAppointmentRecurrenceRequest)

 [EMail\_GetEmailAppointmentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEmailAppointmentRequest)

 [EMail\_GetEmailDocumentAsMimeStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEmailDocumentAsMimeStreamRequest)

 [EMail\_GetEmailMessageIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEmailMessageIdsRequest)

 [EMail\_GetEmailsAsStringRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetEmailsAsStringRequest)

 [EMail\_GetExtendedEmailMessageIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetExtendedEmailMessageIdsRequest)

 [EMail\_GetFolderEMailCountRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetFolderEMailCountRequest)

 [EMail\_GetFolderHasNewEMailRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetFolderHasNewEMailRequest)

 [EMail\_GetFolderInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetFolderInfoRequest)

 [EMail\_GetFolderListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetFolderListRequest)

 [EMail\_GetMailFromInReplyToRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetMailFromInReplyToRequest)

 [EMail\_GetMailFromMessageIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetMailFromMessageIdRequest)

 [EMail\_GetNewMailRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetNewMailRequest)

 [EMail\_GetParticipantInfoFromEmailAddressRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetParticipantInfoFromEmailAddressRequest)

 [EMail\_GetPersonEntitiesFromEmailAddressRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetPersonEntitiesFromEmailAddressRequest)

 [EMail\_GetPersonsFromEmailAddressRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetPersonsFromEmailAddressRequest)

 [EMail\_GetPreviewAttachmentFromDocIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetPreviewAttachmentFromDocIdRequest)

 [EMail\_GetPreviewAttachmentFromIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetPreviewAttachmentFromIdRequest)

 [EMail\_GetReadySyncEmailAccountsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetReadySyncEmailAccountsRequest)

 [EMail\_GetUnsanitizedEMailFromAttachmentIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetUnsanitizedEMailFromAttachmentIdRequest)

 [EMail\_GetUnsanitizedEMailFromDocumentAttachmentIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetUnsanitizedEMailFromDocumentAttachmentIdRequest)

 [EMail\_GetUnsanitizedEMailFromDocumentIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetUnsanitizedEMailFromDocumentIdRequest)

 [EMail\_GetUnsanitizedEMailFromDocumentIdWithoutAttachmentStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetUnsanitizedEMailFromDocumentIdWithoutAttachmentStreamRequest)

 [EMail\_GetUnsanitizedEMailRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetUnsanitizedEMailRequest)

 [EMail\_GetUnsanitizedPreviewAttachmentFromDocIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetUnsanitizedPreviewAttachmentFromDocIdRequest)

 [EMail\_GetUnsanitizedPreviewAttachmentFromIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_GetUnsanitizedPreviewAttachmentFromIdRequest)

 [EMail\_HasMXRecordRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_HasMXRecordRequest)

 [EMail\_HasNewMailRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_HasNewMailRequest)

 [EMail\_HasSPFRecordRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_HasSPFRecordRequest)

 [EMail\_LatestItemIdByFolderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_LatestItemIdByFolderRequest)

 [EMail\_MarkAsReadFromServerIdentifiersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_MarkAsReadFromServerIdentifiersRequest)

 [EMail\_MarkAsReadRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_MarkAsReadRequest)

 [EMail\_MarkEmailsAsReadRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_MarkEmailsAsReadRequest)

 [EMail\_MoveEmailsToFolderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_MoveEmailsToFolderRequest)

 [EMail\_MoveEmailsToTrashRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_MoveEmailsToTrashRequest)

 [EMail\_MoveToFolderFromServerIdentifierRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_MoveToFolderFromServerIdentifierRequest)

 [EMail\_MoveToFolderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_MoveToFolderRequest)

 [EMail\_RefreshFolderForAssociateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_RefreshFolderForAssociateRequest)

 [EMail\_RefreshFolderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_RefreshFolderRequest)

 [EMail\_RelayMessageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_RelayMessageRequest)

 [EMail\_ResolveEMailRecipientsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_ResolveEMailRecipientsRequest)

 [EMail\_SaveDraftRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_SaveDraftRequest)

 [EMail\_SaveEMailRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_SaveEMailRequest)

 [EMail\_SaveEmailToTmpDocumentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_SaveEmailToTmpDocumentRequest)

 [EMail\_SaveRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_SaveRequest)

 [EMail\_SaveToMailServerRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_SaveToMailServerRequest)

 [EMail\_SendAndSaveEmailToTmpDocumentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_SendAndSaveEmailToTmpDocumentRequest)

 [EMail\_SendEMailsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_SendEMailsRequest)

 [EMail\_SendRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_SendRequest)

 [EMail\_SetRepliedAtRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_SetRepliedAtRequest)

 [EMail\_SetSubscriptionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_SetSubscriptionRequest)

 [EMail\_SyncEmailAccountRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_SyncEmailAccountRequest)

 [EMail\_SyncEmailAccountsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_SyncEmailAccountsRequest)

 [EMail\_TestAuthenticateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_TestAuthenticateRequest)

 [EMail\_ToggleSubscriptionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMail_ToggleSubscriptionRequest)

 [EmailFlow](/en/api/reference/webapi/SuperOffice.WebApi.Data.EmailFlow)

Carrier object for EmailFlow.
Email flow carrier

 [UserPreferenceStrings.EmailNotification](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.EmailNotification)

 [EntityCounts](/en/api/reference/webapi/SuperOffice.WebApi.Data.EntityCounts)

Carrier object for EntityCounts.

 [EntityElement](/en/api/reference/webapi/SuperOffice.WebApi.Data.EntityElement)

Carrier object for EntityElement.
Generic carrier object for entity elements such as Phone, EntityElement, Url. These elements do not have id and rank. These values are changed on each save, and should not be depended on. When used in a list or array the rank is the ascending list order.

 [EntityName](/en/api/reference/webapi/SuperOffice.WebApi.Data.EntityName)

Carrier object for EntityName.

 [ErpConnection](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpConnection)

Carrier object for ErpConnection.
Information about a connection to the ERP system.

 [ErpConnectionData](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpConnectionData)

Carrier object for ErpConnectionData.
Contains connection data and field labels for one connection

 [ErpConnectionListMapping](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpConnectionListMapping)

Carrier object for ErpConnectionListMapping.
Information about a mapping between two lists between CRM and the ERP system.

 [ErpConnectionListMappingContainer](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpConnectionListMappingContainer)

Carrier object for ErpConnectionListMappingContainer.
Information about all list connections for a connection between CRM and the ERP system.

 [ErpCrmListItemMapping](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpCrmListItemMapping)

Carrier object for ErpCrmListItemMapping.
One list item mapping between erp and crm.

 [ErpSyncActorSummary](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSyncActorSummary)

Carrier object for ErpSyncActorSummary.
Summary of the current setup of the ERP Integration, for one ERP actor

 [ErpSyncActorTypeMapping](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSyncActorTypeMapping)

Carrier object for ErpSyncActorTypeMapping.
Mapping information for one Actor, contains array of field mappings (ERP &lt;-&gt; SuperOffice)

 [ErpSyncConnectionSummary](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSyncConnectionSummary)

Carrier object for ErpSyncConnectionSummary.
Summary of the current setup of the ERP Integration, for one ERP connection

 [ErpSyncConnectorEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSyncConnectorEntity)

Carrier object for ErpSyncConnectorEntity.
Erp connector information

 [ErpSyncDefaultValue](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSyncDefaultValue)

Carrier object for ErpSyncDefaultValue.
Contains default value information for one ERP field

 [ErpSyncEngine](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSyncEngine)

Carrier object for ErpSyncEngine.
The ERP syncengine.

 [ErpSyncEngineStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSyncEngineStatus)

Carrier object for ErpSyncEngineStatus.
Status of the Sync engine

 [ErpSyncFieldMapping](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSyncFieldMapping)

Carrier object for ErpSyncFieldMapping.
Mapping information for one field pair (ERP &lt;-&gt; SuperOffice)

 [ErpSyncFieldValue](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSyncFieldValue)

Carrier object for ErpSyncFieldValue.

 [ErpSync\_BreakActorLinkRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_BreakActorLinkRequest)

 [ErpSync\_CanDeleteErpSyncConnectorEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_CanDeleteErpSyncConnectorEntityRequest)

 [ErpSync\_ChangeEngineIntervalRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_ChangeEngineIntervalRequest)

 [ErpSync\_ChangeEngineStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_ChangeEngineStatusRequest)

 [ErpSync\_ConnectActorRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_ConnectActorRequest)

 [ErpSync\_CreateActorLinkRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_CreateActorLinkRequest)

 [ErpSync\_CreateErpActorFromCrmRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_CreateErpActorFromCrmRequest)

 [ErpSync\_DeleteConnectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_DeleteConnectionRequest)

 [ErpSync\_ForceResyncBatchTaskRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_ForceResyncBatchTaskRequest)

 [ErpSync\_ForceResyncExternalNoBlankValuesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_ForceResyncExternalNoBlankValuesRequest)

 [ErpSync\_ForceResyncExternalRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_ForceResyncExternalRequest)

 [ErpSync\_ForceResyncNoBlankValuesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_ForceResyncNoBlankValuesRequest)

 [ErpSync\_ForceResyncRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_ForceResyncRequest)

 [ErpSync\_ForceSyncAllRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_ForceSyncAllRequest)

 [ErpSync\_GetActorTypeMappingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_GetActorTypeMappingRequest)

 [ErpSync\_GetAllConnectionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_GetAllConnectionsRequest)

 [ErpSync\_GetAllConnectorsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_GetAllConnectorsRequest)

 [ErpSync\_GetConfigurationFieldsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_GetConfigurationFieldsRequest)

 [ErpSync\_GetConnectionListMappingsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_GetConnectionListMappingsRequest)

 [ErpSync\_GetConnectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_GetConnectionRequest)

 [ErpSync\_GetConnectionsAndDisplayFieldsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_GetConnectionsAndDisplayFieldsRequest)

 [ErpSync\_GetCrmActorTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_GetCrmActorTypeRequest)

 [ErpSync\_GetDefaultValueInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_GetDefaultValueInfoRequest)

 [ErpSync\_GetEngineStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_GetEngineStatusRequest)

 [ErpSync\_GetErpConnectionConfigFieldsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_GetErpConnectionConfigFieldsRequest)

 [ErpSync\_GetErpFieldValuesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_GetErpFieldValuesRequest)

 [ErpSync\_GetErpSyncConnectionSummaryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_GetErpSyncConnectionSummaryRequest)

 [ErpSync\_GetFieldValuesFromCrmRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_GetFieldValuesFromCrmRequest)

 [ErpSync\_GetFieldValuesFromErpRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_GetFieldValuesFromErpRequest)

 [ErpSync\_GetFieldsForNewErpActorRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_GetFieldsForNewErpActorRequest)

 [ErpSync\_MoveErpFieldItemRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_MoveErpFieldItemRequest)

 [ErpSync\_SaveActorTypeMappingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_SaveActorTypeMappingRequest)

 [ErpSync\_SaveConnectionListMappingsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_SaveConnectionListMappingsRequest)

 [ErpSync\_SaveConnectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_SaveConnectionRequest)

 [ErpSync\_SaveDefaultValueInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_SaveDefaultValueInfoRequest)

 [ErpSync\_SaveErpConnectionSyncPrioritiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_SaveErpConnectionSyncPrioritiesRequest)

 [ErpSync\_SupportsAdvancedSearchRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_SupportsAdvancedSearchRequest)

 [ErpSync\_SyncAllRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_SyncAllRequest)

 [ErpSync\_TestConnectionByConfigRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_TestConnectionByConfigRequest)

 [ErpSync\_TestConnectionByIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_TestConnectionByIdRequest)

 [ErpSync\_TestConnectorUrlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_TestConnectorUrlRequest)

 [ErpSync\_ToggleErpConnectionActiveRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_ToggleErpConnectionActiveRequest)

 [ErpSync\_TryConnectActorRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_TryConnectActorRequest)

 [ErpSync\_UpdateConnectionFieldsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_UpdateConnectionFieldsRequest)

 [ErpSync\_ValidateArchiveColumnConfigRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSync_ValidateArchiveColumnConfigRequest)

 [EventData](/en/api/reference/webapi/SuperOffice.WebApi.Data.EventData)

Carrier object for EventData.
EventData containing data related to event handlers

 [ExportArchiveResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.ExportArchiveResult)

Carrier object for ExportArchiveResult.

 [ExtAppEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.ExtAppEntity)

Carrier object for ExtAppEntity.
The ext app entity contains information on an external application

 [ExtendedLicenseInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.ExtendedLicenseInfo)

Carrier object for ExtendedLicenseInfo.

 [ExtendedModuleLicense](/en/api/reference/webapi/SuperOffice.WebApi.Data.ExtendedModuleLicense)

Carrier object for ExtendedModuleLicense.

 [UserPreferenceStrings.ExtraFields](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.ExtraFields)

 [Favourite](/en/api/reference/webapi/SuperOffice.WebApi.Data.Favourite)

Carrier object for Favourite.
Favourite carrier.

 [Favourite\_AddFavouriteRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Favourite_AddFavouriteRequest)

 [Favourite\_AddFavouritesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Favourite_AddFavouritesRequest)

 [Favourite\_AddTicketsToFavouritesByProviderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Favourite_AddTicketsToFavouritesByProviderRequest)

 [Favourite\_GetFavouritesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Favourite_GetFavouritesRequest)

 [Favourite\_IsFavouriteRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Favourite_IsFavouriteRequest)

 [Favourite\_RemoveAllFavouritesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Favourite_RemoveAllFavouritesRequest)

 [Favourite\_RemoveFavouriteRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Favourite_RemoveFavouriteRequest)

 [Favourite\_RemoveFavouritesByIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Favourite_RemoveFavouritesByIdRequest)

 [Favourite\_RemoveFavouritesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Favourite_RemoveFavouritesRequest)

 [Favourite\_RemoveTicketsFromFavouritesByProviderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Favourite_RemoveTicketsFromFavouritesByProviderRequest)

 [Favourite\_ToggleFavouriteRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Favourite_ToggleFavouriteRequest)

 [FencingInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.FencingInfo)

Carrier object for FencingInfo.
Information about entity fencing status, including current count and maximum active entities

 [FieldInfoAttachment](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldInfoAttachment)

Carrier object for FieldInfoAttachment.
Attachment custom database field.

 [FieldInfoBase](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldInfoBase)

Carrier object for FieldInfoBase.
Abstract basic information about database fields. May be custom or user-defined fields. The information can be used to build a user interface.

 [FieldInfoBlob](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldInfoBlob)

Carrier object for FieldInfoBlob.
Blob custom database field. Base64 encoded string value.

 [FieldInfoBool](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldInfoBool)

Carrier object for FieldInfoBool.
Boolean custom database field. Typically rendered as a checkbox.

 [FieldInfoDate](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldInfoDate)

Carrier object for FieldInfoDate.
Date custom database field. Time component is ignored.

 [FieldInfoDateTime](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldInfoDateTime)

Carrier object for FieldInfoDateTime.
Date + time custom database field.

 [FieldInfoDecimal](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldInfoDecimal)

Carrier object for FieldInfoDecimal.
Decimal (double precision) custom database field.

 [FieldInfoDynamicLink](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldInfoDynamicLink)

Carrier object for FieldInfoDynamicLink.
Dynamic Link custom database field. Links are resolved indirectly via by CS.

 [FieldInfoInt](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldInfoInt)

Carrier object for FieldInfoInt.
Integer custom database field.

 [FieldInfoListText](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldInfoListText)

Carrier object for FieldInfoListText.
Text string with a list of item choices - custom database field.

 [FieldInfoLongText](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldInfoLongText)

Carrier object for FieldInfoLongText.
Long Text custom database field.

 [FieldInfoMdoList](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldInfoMdoList)

Carrier object for FieldInfoMdoList.
MDO List custom database field.

 [FieldInfoRelation](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldInfoRelation)

Carrier object for FieldInfoRelation.
Relation to other table database field.

 [FieldInfoShortText](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldInfoShortText)

Carrier object for FieldInfoShortText.
Short text string custom database field.

 [FieldInfoTime](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldInfoTime)

Carrier object for FieldInfoTime.
Time custom database field. Date value is ignored.

 [FieldInfoTimeSpan](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldInfoTimeSpan)

Carrier object for FieldInfoTimeSpan.
Timespan custom database field. Number of minutes, represented as days/hours/minutes.

 [UserPreferenceStrings.FieldLabels](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.FieldLabels)

Field labels

 [FieldMetadata](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldMetadata)

Carrier object for FieldMetadata.
This is used to populate the Admin configuration dialog for a connection. The call to the ERP Connector’s GetConfigurationFields method returns a list of fields and field types. The Admin client builds a dialog with these fields

 [FieldMetadataDictionary](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldMetadataDictionary)

Serializable dictionary.
This class works as a regular dictionary. The only restriction is that the string is a value type

 [FieldProperty](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldProperty)

Carrier object for FieldProperty.
Class holding properties about a field in the carrier.

 [FieldPropertyDictionary](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldPropertyDictionary)

Serializable dictionary.
This class works as a regular dictionary. The only restriction is that the string is a value type

 [UserPreferenceStrings.FieldReplication](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.FieldReplication)

Enables replication of user-defined fields at field level instead of record level. This ensures that field changes for the customer are retained, but may affect the performance of the replication process

 [FieldRight](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldRight)

Carrier object for FieldRight.
Wraps a field right together with an explanatory reason.

 [FieldValue](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldValue)

Carrier object for FieldValue.
FieldValue carrier

 [FieldValueInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldValueInfo)

Carrier object for FieldValueInfo.
This class carries information about the definition and properties of an field used in bulk update system. The class is serializable and used on both the application and Web server sides.

 [UserPreferenceStrings.Filter](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Filter)

Filter (these are available only in PointyHairedBossMode in admin and thus cannot be easily edited)

 [UserPreferenceStrings.FilterDialog](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.FilterDialog)

Filter dialog settings, editable through SoAdmin

 [UserPreferenceStrings.FindResultDialog](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.FindResultDialog)

Find result dialog

 [FindResults](/en/api/reference/webapi/SuperOffice.WebApi.Data.FindResults)

Carrier object for FindResults.
Result carrier for the Find operation. It contains a set of column specifications, and a set of row, where each row contains the columns. The row set is the result of carrying out some search operation.

 [Find\_CreateRestrictionGroupRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_CreateRestrictionGroupRequest)

 [Find\_DeleteRestrictionGroupRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_DeleteRestrictionGroupRequest)

 [Find\_FindFromRestrictions2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_FindFromRestrictions2Request)

 [Find\_FindFromRestrictionsColumns2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_FindFromRestrictionsColumns2Request)

 [Find\_FindFromRestrictionsColumnsOrderBy2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_FindFromRestrictionsColumnsOrderBy2Request)

 [Find\_FindFromRestrictionsColumnsOrderByRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_FindFromRestrictionsColumnsOrderByRequest)

 [Find\_FindFromRestrictionsColumnsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_FindFromRestrictionsColumnsRequest)

 [Find\_FindFromRestrictionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_FindFromRestrictionsRequest)

 [Find\_FindOrderBy2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_FindOrderBy2Request)

 [Find\_FindOrderByRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_FindOrderByRequest)

 [Find\_FindRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_FindRequest)

 [Find\_FindWithColumnsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_FindWithColumnsRequest)

 [Find\_FindWithExtraRestrictions2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_FindWithExtraRestrictions2Request)

 [Find\_FindWithExtraRestrictionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_FindWithExtraRestrictionsRequest)

 [Find\_GetAvailableRestrictionColumnsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_GetAvailableRestrictionColumnsRequest)

 [Find\_GetCriteriaInformationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_GetCriteriaInformationRequest)

 [Find\_GetCriteriaInformationWithContextRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_GetCriteriaInformationWithContextRequest)

 [Find\_GetDefaultDesiredColumnsFromRestrictions2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_GetDefaultDesiredColumnsFromRestrictions2Request)

 [Find\_GetDefaultDesiredColumnsFromRestrictionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_GetDefaultDesiredColumnsFromRestrictionsRequest)

 [Find\_GetDefaultDesiredColumnsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_GetDefaultDesiredColumnsRequest)

 [Find\_GetDefaultOrderByFromDesiredColumnsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_GetDefaultOrderByFromDesiredColumnsRequest)

 [Find\_GetDefaultOrderByRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_GetDefaultOrderByRequest)

 [Find\_GetRestrictionGroupRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_GetRestrictionGroupRequest)

 [Find\_GetRestrictionGroupsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_GetRestrictionGroupsRequest)

 [Find\_GetSpecifiedCriteriaInformationWithDefaultsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_GetSpecifiedCriteriaInformationWithDefaultsRequest)

 [Find\_GetSpecifiedCriteriaInformationWithDefaultsWithContextRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_GetSpecifiedCriteriaInformationWithDefaultsWithContextRequest)

 [Find\_PopulateRestrictionValuesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_PopulateRestrictionValuesRequest)

 [Find\_PopulateRestrictionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_PopulateRestrictionsRequest)

 [Find\_SaveRestrictionGroupRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_SaveRestrictionGroupRequest)

 [Find\_SaveRestrictionGroupsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_SaveRestrictionGroupsRequest)

 [Find\_SaveRestrictions2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_SaveRestrictions2Request)

 [Find\_SaveRestrictionsAndGetCriteriaInformation2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_SaveRestrictionsAndGetCriteriaInformation2Request)

 [Find\_SaveRestrictionsAndGetCriteriaInformationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_SaveRestrictionsAndGetCriteriaInformationRequest)

 [Find\_SaveRestrictionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_SaveRestrictionsRequest)

 [Find\_SaveRestrictionsWithContext2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_SaveRestrictionsWithContext2Request)

 [Find\_SaveRestrictionsWithContextRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Find_SaveRestrictionsWithContextRequest)

 [ForeignAppEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignAppEntity)

Carrier object for ForeignAppEntity.

 [ForeignDevice](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignDevice)

Carrier object for ForeignDevice.
Saves a foreign device belonging to the ForeignDevice and application name specified.

 [ForeignKey](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignKey)

Carrier object for ForeignKey.

 [ForeignSystem\_AddDeviceByIdentifierRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_AddDeviceByIdentifierRequest)

 [ForeignSystem\_AddForeignKeyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_AddForeignKeyRequest)

 [ForeignSystem\_DeleteDeviceByIdentifierRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_DeleteDeviceByIdentifierRequest)

 [ForeignSystem\_DeleteForeignDeviceRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_DeleteForeignDeviceRequest)

 [ForeignSystem\_DeleteForeignKeyOnNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_DeleteForeignKeyOnNameRequest)

 [ForeignSystem\_DeleteForeignKeyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_DeleteForeignKeyRequest)

 [ForeignSystem\_GetAllForeignKeysOnEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_GetAllForeignKeysOnEntityRequest)

 [ForeignSystem\_GetAppByNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_GetAppByNameRequest)

 [ForeignSystem\_GetApplicationDevicesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_GetApplicationDevicesRequest)

 [ForeignSystem\_GetApplicationKeysRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_GetApplicationKeysRequest)

 [ForeignSystem\_GetDeviceByIdentifierRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_GetDeviceByIdentifierRequest)

 [ForeignSystem\_GetDeviceByNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_GetDeviceByNameRequest)

 [ForeignSystem\_GetDeviceKeysOnDeviceIdentifierRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_GetDeviceKeysOnDeviceIdentifierRequest)

 [ForeignSystem\_GetDeviceKeysOnDeviceIdentifierTableRecordIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_GetDeviceKeysOnDeviceIdentifierTableRecordIdRequest)

 [ForeignSystem\_GetDeviceKeysOnDeviceIdentifierTableRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_GetDeviceKeysOnDeviceIdentifierTableRequest)

 [ForeignSystem\_GetDeviceKeysRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_GetDeviceKeysRequest)

 [ForeignSystem\_GetKeyByValueAndIdentifierRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_GetKeyByValueAndIdentifierRequest)

 [ForeignSystem\_GetKeyByValueRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_GetKeyByValueRequest)

 [ForeignSystem\_GetKeyOnDeviceIdentifierRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_GetKeyOnDeviceIdentifierRequest)

 [ForeignSystem\_GetKeyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_GetKeyRequest)

 [ForeignSystem\_GetKeyValueOnDeviceIdentifierRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_GetKeyValueOnDeviceIdentifierRequest)

 [ForeignSystem\_GetKeyValueRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_GetKeyValueRequest)

 [ForeignSystem\_SaveDeviceByIdentifierRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_SaveDeviceByIdentifierRequest)

 [ForeignSystem\_SaveForeignDeviceRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_SaveForeignDeviceRequest)

 [ForeignSystem\_SaveForeignKeyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ForeignSystem_SaveForeignKeyRequest)

 [FormDesignCarrier](/en/api/reference/webapi/SuperOffice.WebApi.Data.FormDesignCarrier)

Carrier object for FormDesignCarrier.
Forms Design analysis results.

 [FormDesignCss](/en/api/reference/webapi/SuperOffice.WebApi.Data.FormDesignCss)

Carrier object for FormDesignCss.
Forms Design CSS class analysis result. Contains the CSS properties for a single named CSS class.

 [FormEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.FormEntity)

Carrier object for FormEntity.
Definition and configuration of a Form

 [FormFieldRestriction](/en/api/reference/webapi/SuperOffice.WebApi.Data.FormFieldRestriction)

Carrier object for FormFieldRestriction.
Carrier for FormFieldRestriction

 [FormSubmissionEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.FormSubmissionEntity)

Carrier object for FormSubmissionEntity.
Content of a Form submission

 [FreeText](/en/api/reference/webapi/SuperOffice.WebApi.Data.FreeText)

Carrier object for FreeText.
Used to manage freetext search.

 [FreeText\_FreetextIndexRowsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.FreeText_FreetextIndexRowsRequest)

 [FreeText\_GetStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.FreeText_GetStatusRequest)

 [FreeText\_RegenerateIndexRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.FreeText_RegenerateIndexRequest)

 [FreeText\_SetEnabledRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.FreeText_SetEnabledRequest)

 [FreeText\_SetMultiWordOperatorRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.FreeText_SetMultiWordOperatorRequest)

 [FreeText\_SetSingleWordOperatorRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.FreeText_SetSingleWordOperatorRequest)

 [UserPreferenceStrings.Freetext](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Freetext)

Freetext

 [FunctionRightStrings](/en/api/reference/webapi/SuperOffice.WebApi.Data.FunctionRightStrings)

 [UserPreferenceStrings.Functions](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Functions)

Functions

 [UserPreferenceStrings.Gdpr](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Gdpr)

GDPR - Misc

 [GrantedModuleLicense](/en/api/reference/webapi/SuperOffice.WebApi.Data.GrantedModuleLicense)

Carrier object for GrantedModuleLicense.
Represents a single module license granted to a user.

 [HeadingEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.HeadingEntity)

Carrier object for HeadingEntity.
The heading entity contains generic list item information

 [HierarchyEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.HierarchyEntity)

Carrier object for HierarchyEntity.
Folder structures

 [History](/en/api/reference/webapi/SuperOffice.WebApi.Data.History)

Carrier object for History.
The usage history of the currently logged in user is represented as History objects.

 [HistoryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.HistoryRequest)

Carrier object for HistoryRequest.
Specification for a request for history information, by id. It is possible to ask for rights for a new record (insert), but then you need to supply the parent ID for entities that have parents (projectmember, selectionmember, person)

 [UserPreferenceStrings.HugoAI](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.HugoAI)

 [ImportColumnInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.ImportColumnInfo)

Carrier object for ImportColumnInfo.
Column info for importlines.

 [ImportErpData](/en/api/reference/webapi/SuperOffice.WebApi.Data.ImportErpData)

Carrier object for ImportErpData.
Used to import data into the system. Representing one entity that will be imported.

 [ImportLine](/en/api/reference/webapi/SuperOffice.WebApi.Data.ImportLine)

Carrier object for ImportLine.
Used to import data into the system. Representing one entity that will be imported.

 [Import\_CreateDefaultImportLineRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Import_CreateDefaultImportLineRequest)

 [Import\_CreateErpImportDataRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Import_CreateErpImportDataRequest)

 [Import\_PreviewImportRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Import_PreviewImportRequest)

 [Import\_SaveImportRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Import_SaveImportRequest)

 [UserPreferenceStrings.InApp](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.InApp)

In-App

 [IncomingMessage](/en/api/reference/webapi/SuperOffice.WebApi.Data.IncomingMessage)

Carrier object for IncomingMessage.
Details about a message to be sent

 [Installation](/en/api/reference/webapi/SuperOffice.WebApi.Data.Installation)

Carrier object for Installation.
Operations related to installation, upgrade and maintenance

 [IntStringDictionary](/en/api/reference/webapi/SuperOffice.WebApi.Data.IntStringDictionary)

Serializable dictionary.
This class works as a regular dictionary. The only restriction is that the string is a value type

 [KbCategory](/en/api/reference/webapi/SuperOffice.WebApi.Data.KbCategory)

Carrier object for KbCategory.
Knowledge base category. Groups knowledge base articles into hierarchy.

 [LandingPageEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.LandingPageEntity)

Carrier object for LandingPageEntity.
Per-associate landing page configuration for an entity (type of landing page), including pinned selections and pinned entity records.

 [LandingPageTab](/en/api/reference/webapi/SuperOffice.WebApi.Data.LandingPageTab)

Carrier object for LandingPageTab.
A single tab on a landing page. Depending on TabType it represents a saved selection, an entity record, or a hardcoded provider.

 [LanguageInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.LanguageInfo)

Carrier object for LanguageInfo.

 [LegalBase](/en/api/reference/webapi/SuperOffice.WebApi.Data.LegalBase)

Carrier object for LegalBase.

 [License](/en/api/reference/webapi/SuperOffice.WebApi.Data.License)

Carrier object for License.
License information, hashed and signed with public keys to prevent tampering. Counts License usage and assignments.

 [UserPreferenceStrings.License](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.License)

License counters to track when to upgrade license

 [LicenseInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.LicenseInfo)

Carrier object for LicenseInfo.
Represents all licenses for this owner at this site.

 [LicenseOwner](/en/api/reference/webapi/SuperOffice.WebApi.Data.LicenseOwner)

Carrier object for LicenseOwner.

 [LicenseResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.LicenseResult)

Carrier object for LicenseResult.

 [License\_ActivateLicenseInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_ActivateLicenseInfoRequest)

 [License\_AddLicenseFromFileRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_AddLicenseFromFileRequest)

 [License\_AssignThirdPartyLicensesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_AssignThirdPartyLicensesRequest)

 [License\_ChangeSerialNumberRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_ChangeSerialNumberRequest)

 [License\_GetCentralLicenseRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_GetCentralLicenseRequest)

 [License\_GetFencingInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_GetFencingInfoRequest)

 [License\_GetLicenseForAllOwnersFromDBRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_GetLicenseForAllOwnersFromDBRequest)

 [License\_GetLicenseFromDBRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_GetLicenseFromDBRequest)

 [License\_GetLicenseFromFileRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_GetLicenseFromFileRequest)

 [License\_GetLicenseFromLicenseServerRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_GetLicenseFromLicenseServerRequest)

 [License\_GetLicenseStatusFromLicenseServerRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_GetLicenseStatusFromLicenseServerRequest)

 [License\_GetModuleLicenseFromDBRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_GetModuleLicenseFromDBRequest)

 [License\_GetModuleLicenseHistoryFromLicenseServerRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_GetModuleLicenseHistoryFromLicenseServerRequest)

 [License\_GetSatelliteLicensesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_GetSatelliteLicensesRequest)

 [License\_GetShopUrlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_GetShopUrlRequest)

 [License\_GetThirdPartyLicenseAssignmentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_GetThirdPartyLicenseAssignmentsRequest)

 [License\_GetUserAndInstallationLicenceStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_GetUserAndInstallationLicenceStatusRequest)

 [License\_GetUserLicensesMDOListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_GetUserLicensesMDOListRequest)

 [License\_GetUserLicensesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_GetUserLicensesRequest)

 [License\_RemoveLicenseFromDBRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_RemoveLicenseFromDBRequest)

 [License\_SaveCentralLicenseRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_SaveCentralLicenseRequest)

 [License\_UnassignThirdPartyLicensesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_UnassignThirdPartyLicensesRequest)

 [License\_ValidateLicenseInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.License_ValidateLicenseInfoRequest)

 [Link](/en/api/reference/webapi/SuperOffice.WebApi.Data.Link)

Carrier object for Link.
Links two entities (contacts/projects/sales/URLs) together.

 [ListEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.ListEntity)

Carrier object for ListEntity.
The list entity contains information about a specific list

 [ListItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.ListItem)

Carrier object for ListItem.
Lightweight generic list item - similar to MDOListItem but without child items and stuff

 [ListItemEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.ListItemEntity)

Carrier object for ListItemEntity.
The list item entity contains generic list item information

 [List\_AddFromListDefinitionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_AddFromListDefinitionRequest)

 [List\_AddFromListNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_AddFromListNameRequest)

 [List\_AddHierarchyToPathRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_AddHierarchyToPathRequest)

 [List\_ChangeCurrencyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_ChangeCurrencyRequest)

 [List\_CreateDefaultCountryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_CreateDefaultCountryRequest)

 [List\_CreateDefaultDocumentTemplateLanguageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_CreateDefaultDocumentTemplateLanguageRequest)

 [List\_CreateDefaultHeadingFromListDefinitionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_CreateDefaultHeadingFromListDefinitionRequest)

 [List\_DeleteAllFromListDefinitionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_DeleteAllFromListDefinitionRequest)

 [List\_DeleteAllFromListNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_DeleteAllFromListNameRequest)

 [List\_DeleteAllHeadingsFromListDefinitionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_DeleteAllHeadingsFromListDefinitionRequest)

 [List\_DeleteAllHeadingsFromNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_DeleteAllHeadingsFromNameRequest)

 [List\_DeleteAppTaskMenusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_DeleteAppTaskMenusRequest)

 [List\_DeleteAppWebPanelsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_DeleteAppWebPanelsRequest)

 [List\_DeleteDocumentTemplateLanguageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_DeleteDocumentTemplateLanguageRequest)

 [List\_DeleteFromListDefinitionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_DeleteFromListDefinitionRequest)

 [List\_DeleteFromListNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_DeleteFromListNameRequest)

 [List\_DeleteHierarchyFromPathRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_DeleteHierarchyFromPathRequest)

 [List\_DeleteListEntityByNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_DeleteListEntityByNameRequest)

 [List\_DeleteProjectStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_DeleteProjectStatusRequest)

 [List\_DeleteWebPanelRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_DeleteWebPanelRequest)

 [List\_GenerateNavigationUrlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GenerateNavigationUrlRequest)

 [List\_GetAllAutomatedCategoryUpdatesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetAllAutomatedCategoryUpdatesRequest)

 [List\_GetAllCountriesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetAllCountriesRequest)

 [List\_GetAllCurrenciesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetAllCurrenciesRequest)

 [List\_GetAllDocumentTemplatesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetAllDocumentTemplatesRequest)

 [List\_GetAllDomainsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetAllDomainsRequest)

 [List\_GetAllFromListDefinitionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetAllFromListDefinitionRequest)

 [List\_GetAllFromListNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetAllFromListNameRequest)

 [List\_GetAllInDomainRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetAllInDomainRequest)

 [List\_GetAllSaleStagesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetAllSaleStagesRequest)

 [List\_GetAllSaleTypeEntitiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetAllSaleTypeEntitiesRequest)

 [List\_GetAllSaleTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetAllSaleTypeRequest)

 [List\_GetAllTasksRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetAllTasksRequest)

 [List\_GetAllTicketPrioritiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetAllTicketPrioritiesRequest)

 [List\_GetAllTicketRelationDefinitionEntitiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetAllTicketRelationDefinitionEntitiesRequest)

 [List\_GetAppTaskMenusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetAppTaskMenusRequest)

 [List\_GetAppWebPanelsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetAppWebPanelsRequest)

 [List\_GetBaseCurrencyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetBaseCurrencyRequest)

 [List\_GetBusinessesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetBusinessesRequest)

 [List\_GetCategoriesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetCategoriesRequest)

 [List\_GetCompetitorsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetCompetitorsRequest)

 [List\_GetConsentPurposesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetConsentPurposesRequest)

 [List\_GetConsentSourcesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetConsentSourcesRequest)

 [List\_GetCountriesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetCountriesRequest)

 [List\_GetCreditedsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetCreditedsRequest)

 [List\_GetCurrenciesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetCurrenciesRequest)

 [List\_GetCustomerLanguagesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetCustomerLanguagesRequest)

 [List\_GetDeliveryTermsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetDeliveryTermsRequest)

 [List\_GetDeliveryTypesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetDeliveryTypesRequest)

 [List\_GetDepartmentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetDepartmentsRequest)

 [List\_GetDocumentTemplateExtensionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetDocumentTemplateExtensionRequest)

 [List\_GetDocumentTemplateLanguagesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetDocumentTemplateLanguagesRequest)

 [List\_GetDocumentTemplatePropertiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetDocumentTemplatePropertiesRequest)

 [List\_GetDocumentTemplateStreamFromIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetDocumentTemplateStreamFromIdRequest)

 [List\_GetDocumentTemplateTaskRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetDocumentTemplateTaskRequest)

 [List\_GetDocumentTemplateUrlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetDocumentTemplateUrlRequest)

 [List\_GetDocumentTemplateUsedInProjectStageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetDocumentTemplateUsedInProjectStageRequest)

 [List\_GetDocumentTemplateUsedInSalesStageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetDocumentTemplateUsedInSalesStageRequest)

 [List\_GetDocumentTemplatesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetDocumentTemplatesRequest)

 [List\_GetDocumentTemplatesTasksRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetDocumentTemplatesTasksRequest)

 [List\_GetDocumentTypesForPluginRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetDocumentTypesForPluginRequest)

 [List\_GetEmarketingStrictModeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetEmarketingStrictModeRequest)

 [List\_GetFromListDefinitionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetFromListDefinitionRequest)

 [List\_GetFromListNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetFromListNameRequest)

 [List\_GetHeadingsForListItemFromListNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetHeadingsForListItemFromListNameRequest)

 [List\_GetHeadingsFromListDefinitionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetHeadingsFromListDefinitionRequest)

 [List\_GetHeadingsFromNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetHeadingsFromNameRequest)

 [List\_GetHeadingsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetHeadingsRequest)

 [List\_GetHierarchyFromPathRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetHierarchyFromPathRequest)

 [List\_GetInstalledLanguagesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetInstalledLanguagesRequest)

 [List\_GetLegalBasesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetLegalBasesRequest)

 [List\_GetListDefinitionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetListDefinitionsRequest)

 [List\_GetListEntityByNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetListEntityByNameRequest)

 [List\_GetListItemsForHeadingFromListNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetListItemsForHeadingFromListNameRequest)

 [List\_GetListItemsForHeadingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetListItemsForHeadingRequest)

 [List\_GetListItemsForUserGroupFromListNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetListItemsForUserGroupFromListNameRequest)

 [List\_GetListItemsForUserGroupRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetListItemsForUserGroupRequest)

 [List\_GetLocalizedTextByTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetLocalizedTextByTypeRequest)

 [List\_GetLocalizedTextsByLanguageIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetLocalizedTextsByLanguageIdRequest)

 [List\_GetLocalizedTextsByTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetLocalizedTextsByTypeRequest)

 [List\_GetLocalizedTextsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetLocalizedTextsRequest)

 [List\_GetMrMrsesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetMrMrsesRequest)

 [List\_GetNeedCategoryUpdateForPersonRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetNeedCategoryUpdateForPersonRequest)

 [List\_GetNeedCategoryUpdateForSaleRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetNeedCategoryUpdateForSaleRequest)

 [List\_GetOurCurrencyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetOurCurrencyRequest)

 [List\_GetOwnerCompanysCurrencyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetOwnerCompanysCurrencyRequest)

 [List\_GetPaymentTermsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetPaymentTermsRequest)

 [List\_GetPaymentTypesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetPaymentTypesRequest)

 [List\_GetPluginCapabilitiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetPluginCapabilitiesRequest)

 [List\_GetPluginListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetPluginListRequest)

 [List\_GetPositionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetPositionsRequest)

 [List\_GetPrioritiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetPrioritiesRequest)

 [List\_GetProductCategoriesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetProductCategoriesRequest)

 [List\_GetProductFamiliesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetProductFamiliesRequest)

 [List\_GetProductTypesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetProductTypesRequest)

 [List\_GetProjectStatusesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetProjectStatusesRequest)

 [List\_GetProjectTypesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetProjectTypesRequest)

 [List\_GetQuickRepliesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetQuickRepliesRequest)

 [List\_GetQuoteApproveReasonsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetQuoteApproveReasonsRequest)

 [List\_GetQuoteDenyReasonsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetQuoteDenyReasonsRequest)

 [List\_GetRatingsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetRatingsRequest)

 [List\_GetReasonsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetReasonsRequest)

 [List\_GetSourcesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetSourcesRequest)

 [List\_GetTaskMenuByProgIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetTaskMenuByProgIdRequest)

 [List\_GetTasksRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetTasksRequest)

 [List\_GetTicketCategoriesForUserGroupsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetTicketCategoriesForUserGroupsRequest)

 [List\_GetTicketCategoriesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetTicketCategoriesRequest)

 [List\_GetTicketPrioritiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetTicketPrioritiesRequest)

 [List\_GetTicketStatusesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetTicketStatusesRequest)

 [List\_GetTicketTypesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetTicketTypesRequest)

 [List\_GetVisibleForUserGroupsFromListNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetVisibleForUserGroupsFromListNameRequest)

 [List\_GetVisibleForUserGroupsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetVisibleForUserGroupsRequest)

 [List\_GetWebPanelByProgIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetWebPanelByProgIdRequest)

 [List\_GetWebPanelListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GetWebPanelListRequest)

 [List\_GlobalChangeTicketStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GlobalChangeTicketStatusRequest)

 [List\_GlobalChangeTicketTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_GlobalChangeTicketTypeRequest)

 [List\_MoveAllTicketsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_MoveAllTicketsRequest)

 [List\_MoveListItemRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_MoveListItemRequest)

 [List\_PerformNeededCategoryUpdateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_PerformNeededCategoryUpdateRequest)

 [List\_RebuildFullnamesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_RebuildFullnamesRequest)

 [List\_SaveAllAutomatedCategoryUpdatesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveAllAutomatedCategoryUpdatesRequest)

 [List\_SaveAllFromListDefinitionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveAllFromListDefinitionRequest)

 [List\_SaveAllFromListNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveAllFromListNameRequest)

 [List\_SaveConsentPurposeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveConsentPurposeRequest)

 [List\_SaveConsentSourceRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveConsentSourceRequest)

 [List\_SaveCountryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveCountryRequest)

 [List\_SaveDocumentTemplateStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveDocumentTemplateStreamRequest)

 [List\_SaveFromListDefinitionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveFromListDefinitionRequest)

 [List\_SaveFromListNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveFromListNameRequest)

 [List\_SaveHeadingFromListDefinitionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveHeadingFromListDefinitionRequest)

 [List\_SaveHeadingFromNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveHeadingFromNameRequest)

 [List\_SaveHeadingsForListItemFromListDefinitionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveHeadingsForListItemFromListDefinitionRequest)

 [List\_SaveHeadingsForListItemFromListNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveHeadingsForListItemFromListNameRequest)

 [List\_SaveHeadingsFromListDefinitionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveHeadingsFromListDefinitionRequest)

 [List\_SaveHeadingsFromNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveHeadingsFromNameRequest)

 [List\_SaveLegalBaseRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveLegalBaseRequest)

 [List\_SaveListEntityByNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveListEntityByNameRequest)

 [List\_SaveListItemEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveListItemEntityRequest)

 [List\_SaveQuickRepliesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SaveQuickRepliesRequest)

 [List\_SetDocumentPluginForAllDocumentTemplatesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetDocumentPluginForAllDocumentTemplatesRequest)

 [List\_SetDocumentTemplateFromDocumentTemplateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetDocumentTemplateFromDocumentTemplateRequest)

 [List\_SetDocumentTemplateStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetDocumentTemplateStreamRequest)

 [List\_SetEmarketingStrictModeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetEmarketingStrictModeRequest)

 [List\_SetHeadingsForListItemRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetHeadingsForListItemRequest)

 [List\_SetListItemsForHeadingFromListNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetListItemsForHeadingFromListNameRequest)

 [List\_SetListItemsForHeadingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetListItemsForHeadingRequest)

 [List\_SetListItemsForUserGroupFromListNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetListItemsForUserGroupFromListNameRequest)

 [List\_SetListItemsForUserGroupRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetListItemsForUserGroupRequest)

 [List\_SetRankOnHeadingsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetRankOnHeadingsRequest)

 [List\_SetRankOnProjectActivityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetRankOnProjectActivityRequest)

 [List\_SetRankOnProjectDocumentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetRankOnProjectDocumentRequest)

 [List\_SetRankOnSaleActivityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetRankOnSaleActivityRequest)

 [List\_SetRankOnSaleDocumentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetRankOnSaleDocumentRequest)

 [List\_SetTicketCategoriesForUserGroupRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetTicketCategoriesForUserGroupRequest)

 [List\_SetTicketPrioritySortOrderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetTicketPrioritySortOrderRequest)

 [List\_SetTicketRelationDefinitionSortOrderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetTicketRelationDefinitionSortOrderRequest)

 [List\_SetTicketStatusSortOrderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetTicketStatusSortOrderRequest)

 [List\_SetTicketTypeSortOrderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetTicketTypeSortOrderRequest)

 [List\_SetVisibleForUserGroupRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetVisibleForUserGroupRequest)

 [List\_SetVisibleForUserGroupsFromListNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetVisibleForUserGroupsFromListNameRequest)

 [List\_SetVisibleForUserGroupsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SetVisibleForUserGroupsRequest)

 [List\_SortListItemsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_SortListItemsRequest)

 [List\_UpdateCategoryMembershipsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_UpdateCategoryMembershipsRequest)

 [List\_UpdateHierarchyFromPathRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_UpdateHierarchyFromPathRequest)

 [List\_ValidateTicketRelationDefinitionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.List_ValidateTicketRelationDefinitionRequest)

 [LiveUiConfig](/en/api/reference/webapi/SuperOffice.WebApi.Data.LiveUiConfig)

Carrier object for LiveUiConfig.
A config containing parameters for live UI

 [LiveUiSyncResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.LiveUiSyncResult)

Carrier object for LiveUiSyncResult.
The combined answer of one live UI sync call: new UI events, plus the sync state of the tickets the client asked about.

 [LocalizedField](/en/api/reference/webapi/SuperOffice.WebApi.Data.LocalizedField)

Carrier object for LocalizedField.
LocalizedField is used to store localized information such as formatted address data, a formatted phone number, etc.

 [LocalizedText](/en/api/reference/webapi/SuperOffice.WebApi.Data.LocalizedText)

Carrier object for LocalizedText.

 [Logging](/en/api/reference/webapi/SuperOffice.WebApi.Data.Logging)

Carrier object for Logging.
Change NetServer logging settings

 [UserPreferenceStrings.MDOList](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.MDOList)

Grouped and filtered lists

 [MDOListItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.MDOListItem)

Carrier object for MDOListItem.

 [MDO\_GetListIdByListNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.MDO_GetListIdByListNameRequest)

 [MDO\_GetListItemRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.MDO_GetListItemRequest)

 [MDO\_GetListNamesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.MDO_GetListNamesRequest)

 [MDO\_GetListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.MDO_GetListRequest)

 [MDO\_GetListWithHistoryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.MDO_GetListWithHistoryRequest)

 [MDO\_GetListWithRestrictionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.MDO_GetListWithRestrictionRequest)

 [MDO\_GetSelectableListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.MDO_GetSelectableListRequest)

 [MDO\_GetSelectableListWithHistoryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.MDO_GetSelectableListWithHistoryRequest)

 [MDO\_GetSelectableListWithRestrictionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.MDO_GetSelectableListWithRestrictionRequest)

 [MDO\_GetSelectableSimpleListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.MDO_GetSelectableSimpleListRequest)

 [MDO\_GetSimpleListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.MDO_GetSimpleListRequest)

 [MDO\_SetSelectedRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.MDO_SetSelectedRequest)

 [UserPreferenceStrings.Mail](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Mail)

E-mail synchronisation

 [Mailbox](/en/api/reference/webapi/SuperOffice.WebApi.Data.Mailbox)

Carrier object for Mailbox.
Carrier containing information about a Service mailbox

 [MailboxEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.MailboxEntity)

Carrier object for MailboxEntity.
This entity represent a mailbox used for importing emails in Service

 [UserPreferenceStrings.Mailing](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Mailing)

 [MailingEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.MailingEntity)

Carrier object for MailingEntity.
A mailing (shipment) — the run that sends a message to a set of recipients

 [MailingStatistics](/en/api/reference/webapi/SuperOffice.WebApi.Data.MailingStatistics)

Carrier object for MailingStatistics.
Methods for calculating mailing statistics

 [Marketing\_CalculateMailingStatisticsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Marketing_CalculateMailingStatisticsRequest)

 [Marketing\_CreateDefaultAvailableFontRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Marketing_CreateDefaultAvailableFontRequest)

 [Marketing\_DeleteAvailableFontRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Marketing_DeleteAvailableFontRequest)

 [Marketing\_DeleteMessageAndShipmentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Marketing_DeleteMessageAndShipmentRequest)

 [Marketing\_GetAvailableFontRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Marketing_GetAvailableFontRequest)

 [Marketing\_GetFormSubmissionsCountRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Marketing_GetFormSubmissionsCountRequest)

 [Marketing\_GetPreviewMailingHeaderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Marketing_GetPreviewMailingHeaderRequest)

 [Marketing\_GetPreviewMailingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Marketing_GetPreviewMailingRequest)

 [Marketing\_GetUserBlocksRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Marketing_GetUserBlocksRequest)

 [Marketing\_MoveFormsToFolderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Marketing_MoveFormsToFolderRequest)

 [Marketing\_MoveMailingsToFolderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Marketing_MoveMailingsToFolderRequest)

 [Marketing\_ReloadAndSendMailingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Marketing_ReloadAndSendMailingRequest)

 [Marketing\_RenameMailingByMessageIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Marketing_RenameMailingByMessageIdRequest)

 [Marketing\_SaveAvailableFontRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Marketing_SaveAvailableFontRequest)

 [Marketing\_SetAllFormFieldRestrictionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Marketing_SetAllFormFieldRestrictionsRequest)

 [Marketing\_SetFontRankRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Marketing_SetFontRankRequest)

 [Marketing\_StopMailingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Marketing_StopMailingRequest)

 [Marketing\_VerifyGoogleRecaptchaRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Marketing_VerifyGoogleRecaptchaRequest)

 [MassOperationResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.MassOperationResult)

Carrier object for MassOperationResult.
Information about the results of a mass-update operation

 [MessageDeliveryStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.MessageDeliveryStatus)

Carrier object for MessageDeliveryStatus.
Class used to describe the message status.

 [Messaging\_CreateMessageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Messaging_CreateMessageRequest)

 [Messaging\_GetDeliveryStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Messaging_GetDeliveryStatusRequest)

 [Messaging\_GetPluginsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Messaging_GetPluginsRequest)

 [Messaging\_SendMessagesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Messaging_SendMessagesRequest)

 [Messaging\_SendMessagesWithConfigRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Messaging_SendMessagesWithConfigRequest)

 [Messaging\_SetDeliveryStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Messaging_SetDeliveryStatusRequest)

 [MetricPair](/en/api/reference/webapi/SuperOffice.WebApi.Data.MetricPair)

Carrier object for MetricPair.
Metric, in the form of string-&gt;int

 [UserPreferenceStrings.MiniPanel](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.MiniPanel)

Preference keys for the mini panel.

 [ModuleLicense](/en/api/reference/webapi/SuperOffice.WebApi.Data.ModuleLicense)

Carrier object for ModuleLicense.
Represents a single module license info.

 [ModuleLicenseLink](/en/api/reference/webapi/SuperOffice.WebApi.Data.ModuleLicenseLink)

Carrier object for ModuleLicenseLink.
License assignments carrier

 [MrMrs](/en/api/reference/webapi/SuperOffice.WebApi.Data.MrMrs)

Carrier object for MrMrs.

 [MultiAlarmData](/en/api/reference/webapi/SuperOffice.WebApi.Data.MultiAlarmData)

Carrier object for MultiAlarmData.

 [NaturalLanguageSearch](/en/api/reference/webapi/SuperOffice.WebApi.Data.NaturalLanguageSearch)

Carrier object for NaturalLanguageSearch.
Natural language search results.

 [NavigationElement](/en/api/reference/webapi/SuperOffice.WebApi.Data.NavigationElement)

Carrier object for NavigationElement.
A single navigation entry - a key and its navigation value (soprotocol).

 [NavigatorCompany](/en/api/reference/webapi/SuperOffice.WebApi.Data.NavigatorCompany)

Carrier object for NavigatorCompany.

 [Navigator\_GetNavigatorCompaniesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Navigator_GetNavigatorCompaniesRequest)

 [UserPreferenceStrings.NetServices](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.NetServices)

SuperOffice Web Services settings

 [NewsFeedCTA](/en/api/reference/webapi/SuperOffice.WebApi.Data.NewsFeedCTA)

Carrier object for NewsFeedCTA.
News feed CTAs are created by agents or other services for display on the start page and other relevant places in the UI.

 [NewsFeedItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.NewsFeedItem)

Carrier object for NewsFeedItem.
News feed items are created by agents or other services for display on the start page and other relevant places in the UI.

 [NewsFeed\_GetNewsFeedItemsByIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.NewsFeed_GetNewsFeedItemsByIdRequest)

 [NewsFeed\_GetNewsItemsForUserRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.NewsFeed_GetNewsItemsForUserRequest)

 [NewsFeed\_MarkAsHandledRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.NewsFeed_MarkAsHandledRequest)

 [NewsFeed\_MarkAsReadRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.NewsFeed_MarkAsReadRequest)

 [NewsFeed\_PostNewsFeedItemRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.NewsFeed_PostNewsFeedItemRequest)

 [NewsFeed\_StartHandlingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.NewsFeed_StartHandlingRequest)

 [NextAvailableTime](/en/api/reference/webapi/SuperOffice.WebApi.Data.NextAvailableTime)

Carrier object for NextAvailableTime.
The soonest-available working time slot

 [UserPreferenceStrings.Notification](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Notification)

 [UserPreferenceStrings.NotificationEmail](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.NotificationEmail)

 [NotificationEvent](/en/api/reference/webapi/SuperOffice.WebApi.Data.NotificationEvent)

Carrier object for NotificationEvent.
Carrier for a notification event for a given notification type and entity id

 [UserPreferenceStrings.NotificationSms](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.NotificationSms)

 [NumberAllocation\_GetNumberEachTemplateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.NumberAllocation_GetNumberEachTemplateRequest)

 [NumberAllocation\_SaveDefaultNumberingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.NumberAllocation_SaveDefaultNumberingRequest)

 [NumberAllocation\_SetNumberEachTemplateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.NumberAllocation_SetNumberEachTemplateRequest)

 [UserPreferenceStrings.Numbering](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Numbering)

Numbering

 [OperationInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.OperationInfo)

Carrier object for OperationInfo.
This class carries information about the definition and properties of an field used in bulk update system. The class is serializable and used on both the application and Web server sides.

 [OperationResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.OperationResult)

Carrier object for OperationResult.
Operation result

 [OutgoingMessage](/en/api/reference/webapi/SuperOffice.WebApi.Data.OutgoingMessage)

Carrier object for OutgoingMessage.
Details about a message to be sent

 [ParticipantInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.ParticipantInfo)

Carrier object for ParticipantInfo.
Class representing an appointment participant.

 [ParticipantSyncData](/en/api/reference/webapi/SuperOffice.WebApi.Data.ParticipantSyncData)

Carrier object for ParticipantSyncData.

 [PaymentTerm](/en/api/reference/webapi/SuperOffice.WebApi.Data.PaymentTerm)

Carrier object for PaymentTerm.
The built-in payment terms list. Used by the SuperOffice Quote Connector to categorize quote versions.

 [PaymentType](/en/api/reference/webapi/SuperOffice.WebApi.Data.PaymentType)

Carrier object for PaymentType.
The built-in payment types list. Used by the SuperOffice Quote Connector in quote versions.

 [Person](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person)

Carrier object for Person.

 [UserPreferenceStrings.Person](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Person)

Person preferences

 [PersonEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.PersonEntity)

Carrier object for PersonEntity.
The Person Service. The service implements all services working with the Person object.

 [PersonImage](/en/api/reference/webapi/SuperOffice.WebApi.Data.PersonImage)

Carrier object for PersonImage.
Scaled person image

 [PersonSummary](/en/api/reference/webapi/SuperOffice.WebApi.Data.PersonSummary)

Carrier object for PersonSummary.
Summary of person, with recent activities, chats, and requests included.

 [Person\_AddBounceRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_AddBounceRequest)

 [Person\_AddBounceWithCountRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_AddBounceWithCountRequest)

 [Person\_AddBounceWithDetailsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_AddBounceWithDetailsRequest)

 [Person\_AddEmailAddressToPersonRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_AddEmailAddressToPersonRequest)

 [Person\_ChangeCountryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_ChangeCountryRequest)

 [Person\_ChangePersonRankRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_ChangePersonRankRequest)

 [Person\_CheckTemporaryKeyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_CheckTemporaryKeyRequest)

 [Person\_CreateDefaultByContactIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_CreateDefaultByContactIdRequest)

 [Person\_CreateDefaultFromCredentialRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_CreateDefaultFromCredentialRequest)

 [Person\_CreateOrUpdateUserCandidateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_CreateOrUpdateUserCandidateRequest)

 [Person\_CreateTemporaryKeyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_CreateTemporaryKeyRequest)

 [Person\_CreateTemporaryKeyWithPayloadRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_CreateTemporaryKeyWithPayloadRequest)

 [Person\_DeleteExpiredRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_DeleteExpiredRequest)

 [Person\_DeleteTemporaryKeysOfDomainRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_DeleteTemporaryKeysOfDomainRequest)

 [Person\_GetAddressByCountryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetAddressByCountryRequest)

 [Person\_GetAddressRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetAddressRequest)

 [Person\_GetBestSoLanguageForTemplateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetBestSoLanguageForTemplateRequest)

 [Person\_GetColleaguesByDepartmentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetColleaguesByDepartmentRequest)

 [Person\_GetColleaguesBySourceRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetColleaguesBySourceRequest)

 [Person\_GetColleaguesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetColleaguesRequest)

 [Person\_GetConsentInfosRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetConsentInfosRequest)

 [Person\_GetCustomerCentrePasswordReplyTemplateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetCustomerCentrePasswordReplyTemplateRequest)

 [Person\_GetMyOwnerRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetMyOwnerRequest)

 [Person\_GetMyPersonRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetMyPersonRequest)

 [Person\_GetOwnerOnPersonIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetOwnerOnPersonIdRequest)

 [Person\_GetPersonImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetPersonImageRequest)

 [Person\_GetPersonImagesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetPersonImagesRequest)

 [Person\_GetPersonSummaryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetPersonSummaryRequest)

 [Person\_GetPersonsFromContactRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetPersonsFromContactRequest)

 [Person\_GetPersonsFromProjectRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetPersonsFromProjectRequest)

 [Person\_GetPhonesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetPhonesRequest)

 [Person\_GetSalesRepRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetSalesRepRequest)

 [Person\_GetUserCandidateByPersonRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetUserCandidateByPersonRequest)

 [Person\_GetUtmParametersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_GetUtmParametersRequest)

 [Person\_HasConsentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_HasConsentRequest)

 [Person\_IsNumberValidRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_IsNumberValidRequest)

 [Person\_MergeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_MergeRequest)

 [Person\_MoveRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_MoveRequest)

 [Person\_NormalizeRanksRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_NormalizeRanksRequest)

 [Person\_RemoveConsentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_RemoveConsentRequest)

 [Person\_ResolvePersonFromInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_ResolvePersonFromInfoRequest)

 [Person\_SendConsentConfirmationEmailRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_SendConsentConfirmationEmailRequest)

 [Person\_SendCustomerCentrePasswordEmailRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_SendCustomerCentrePasswordEmailRequest)

 [Person\_SendEmailRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_SendEmailRequest)

 [Person\_SendEmailWithEventNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_SendEmailWithEventNameRequest)

 [Person\_SetConsentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_SetConsentRequest)

 [Person\_SetPersonImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_SetPersonImageRequest)

 [Person\_SetPersonRankRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_SetPersonRankRequest)

 [Person\_SetUtmCreatedPersonContactForFormSubmissionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_SetUtmCreatedPersonContactForFormSubmissionRequest)

 [Person\_TryAddPersonsToEmailFlowRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_TryAddPersonsToEmailFlowRequest)

 [Person\_UndeleteRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_UndeleteRequest)

 [Person\_UpdateInterestsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_UpdateInterestsRequest)

 [Person\_ValidatePersonEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Person_ValidatePersonEntityRequest)

 [PhoneListItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.PhoneListItem)

Carrier object for PhoneListItem.

 [PhoneListPreferences](/en/api/reference/webapi/SuperOffice.WebApi.Data.PhoneListPreferences)

Carrier object for PhoneListPreferences.
Search preferences for a phone list search

 [PhoneList\_AddToFavoritesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.PhoneList_AddToFavoritesRequest)

 [PhoneList\_GetContactPhonesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.PhoneList_GetContactPhonesRequest)

 [PhoneList\_GetDepartmentPhonesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.PhoneList_GetDepartmentPhonesRequest)

 [PhoneList\_GetFavoritesPhonesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.PhoneList_GetFavoritesPhonesRequest)

 [PhoneList\_GetPreferencesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.PhoneList_GetPreferencesRequest)

 [PhoneList\_SearchRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.PhoneList_SearchRequest)

 [PhoneList\_SearchWithPreferencesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.PhoneList_SearchWithPreferencesRequest)

 [PhoneList\_SetPreferencesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.PhoneList_SetPreferencesRequest)

 [PictureFolderEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.PictureFolderEntity)

Carrier object for PictureFolderEntity.
Picture/mailing folder (s_picture_folder)

 [PluginResponse](/en/api/reference/webapi/SuperOffice.WebApi.Data.PluginResponse)

Carrier object for PluginResponse.
PluginResponse exists to be able to respond with more than just a true/false, but also an explanation. Such an explanation can be displayed on for instance a disabled “Place Order” button.

 [PluginUrlResponse](/en/api/reference/webapi/SuperOffice.WebApi.Data.PluginUrlResponse)

Carrier object for PluginUrlResponse.
PluginUrlResponse exists to be able to respond with more than just a true/false, but also an explanation. Such an explanation can be displayed on for instance a disabled “Place Order” button. In addition, an URL can be returned, which the GUI should navigate to/display if it is non-blank. The URL can be an SoProtocol or http(s)

 [PocketDeviceInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.PocketDeviceInfo)

Carrier object for PocketDeviceInfo.
Describes a device running Pocket CRM

 [PocketNotificationMessage](/en/api/reference/webapi/SuperOffice.WebApi.Data.PocketNotificationMessage)

Carrier object for PocketNotificationMessage.
Describes behaviour and content for a push notification message

 [UserPreferenceStrings.PocketSection](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.PocketSection)

Preferences for Pocket client.

 [PocketStartupData](/en/api/reference/webapi/SuperOffice.WebApi.Data.PocketStartupData)

Carrier object for PocketStartupData.
The Pocket Service. The service implements pocket specific stuff, like consolidated post-login stuff.

 [Pocket\_GetCallerIDsFromSelectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Pocket_GetCallerIDsFromSelectionRequest)

 [Pocket\_GetMyCallerIDsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Pocket_GetMyCallerIDsRequest)

 [Pocket\_GetPocketStartupDataRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Pocket_GetPocketStartupDataRequest)

 [Pocket\_GetPushNotificationTagsForDeviceRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Pocket_GetPushNotificationTagsForDeviceRequest)

 [Pocket\_GetRegisteredDevicesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Pocket_GetRegisteredDevicesRequest)

 [Pocket\_NotificationHandledRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Pocket_NotificationHandledRequest)

 [Pocket\_NotificationsHandledRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Pocket_NotificationsHandledRequest)

 [Pocket\_NotifyUsersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Pocket_NotifyUsersRequest)

 [Pocket\_RegisterDeviceForPushNotificationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Pocket_RegisterDeviceForPushNotificationRequest)

 [Pocket\_RunAppointmentAlarmBrokerRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Pocket_RunAppointmentAlarmBrokerRequest)

 [Pocket\_SendPushNotificationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Pocket_SendPushNotificationRequest)

 [Pocket\_SetPushNotificationTagsForDeviceRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Pocket_SetPushNotificationTagsForDeviceRequest)

 [Pocket\_SetPushNotificationTagsForUserRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Pocket_SetPushNotificationTagsForUserRequest)

 [Position](/en/api/reference/webapi/SuperOffice.WebApi.Data.Position)

Carrier object for Position.

 [Preference](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference)

Carrier object for Preference.
One preference item, which contains a specification (section/key), the raw string value, and the level at which this value was found.

 [PreferenceDescription](/en/api/reference/webapi/SuperOffice.WebApi.Data.PreferenceDescription)

Carrier object for PreferenceDescription.
Preference description, drives the Preference GUI. Maintenance client - controls all preferences in SuperOffice.

 [PreferenceDescriptionLine](/en/api/reference/webapi/SuperOffice.WebApi.Data.PreferenceDescriptionLine)

Carrier object for PreferenceDescriptionLine.
Preference description line, contains information about a single item in a preference description list.

 [PreferenceSpec](/en/api/reference/webapi/SuperOffice.WebApi.Data.PreferenceSpec)

Carrier object for PreferenceSpec.
Preference specification, consisting of the section name and key name

 [Preference\_DeleteFromSectionAndKeyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_DeleteFromSectionAndKeyRequest)

 [Preference\_DeletePrefDescLinesByPrefDescIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_DeletePrefDescLinesByPrefDescIdRequest)

 [Preference\_DeletePrefDescLinesByPrefDescIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_DeletePrefDescLinesByPrefDescIdsRequest)

 [Preference\_DeletePreferenceDescriptionLinesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_DeletePreferenceDescriptionLinesRequest)

 [Preference\_DeletePreferenceRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_DeletePreferenceRequest)

 [Preference\_DeletePreferencesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_DeletePreferencesRequest)

 [Preference\_GetAllFromSectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_GetAllFromSectionRequest)

 [Preference\_GetAllRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_GetAllRequest)

 [Preference\_GetFromSectionAndKeyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_GetFromSectionAndKeyRequest)

 [Preference\_GetNetServicesStatusUrlRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_GetNetServicesStatusUrlRequest)

 [Preference\_GetPreferenceByNameAndIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_GetPreferenceByNameAndIdRequest)

 [Preference\_GetPreferenceByNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_GetPreferenceByNameRequest)

 [Preference\_GetPreferenceDescriptionLineFromIdAndValueRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_GetPreferenceDescriptionLineFromIdAndValueRequest)

 [Preference\_GetPreferenceDescriptionLinesByPrefDescIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_GetPreferenceDescriptionLinesByPrefDescIdRequest)

 [Preference\_GetPreferenceRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_GetPreferenceRequest)

 [Preference\_GetPreferencesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_GetPreferencesRequest)

 [Preference\_GetPreferencesWithDisplayValuesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_GetPreferencesWithDisplayValuesRequest)

 [Preference\_GetTabOrderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_GetTabOrderRequest)

 [Preference\_GetTabOrdersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_GetTabOrdersRequest)

 [Preference\_GetXsrfPathsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_GetXsrfPathsRequest)

 [Preference\_GetXsrfValueRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_GetXsrfValueRequest)

 [Preference\_SaveFromSectionAndKeyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_SaveFromSectionAndKeyRequest)

 [Preference\_SavePreferenceDescriptionLinesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_SavePreferenceDescriptionLinesRequest)

 [Preference\_SavePreferenceEntitiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_SavePreferenceEntitiesRequest)

 [Preference\_SavePreferenceEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_SavePreferenceEntityRequest)

 [Preference\_SavePreferenceRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_SavePreferenceRequest)

 [Preference\_SavePreferencesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_SavePreferencesRequest)

 [Preference\_SaveTabOrderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_SaveTabOrderRequest)

 [Preference\_SaveTabOrdersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_SaveTabOrdersRequest)

 [Preference\_SetXsrfPathsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_SetXsrfPathsRequest)

 [Preference\_SetXsrfValueRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_SetXsrfValueRequest)

 [Preference\_UpdateNetServicesStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Preference_UpdateNetServicesStatusRequest)

 [PreferredTimeZone](/en/api/reference/webapi/SuperOffice.WebApi.Data.PreferredTimeZone)

Carrier object for PreferredTimeZone.
This service support fetching of client interface configuration data. &lt;para/&gt; Client configuration data include Application configuration, which defines how an application is composed out of pages; and page configuration that defines how a page is built out of panels, cards, views and controls. &lt;para/&gt; Separate access points also exist for fetching object mappings and filter lists.

 [PreviewContact](/en/api/reference/webapi/SuperOffice.WebApi.Data.PreviewContact)

Carrier object for PreviewContact.
Contact preview

 [PreviewDash](/en/api/reference/webapi/SuperOffice.WebApi.Data.PreviewDash)

Carrier object for PreviewDash.
Dashboard preview

 [PreviewDashTile](/en/api/reference/webapi/SuperOffice.WebApi.Data.PreviewDashTile)

Carrier object for PreviewDashTile.
Dashboard tile preview

 [PreviewFaqEntry](/en/api/reference/webapi/SuperOffice.WebApi.Data.PreviewFaqEntry)

Carrier object for PreviewFaqEntry.
A faq entry with information suitable for preview.

 [PreviewMailing](/en/api/reference/webapi/SuperOffice.WebApi.Data.PreviewMailing)

Carrier object for PreviewMailing.
A shipment message with information about the recipient it was sent to.

 [PreviewMailingHeader](/en/api/reference/webapi/SuperOffice.WebApi.Data.PreviewMailingHeader)

Carrier object for PreviewMailingHeader.
A shipment with information intended for previewing it.

 [PreviewQuickReply](/en/api/reference/webapi/SuperOffice.WebApi.Data.PreviewQuickReply)

Carrier object for PreviewQuickReply.
A quick reply with information suitable for preview.

 [PreviewReplyTemplate](/en/api/reference/webapi/SuperOffice.WebApi.Data.PreviewReplyTemplate)

Carrier object for PreviewReplyTemplate.
A reply template with information suitable for preview.

 [Previews](/en/api/reference/webapi/SuperOffice.WebApi.Data.Previews)

Carrier object for Previews.
This service provides Previews. It is based on 'Preview hints', strings that may eiehter be a Preview text (if provided by someone else), or a set of key/value pairs or other specification.&lt;para /&gt;The Preview implementation is plugin based, and Preview hints are passed to all registered plugin providers according to certain rules. It is therefore up to providers to parse and process Preview hints as they see fit.

 [Previews\_GetPreviewRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Previews_GetPreviewRequest)

 [PriceList](/en/api/reference/webapi/SuperOffice.WebApi.Data.PriceList)

Carrier object for PriceList.
A pricelist is basically a collection of products. It can be valid in a time period, and outright deactivated. All prices in the product list is in a specific currency. We have decieded not to separate prices and products, which means that we get a simpler data model, but some redundancy.

 [Priority](/en/api/reference/webapi/SuperOffice.WebApi.Data.Priority)

Carrier object for Priority.

 [Product](/en/api/reference/webapi/SuperOffice.WebApi.Data.Product)

Carrier object for Product.
Information about a Product from the ERP system. This may be fetched from SuperOffice Product table through the SO Connector

 [ProductCategory](/en/api/reference/webapi/SuperOffice.WebApi.Data.ProductCategory)

Carrier object for ProductCategory.
The built-in product category list. Used by the SuperOffice Quote Connector to categorize products.

 [ProductExtraDataField](/en/api/reference/webapi/SuperOffice.WebApi.Data.ProductExtraDataField)

Carrier object for ProductExtraDataField.
A way to show some simple extra data on a product, typically to hep the user to identify the correct product. Basically a bucket of additional info that the ERP system would like to store and show in the user interface. Information placed here is shown in the GUI if the “provide-extra-data” capability is true.

 [ProductFamily](/en/api/reference/webapi/SuperOffice.WebApi.Data.ProductFamily)

Carrier object for ProductFamily.
The built-in product family list. Used by the SuperOffice Quote Connector to categorize products.

 [ProductType](/en/api/reference/webapi/SuperOffice.WebApi.Data.ProductType)

Carrier object for ProductType.
The built-in product type list. Used by the SuperOffice Quote Connector to categorize products.

 [Project](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project)

Carrier object for Project.

 [UserPreferenceStrings.Project](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Project)

Project preferences

 [ProjectBoardViewSettings](/en/api/reference/webapi/SuperOffice.WebApi.Data.ProjectBoardViewSettings)

Carrier object for ProjectBoardViewSettings.
Settings for project selection board view.

 [ProjectEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.ProjectEntity)

Carrier object for ProjectEntity.
The Project Service. The service implements all services working with the Project object

 [ProjectEvent](/en/api/reference/webapi/SuperOffice.WebApi.Data.ProjectEvent)

Carrier object for ProjectEvent.
A project that is also an event. Used by Audience

 [ProjectEventEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.ProjectEventEntity)

Carrier object for ProjectEventEntity.
The ProjectEvent Service. The service implements all services working with the ProjectEvent object

 [ProjectMember](/en/api/reference/webapi/SuperOffice.WebApi.Data.ProjectMember)

Carrier object for ProjectMember.
A person can be a member of multiple projects, with a different comment and member type in each project.

 [ProjectStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.ProjectStatus)

Carrier object for ProjectStatus.

 [ProjectType](/en/api/reference/webapi/SuperOffice.WebApi.Data.ProjectType)

Carrier object for ProjectType.

 [ProjectTypeEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.ProjectTypeEntity)

Carrier object for ProjectTypeEntity.

 [ProjectTypeStatusLink](/en/api/reference/webapi/SuperOffice.WebApi.Data.ProjectTypeStatusLink)

Carrier object for ProjectTypeStatusLink.

 [Project\_AddProjectMembersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_AddProjectMembersRequest)

 [Project\_CreateNewEntryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_CreateNewEntryRequest)

 [Project\_DeleteProjectEventEntityFromProjectIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_DeleteProjectEventEntityFromProjectIdRequest)

 [Project\_DeleteProjectMemberByIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_DeleteProjectMemberByIdsRequest)

 [Project\_DeleteProjectMembersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_DeleteProjectMembersRequest)

 [Project\_GetDuplicateRulesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetDuplicateRulesRequest)

 [Project\_GetDuplicatesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetDuplicatesRequest)

 [Project\_GetMyMemberProjectsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetMyMemberProjectsRequest)

 [Project\_GetMyProjectEventsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetMyProjectEventsRequest)

 [Project\_GetMyProjectsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetMyProjectsRequest)

 [Project\_GetMyPublishedProjectsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetMyPublishedProjectsRequest)

 [Project\_GetNextMilestoneRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetNextMilestoneRequest)

 [Project\_GetNextProjectStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetNextProjectStatusRequest)

 [Project\_GetProjectEntityWithoutMembersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetProjectEntityWithoutMembersRequest)

 [Project\_GetProjectEventEntityFromProjectIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetProjectEventEntityFromProjectIdRequest)

 [Project\_GetProjectEventOnPersonRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetProjectEventOnPersonRequest)

 [Project\_GetProjectEventsOnPersonRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetProjectEventsOnPersonRequest)

 [Project\_GetProjectFencingInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetProjectFencingInfoRequest)

 [Project\_GetProjectImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetProjectImageRequest)

 [Project\_GetProjectMembersByIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetProjectMembersByIdRequest)

 [Project\_GetProjectMembersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetProjectMembersRequest)

 [Project\_GetProjectsFromContactRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetProjectsFromContactRequest)

 [Project\_GetProjectsFromPersonRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetProjectsFromPersonRequest)

 [Project\_GetPublishedProjectRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetPublishedProjectRequest)

 [Project\_GetPublishedProjectsOnPersonIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetPublishedProjectsOnPersonIdRequest)

 [Project\_GetPublishedProjectsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_GetPublishedProjectsRequest)

 [Project\_HasGuideActivitiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_HasGuideActivitiesRequest)

 [Project\_HasGuideRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_HasGuideRequest)

 [Project\_IsNumberValidRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_IsNumberValidRequest)

 [Project\_MergeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_MergeRequest)

 [Project\_OfferAutoNextStatusOnApppointmentCompletedRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_OfferAutoNextStatusOnApppointmentCompletedRequest)

 [Project\_SaveProjectEntityWithoutMembersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_SaveProjectEntityWithoutMembersRequest)

 [Project\_SaveProjectMembersFunctionAndCommentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_SaveProjectMembersFunctionAndCommentRequest)

 [Project\_SetCompletedRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_SetCompletedRequest)

 [Project\_SetDuplicateRulesStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_SetDuplicateRulesStatusRequest)

 [Project\_SetProjectImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_SetProjectImageRequest)

 [Project\_UpdateProjectMemberRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_UpdateProjectMemberRequest)

 [Project\_UpdateProjectMembersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_UpdateProjectMembersRequest)

 [Project\_ValidateProjectEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Project_ValidateProjectEntityRequest)

 [ProvidedCredential](/en/api/reference/webapi/SuperOffice.WebApi.Data.ProvidedCredential)

Carrier object for ProvidedCredential.
Representation of how the current user got authenticated

 [ProviderInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.ProviderInfo)

Carrier object for ProviderInfo.
Information about a provider plugin

 [QuickFilterInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuickFilterInfo)

Carrier object for QuickFilterInfo.
Info associated with quick filter

 [UserPreferenceStrings.QuickLaunchBar](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.QuickLaunchBar)

Preference keys for QuickLaunchBar settings

 [QuickReply](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuickReply)

Carrier object for QuickReply.

 [Quote](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote)

Carrier object for Quote.
Quote carrier with the active quote version and favorite alternative.

 [UserPreferenceStrings.Quote](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Quote)

Preference keys for the quote system.

 [QuoteAlternative](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuoteAlternative)

Carrier object for QuoteAlternative.
Information about a connection to the ERP system.

 [QuoteAlternativeRecalculated](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuoteAlternativeRecalculated)

Carrier object for QuoteAlternativeRecalculated.
The QuoteAlternativeRecalculated is returned after a call to RecalculateQuoteAlternative. It contains flags indicating changes to quote alternative and quote lines.

 [QuoteApproveReason](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuoteApproveReason)

Carrier object for QuoteApproveReason.
List of quote approval reasons

 [QuoteConnection](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuoteConnection)

Carrier object for QuoteConnection.
Information about a connection to the ERP system.

 [QuoteContext](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuoteContext)

Carrier object for QuoteContext.
The context a quote is created in.

 [QuoteDenyReason](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuoteDenyReason)

Carrier object for QuoteDenyReason.
List of quote denial reasons

 [QuoteEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuoteEntity)

Carrier object for QuoteEntity.
Quote methods

 [QuoteLine](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuoteLine)

Carrier object for QuoteLine.
Information about a connection to the ERP system.

 [QuoteLineConfiguration](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuoteLineConfiguration)

Carrier object for QuoteLineConfiguration.
QuoteLineConfiguration contains information about which configuration fields that is visible in the GUI. Read only and mandatory fields are also specified.

 [QuotePublishDocuments](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuotePublishDocuments)

Carrier object for QuotePublishDocuments.
Prepare the documents required to publish a quote - Email, main document as pdf, quoted products as pdf, attachments

 [QuoteVersion](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuoteVersion)

Carrier object for QuoteVersion.
Information about a version of the .

 [QuoteVersionAttachment](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuoteVersionAttachment)

Carrier object for QuoteVersionAttachment.
Attachment documents for a given version of a Quote

 [QuoteVersionButtonState](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuoteVersionButtonState)

Carrier object for QuoteVersionButtonState.
Information about possible button actions for the quote version dialog.

 [QuoteVersionStatusInformation](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuoteVersionStatusInformation)

Carrier object for QuoteVersionStatusInformation.
Information about possible button actions for the quote version dialog.

 [QuoteVersionValidated](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuoteVersionValidated)

Carrier object for QuoteVersionValidated.
The QuoteVersionValidated is returned after a call to ValidateQuoteAlternative. It contains flags indicating changes to quote, quoteversion/quotealternative and quotelines.

 [QuoteVersionWorkflowState](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuoteVersionWorkflowState)

Carrier object for QuoteVersionWorkflowState.
Information about possible button actions for the quote version dialog.

 [Quote\_AddSaleTypeQuoteAttachmentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_AddSaleTypeQuoteAttachmentRequest)

 [Quote\_ApproveQuoteVersionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_ApproveQuoteVersionRequest)

 [Quote\_CanConnectorProvideCapabilityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_CanConnectorProvideCapabilityRequest)

 [Quote\_CancelApprovalRequestRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_CancelApprovalRequestRequest)

 [Quote\_CopyQuoteAlternativeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_CopyQuoteAlternativeRequest)

 [Quote\_CopySuperOfficePriceListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_CopySuperOfficePriceListRequest)

 [Quote\_CreateAndSaveQuoteFromSaleRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_CreateAndSaveQuoteFromSaleRequest)

 [Quote\_CreateAndSaveQuoteLinesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_CreateAndSaveQuoteLinesRequest)

 [Quote\_CreateAndSaveQuoteRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_CreateAndSaveQuoteRequest)

 [Quote\_CreateAndSaveQuoteVersionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_CreateAndSaveQuoteVersionRequest)

 [Quote\_CreateConnectionFromConnectorNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_CreateConnectionFromConnectorNameRequest)

 [Quote\_CreateOrUpdateQuoteVersionAttachmentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_CreateOrUpdateQuoteVersionAttachmentsRequest)

 [Quote\_CreateQuoteAlternativeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_CreateQuoteAlternativeRequest)

 [Quote\_CreateQuoteLineFromProductRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_CreateQuoteLineFromProductRequest)

 [Quote\_CreateQuoteLineRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_CreateQuoteLineRequest)

 [Quote\_DeleteConnectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_DeleteConnectionRequest)

 [Quote\_DeleteQuoteAlternativeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_DeleteQuoteAlternativeRequest)

 [Quote\_DeleteQuoteLineRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_DeleteQuoteLineRequest)

 [Quote\_DeleteQuoteLinesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_DeleteQuoteLinesRequest)

 [Quote\_DeleteQuoteRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_DeleteQuoteRequest)

 [Quote\_DeleteSaleTypeQuoteAttachmentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_DeleteSaleTypeQuoteAttachmentRequest)

 [Quote\_FindProductRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_FindProductRequest)

 [Quote\_GenerateQuoteDocumentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GenerateQuoteDocumentsRequest)

 [Quote\_GetActivePriceListsByCurrencyIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetActivePriceListsByCurrencyIdRequest)

 [Quote\_GetActivePriceListsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetActivePriceListsRequest)

 [Quote\_GetAllAvailableQuoteConnectionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetAllAvailableQuoteConnectionsRequest)

 [Quote\_GetAllAvailableQuoteConnectionsWithPriceListsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetAllAvailableQuoteConnectionsWithPriceListsRequest)

 [Quote\_GetAllInstalledQuoteConnectionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetAllInstalledQuoteConnectionsRequest)

 [Quote\_GetAllPriceListsByCurrencyIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetAllPriceListsByCurrencyIdRequest)

 [Quote\_GetAllPriceListsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetAllPriceListsRequest)

 [Quote\_GetAllQuoteLineConfigurationsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetAllQuoteLineConfigurationsRequest)

 [Quote\_GetConfigurationFieldsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetConfigurationFieldsRequest)

 [Quote\_GetConnectionConfigFieldsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetConnectionConfigFieldsRequest)

 [Quote\_GetConnectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetConnectionRequest)

 [Quote\_GetConnectionStartupErrorsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetConnectionStartupErrorsRequest)

 [Quote\_GetConnectionStartupResponseRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetConnectionStartupResponseRequest)

 [Quote\_GetConnectionsForAssociateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetConnectionsForAssociateRequest)

 [Quote\_GetConnectorCapabilitiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetConnectorCapabilitiesRequest)

 [Quote\_GetConnectorCapabilityNamesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetConnectorCapabilityNamesRequest)

 [Quote\_GetExtraInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetExtraInfoRequest)

 [Quote\_GetInUseQuoteLineConfigurationsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetInUseQuoteLineConfigurationsRequest)

 [Quote\_GetOrderConfirmationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetOrderConfirmationRequest)

 [Quote\_GetOrderStateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetOrderStateRequest)

 [Quote\_GetProductFromDbIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetProductFromDbIdRequest)

 [Quote\_GetProductImageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetProductImageRequest)

 [Quote\_GetProductRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetProductRequest)

 [Quote\_GetQuoteAlternativesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetQuoteAlternativesRequest)

 [Quote\_GetQuoteEntityFromSaleIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetQuoteEntityFromSaleIdRequest)

 [Quote\_GetQuoteEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetQuoteEntityRequest)

 [Quote\_GetQuoteFromSaleIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetQuoteFromSaleIdRequest)

 [Quote\_GetQuoteLineConfigurationFromFieldNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetQuoteLineConfigurationFromFieldNameRequest)

 [Quote\_GetQuoteLineConfigurationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetQuoteLineConfigurationRequest)

 [Quote\_GetQuoteLineRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetQuoteLineRequest)

 [Quote\_GetQuoteLinesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetQuoteLinesRequest)

 [Quote\_GetQuoteRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetQuoteRequest)

 [Quote\_GetQuoteVersionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetQuoteVersionRequest)

 [Quote\_GetQuoteVersionWorkflowButtonStatesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetQuoteVersionWorkflowButtonStatesRequest)

 [Quote\_GetQuoteVersionWorkflowImageStateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetQuoteVersionWorkflowImageStateRequest)

 [Quote\_GetQuoteVersionWorkflowStateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetQuoteVersionWorkflowStateRequest)

 [Quote\_GetQuoteVersionWorkflowStatusInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetQuoteVersionWorkflowStatusInfoRequest)

 [Quote\_GetQuoteVersionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_GetQuoteVersionsRequest)

 [Quote\_HasConnectionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_HasConnectionsRequest)

 [Quote\_IncludeQuoteVersionAttachmentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_IncludeQuoteVersionAttachmentRequest)

 [Quote\_MoveQuoteLineRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_MoveQuoteLineRequest)

 [Quote\_PlaceOrderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_PlaceOrderRequest)

 [Quote\_RecalculateQuoteAlternativeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_RecalculateQuoteAlternativeRequest)

 [Quote\_RecalculateQuoteLineRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_RecalculateQuoteLineRequest)

 [Quote\_RejectQuoteVersionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_RejectQuoteVersionRequest)

 [Quote\_RemoveProductRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_RemoveProductRequest)

 [Quote\_RequestApprovalRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_RequestApprovalRequest)

 [Quote\_RestoreConnectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_RestoreConnectionRequest)

 [Quote\_SaveConnectionConfigFieldsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_SaveConnectionConfigFieldsRequest)

 [Quote\_SaveConnectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_SaveConnectionRequest)

 [Quote\_SaveQuoteEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_SaveQuoteEntityRequest)

 [Quote\_SaveQuoteLineConfigurationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_SaveQuoteLineConfigurationRequest)

 [Quote\_SaveQuoteLineConfigurationsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_SaveQuoteLineConfigurationsRequest)

 [Quote\_SaveQuoteLineRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_SaveQuoteLineRequest)

 [Quote\_SaveQuoteLinesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_SaveQuoteLinesRequest)

 [Quote\_SaveQuoteRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_SaveQuoteRequest)

 [Quote\_SaveQuoteVersionNumberRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_SaveQuoteVersionNumberRequest)

 [Quote\_SaveQuoteVersionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_SaveQuoteVersionRequest)

 [Quote\_SendQuoteVersionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_SendQuoteVersionRequest)

 [Quote\_TestConnectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_TestConnectionRequest)

 [Quote\_ToggleProductInAssortmentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_ToggleProductInAssortmentRequest)

 [Quote\_ToggleQuoteVersionAttachmentIncludedRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_ToggleQuoteVersionAttachmentIncludedRequest)

 [Quote\_ToggleSaleTypeQuoteAttachmentDefaultIncludedRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_ToggleSaleTypeQuoteAttachmentDefaultIncludedRequest)

 [Quote\_UpdateQuoteVersionPricesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_UpdateQuoteVersionPricesRequest)

 [Quote\_ValidateQuoteVersionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Quote_ValidateQuoteVersionRequest)

 [UserPreferenceStrings.ROU](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.ROU)

Preference keys Regeneration of StatusMonitors

 [RagAnswer](/en/api/reference/webapi/SuperOffice.WebApi.Data.RagAnswer)

Carrier object for RagAnswer.
Contains the answer to an AI search, with sources used.

 [RagContextIds](/en/api/reference/webapi/SuperOffice.WebApi.Data.RagContextIds)

Carrier object for RagContextIds.
Parameters specifying contact/person/project/sale/ticket ids for populating context.

 [RagResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.RagResult)

Carrier object for RagResult.
Contains the result of an AI search, with relevance score, document refernce, relevant text chunk.

 [RagStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.RagStatus)

Carrier object for RagStatus.
RAG namespace status. How is the indexing going?

 [RagStatusChunks](/en/api/reference/webapi/SuperOffice.WebApi.Data.RagStatusChunks)

Carrier object for RagStatusChunks.
RAG namespace document index statistics

 [Rating](/en/api/reference/webapi/SuperOffice.WebApi.Data.Rating)

Carrier object for Rating.
Sale Stage with probability percent.

 [Reason](/en/api/reference/webapi/SuperOffice.WebApi.Data.Reason)

Carrier object for Reason.
Reason for a lost sale

 [ReasonSold](/en/api/reference/webapi/SuperOffice.WebApi.Data.ReasonSold)

Carrier object for ReasonSold.
Reason for a succesfully sold sale

 [ReasonStalled](/en/api/reference/webapi/SuperOffice.WebApi.Data.ReasonStalled)

Carrier object for ReasonStalled.

 [Recipient](/en/api/reference/webapi/SuperOffice.WebApi.Data.Recipient)

Carrier object for Recipient.
Class representing ticket message recipient.

 [RecipientStatistics](/en/api/reference/webapi/SuperOffice.WebApi.Data.RecipientStatistics)

Carrier object for RecipientStatistics.
Class representing a RecipientStatistics carrier used for counting attributes on selection members.

 [RecurrenceDate](/en/api/reference/webapi/SuperOffice.WebApi.Data.RecurrenceDate)

Carrier object for RecurrenceDate.
Class representing a recurrence date.

 [RecurrenceDayPattern](/en/api/reference/webapi/SuperOffice.WebApi.Data.RecurrenceDayPattern)

Carrier object for RecurrenceDayPattern.
Class representing a pattern for daily recurrence

 [RecurrenceInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.RecurrenceInfo)

Carrier object for RecurrenceInfo.
Class representing a recurrence date.

 [RecurrenceMonthPattern](/en/api/reference/webapi/SuperOffice.WebApi.Data.RecurrenceMonthPattern)

Carrier object for RecurrenceMonthPattern.
Class representing a pattern for monthly recurrence.

 [RecurrenceWeekPattern](/en/api/reference/webapi/SuperOffice.WebApi.Data.RecurrenceWeekPattern)

Carrier object for RecurrenceWeekPattern.
Class representing a pattern for weekly recurrence

 [RecurrenceYearPattern](/en/api/reference/webapi/SuperOffice.WebApi.Data.RecurrenceYearPattern)

Carrier object for RecurrenceYearPattern.
Class representing a pattern for yearly recurrence

 [RedLetterDetails](/en/api/reference/webapi/SuperOffice.WebApi.Data.RedLetterDetails)

Carrier object for RedLetterDetails.
Red letter day text details, all the texts related to the given day.

 [RedLetterInformationListItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.RedLetterInformationListItem)

Carrier object for RedLetterInformationListItem.
Detailed information on red-letter days, including both holidays and other days with their individual texts.

 [RedLetterSummary](/en/api/reference/webapi/SuperOffice.WebApi.Data.RedLetterSummary)

Carrier object for RedLetterSummary.
Summary of redletter day information - holiday in own country, and in other countries known to the system.

 [RefCountEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.RefCountEntity)

Carrier object for RefCountEntity.
A refcount entity for a number allocation

 [RelatedData](/en/api/reference/webapi/SuperOffice.WebApi.Data.RelatedData)

Carrier object for RelatedData.
Display data items for one or more related-table fields, specified using a root entity and dot-syntax

 [RelationDefinitionEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.RelationDefinitionEntity)

Carrier object for RelationDefinitionEntity.
A relation definition entity with source and destination tables

 [RelationToOwner](/en/api/reference/webapi/SuperOffice.WebApi.Data.RelationToOwner)

Carrier object for RelationToOwner.

 [Relation\_DeleteContactRelationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Relation_DeleteContactRelationRequest)

 [Relation\_SaveContactRelationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Relation_SaveContactRelationRequest)

 [RemoteTimeZoneMethods](/en/api/reference/webapi/SuperOffice.WebApi.Data.RemoteTimeZoneMethods)

Carrier object for RemoteTimeZoneMethods.
This service support fetching of client interface configuration data. &lt;para/&gt; Client configuration data include Application configuration, which defines how an application is composed out of pages; and page configuration that defines how a page is built out of panels, cards, views and controls. &lt;para/&gt; Separate access points also exist for fetching object mappings and filter lists.

 [ReplyTemplate](/en/api/reference/webapi/SuperOffice.WebApi.Data.ReplyTemplate)

Carrier object for ReplyTemplate.
Reply template item

 [ReplyTemplateParsed](/en/api/reference/webapi/SuperOffice.WebApi.Data.ReplyTemplateParsed)

Carrier object for ReplyTemplateParsed.
Reply template with parsed text. Represent one language version of reply template

 [ReportLabelLayoutEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.ReportLabelLayoutEntity)

Carrier object for ReportLabelLayoutEntity.

 [UserPreferenceStrings.Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Request)

 [UserPreferenceStrings.RequiredDynamicLoad](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.RequiredDynamicLoad)

Required dynamic load:  Add new preference section for NetServer only [RequiredDynamicLoad] name = regexp

 [ResolvedPerson](/en/api/reference/webapi/SuperOffice.WebApi.Data.ResolvedPerson)

Carrier object for ResolvedPerson.
The Person Service. The service implements all services working with the Person object.

 [ResolvedUser](/en/api/reference/webapi/SuperOffice.WebApi.Data.ResolvedUser)

Carrier object for ResolvedUser.
The User Service. The service implements all services working with the User object.

 [ResourceEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.ResourceEntity)

Carrier object for ResourceEntity.
The resource entity contains resource information

 [ResourceOverride](/en/api/reference/webapi/SuperOffice.WebApi.Data.ResourceOverride)

Carrier object for ResourceOverride.
Override specifications for resources/labels

 [Resource\_ActivateResourceSubstitutionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Resource_ActivateResourceSubstitutionRequest)

 [Resource\_DeleteResourceSubstitutionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Resource_DeleteResourceSubstitutionRequest)

 [Resource\_GetResourceSubstitutionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Resource_GetResourceSubstitutionsRequest)

 [Resource\_IsResourceSubstitutionActiveRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Resource_IsResourceSubstitutionActiveRequest)

 [Resource\_SetResourceSubstitutionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Resource_SetResourceSubstitutionRequest)

 [ReturnInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.ReturnInfo)

Carrier object for ReturnInfo.
Return value object for Document Plugin API calls

 [Role](/en/api/reference/webapi/SuperOffice.WebApi.Data.Role)

Carrier object for Role.
Users role for role-based security

 [RoleEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.RoleEntity)

Carrier object for RoleEntity.

 [SaintConfiguration](/en/api/reference/webapi/SuperOffice.WebApi.Data.SaintConfiguration)

Carrier object for SaintConfiguration.
Definition of the three perionds for status monitors and some other settings per saint entity

 [Saint\_GetSaintConfigurationsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Saint_GetSaintConfigurationsRequest)

 [Saint\_GetStatusMonitorPeriodsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Saint_GetStatusMonitorPeriodsRequest)

 [Saint\_GetStatusMonitorRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Saint_GetStatusMonitorRequest)

 [Saint\_GetStatusMonitorsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Saint_GetStatusMonitorsRequest)

 [Saint\_RegenerateCountersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Saint_RegenerateCountersRequest)

 [Saint\_RegenerateStatusMonitorRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Saint_RegenerateStatusMonitorRequest)

 [Saint\_RegenerateStatusMonitorsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Saint_RegenerateStatusMonitorsRequest)

 [Saint\_SetRankOnStatusMonitorsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Saint_SetRankOnStatusMonitorsRequest)

 [Sale](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale)

Carrier object for Sale.

 [UserPreferenceStrings.Sale](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Sale)

Preference keys for sales settings

 [SaleBoardViewSettings](/en/api/reference/webapi/SuperOffice.WebApi.Data.SaleBoardViewSettings)

Carrier object for SaleBoardViewSettings.
Settings for sale selection board view.

 [SaleEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.SaleEntity)

Carrier object for SaleEntity.
The Sale Entity contains the sale amount, currency, and sale members. Sales are linked to contacts, persons, and/or projects.

 [UserPreferenceStrings.SaleFilterDialog](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.SaleFilterDialog)

 [SaleStageEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.SaleStageEntity)

Carrier object for SaleStageEntity.
This entity represents a sale stage with a default percentage rating.

 [SaleStakeholder](/en/api/reference/webapi/SuperOffice.WebApi.Data.SaleStakeholder)

Carrier object for SaleStakeholder.

 [SaleSummary](/en/api/reference/webapi/SuperOffice.WebApi.Data.SaleSummary)

Carrier object for SaleSummary.

 [SaleSummaryItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.SaleSummaryItem)

Carrier object for SaleSummaryItem.
Sale Summary item

 [SaleType](/en/api/reference/webapi/SuperOffice.WebApi.Data.SaleType)

Carrier object for SaleType.

 [SaleTypeEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.SaleTypeEntity)

Carrier object for SaleTypeEntity.

 [SaleTypeStageLink](/en/api/reference/webapi/SuperOffice.WebApi.Data.SaleTypeStageLink)

Carrier object for SaleTypeStageLink.

 [Sale\_AddSaleStakeholdersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_AddSaleStakeholdersRequest)

 [Sale\_BatchUpdateNextDueDateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_BatchUpdateNextDueDateRequest)

 [Sale\_DeleteSaleStakeholdersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_DeleteSaleStakeholdersRequest)

 [Sale\_GetMyOpportunitiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_GetMyOpportunitiesRequest)

 [Sale\_GetNextDueDateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_GetNextDueDateRequest)

 [Sale\_GetNextSaleStageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_GetNextSaleStageRequest)

 [Sale\_GetOpenSalesForContactRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_GetOpenSalesForContactRequest)

 [Sale\_GetProbabilityFromStageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_GetProbabilityFromStageRequest)

 [Sale\_GetRecentSalesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_GetRecentSalesRequest)

 [Sale\_GetSaleStakeholderByIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_GetSaleStakeholderByIdRequest)

 [Sale\_GetSaleStakeholdersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_GetSaleStakeholdersRequest)

 [Sale\_GetSalesByDateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_GetSalesByDateRequest)

 [Sale\_GetSalesFromContactRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_GetSalesFromContactRequest)

 [Sale\_GetSalesFromPersonRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_GetSalesFromPersonRequest)

 [Sale\_GetSummaryByAssociateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_GetSummaryByAssociateRequest)

 [Sale\_GetSummaryByContactRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_GetSummaryByContactRequest)

 [Sale\_GetSummaryByGroupRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_GetSummaryByGroupRequest)

 [Sale\_GetUpcomingSalesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_GetUpcomingSalesRequest)

 [Sale\_HasGuideActivitiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_HasGuideActivitiesRequest)

 [Sale\_HasGuideRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_HasGuideRequest)

 [Sale\_HasStakeholderSettingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_HasStakeholderSettingRequest)

 [Sale\_IsNumberValidRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_IsNumberValidRequest)

 [Sale\_OfferAutoNextStageOnApppointmentCompletedRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_OfferAutoNextStageOnApppointmentCompletedRequest)

 [Sale\_SaleHasQuoteRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_SaleHasQuoteRequest)

 [Sale\_SetAsLostRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_SetAsLostRequest)

 [Sale\_SetAsLostWithReasonRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_SetAsLostWithReasonRequest)

 [Sale\_SetAsSoldRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_SetAsSoldRequest)

 [Sale\_SetAsSoldWithReasonRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_SetAsSoldWithReasonRequest)

 [Sale\_UpdateSaleStakeholdersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_UpdateSaleStakeholdersRequest)

 [Sale\_ValidateSaleEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sale_ValidateSaleEntityRequest)

 [SalesActivity](/en/api/reference/webapi/SuperOffice.WebApi.Data.SalesActivity)

Carrier object for SalesActivity.

 [UserPreferenceStrings.SalesNotification](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.SalesNotification)

 [SavedUtmParameters](/en/api/reference/webapi/SuperOffice.WebApi.Data.SavedUtmParameters)

Carrier object for SavedUtmParameters.
Inital Utm parameters for a given person or contact.

 [Script](/en/api/reference/webapi/SuperOffice.WebApi.Data.Script)

Carrier object for Script.
The Script Service. The service implements all services working with the CRMScript object without reference to primary keys. Primarily used by VSCode integration.

 [SelectableMDOListItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.SelectableMDOListItem)

Carrier object for SelectableMDOListItem.

 [UserPreferenceStrings.Selection](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Selection)

Selection

 [SelectionCategory](/en/api/reference/webapi/SuperOffice.WebApi.Data.SelectionCategory)

Carrier object for SelectionCategory.

 [SelectionEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.SelectionEntity)

Carrier object for SelectionEntity.

 [SelectionForFind](/en/api/reference/webapi/SuperOffice.WebApi.Data.SelectionForFind)

Carrier object for SelectionForFind.
Provider name and selection id for a Find temporary selection

 [SelectionMemberEditValues](/en/api/reference/webapi/SuperOffice.WebApi.Data.SelectionMemberEditValues)

Carrier object for SelectionMemberEditValues.
A class representing values to change when changing company and contact details from the Edit Company Details task for selections.

 [SelectionSummaryItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.SelectionSummaryItem)

Carrier object for SelectionSummaryItem.
The selection summary, for tabs or other concise views.

 [Selection\_AddContactSelectionMembersFromSearchRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_AddContactSelectionMembersFromSearchRequest)

 [Selection\_AddContactSelectionMembersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_AddContactSelectionMembersRequest)

 [Selection\_AddRemoveContactSelectionMemberInterestsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_AddRemoveContactSelectionMemberInterestsRequest)

 [Selection\_AddSelectionMembersFromSearchRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_AddSelectionMembersFromSearchRequest)

 [Selection\_AddSelectionMembersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_AddSelectionMembersRequest)

 [Selection\_CopyContactSelectionMembersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_CopyContactSelectionMembersRequest)

 [Selection\_CopySelectionMembersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_CopySelectionMembersRequest)

 [Selection\_CreateContactSelectionFromSelectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_CreateContactSelectionFromSelectionRequest)

 [Selection\_CreateContactSelectionFromShadowSelectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_CreateContactSelectionFromShadowSelectionRequest)

 [Selection\_CreateDefaultBoardViewSettingsFromEntityTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_CreateDefaultBoardViewSettingsFromEntityTypeRequest)

 [Selection\_CreateNewEntryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_CreateNewEntryRequest)

 [Selection\_CreateSelectionEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_CreateSelectionEntityRequest)

 [Selection\_CreateSelectionFromSelectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_CreateSelectionFromSelectionRequest)

 [Selection\_CreateTemporaryContactSelectionFromContactPersonIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_CreateTemporaryContactSelectionFromContactPersonIdsRequest)

 [Selection\_CreateTemporaryContactSelectionFromSelectionMemberIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_CreateTemporaryContactSelectionFromSelectionMemberIdsRequest)

 [Selection\_CreateTemporaryContactSelectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_CreateTemporaryContactSelectionRequest)

 [Selection\_CreateTemporarySelectionFromIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_CreateTemporarySelectionFromIdsRequest)

 [Selection\_DeleteBoardViewSettingsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_DeleteBoardViewSettingsRequest)

 [Selection\_DeleteContactsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_DeleteContactsRequest)

 [Selection\_DeleteEntitiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_DeleteEntitiesRequest)

 [Selection\_DeletePersonsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_DeletePersonsRequest)

 [Selection\_DeleteSelectionEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_DeleteSelectionEntityRequest)

 [Selection\_EditContactSelectionMemberDetailsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_EditContactSelectionMemberDetailsRequest)

 [Selection\_ExportSelectionMembersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_ExportSelectionMembersRequest)

 [Selection\_ExportSelectionMembersWithOrderByRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_ExportSelectionMembersWithOrderByRequest)

 [Selection\_GenerateFollowUpsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_GenerateFollowUpsRequest)

 [Selection\_GetBoardViewSettingsForSelectionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_GetBoardViewSettingsForSelectionRequest)

 [Selection\_GetBoardViewSettingsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_GetBoardViewSettingsRequest)

 [Selection\_GetDuplicateRulesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_GetDuplicateRulesRequest)

 [Selection\_GetDuplicatesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_GetDuplicatesRequest)

 [Selection\_GetDynamicSelectionCriteriaGroupsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_GetDynamicSelectionCriteriaGroupsRequest)

 [Selection\_GetDynamicSelectionCriteriaRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_GetDynamicSelectionCriteriaRequest)

 [Selection\_GetParentCombinedSelectionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_GetParentCombinedSelectionsRequest)

 [Selection\_GetRecipientStatisticsFromContactPersonIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_GetRecipientStatisticsFromContactPersonIdsRequest)

 [Selection\_GetRecipientStatisticsFromProjectMembersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_GetRecipientStatisticsFromProjectMembersRequest)

 [Selection\_GetRecipientStatisticsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_GetRecipientStatisticsRequest)

 [Selection\_GetSelectionForFindRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_GetSelectionForFindRequest)

 [Selection\_GetSelectionMembersArchiveRowsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_GetSelectionMembersArchiveRowsRequest)

 [Selection\_GetSelectionShadowMembersArchiveRowsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_GetSelectionShadowMembersArchiveRowsRequest)

 [Selection\_GetSelectionSummariesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_GetSelectionSummariesRequest)

 [Selection\_RemoveContactSelectionMembersFromIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_RemoveContactSelectionMembersFromIdsRequest)

 [Selection\_RemoveContactSelectionMembersFromSearchRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_RemoveContactSelectionMembersFromSearchRequest)

 [Selection\_RemoveContactSelectionMembersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_RemoveContactSelectionMembersRequest)

 [Selection\_RemoveSelectionMembersByIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_RemoveSelectionMembersByIdRequest)

 [Selection\_RemoveSelectionMembersFromSearchRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_RemoveSelectionMembersFromSearchRequest)

 [Selection\_RemoveSelectionMembersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_RemoveSelectionMembersRequest)

 [Selection\_SaveBoardViewSettingsForSelectionAndAssociateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_SaveBoardViewSettingsForSelectionAndAssociateRequest)

 [Selection\_SaveBoardViewSettingsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_SaveBoardViewSettingsRequest)

 [Selection\_SetDuplicateRulesStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_SetDuplicateRulesStatusRequest)

 [Selection\_SetDynamicSelectionCriteria2Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_SetDynamicSelectionCriteria2Request)

 [Selection\_SetDynamicSelectionCriteria3Request](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_SetDynamicSelectionCriteria3Request)

 [Selection\_SetDynamicSelectionCriteriaGroupsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_SetDynamicSelectionCriteriaGroupsRequest)

 [Selection\_SetDynamicSelectionCriteriaRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_SetDynamicSelectionCriteriaRequest)

 [Selection\_UpdateTypicalSearchRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Selection_UpdateTypicalSearchRequest)

 [Sentiment](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sentiment)

Carrier object for Sentiment.
Sentiment information: Score (-100 to 100) and Confidence (0 to 100)

 [Sentry](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sentry)

Carrier object for Sentry.
This service provides sentry information such as TableRights.

 [Sentry\_CanCreateAppointmentInAllDiariesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sentry_CanCreateAppointmentInAllDiariesRequest)

 [Sentry\_CanCreateAppointmentInAssociateDiariesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sentry_CanCreateAppointmentInAssociateDiariesRequest)

 [Sentry\_CanCreateAppointmentInEachAssociatesDiaryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sentry_CanCreateAppointmentInEachAssociatesDiaryRequest)

 [Sentry\_GetFunctionRightsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sentry_GetFunctionRightsRequest)

 [Sentry\_GetNewTableRightRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sentry_GetNewTableRightRequest)

 [Sentry\_GetTableRightByContactOwnershipRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sentry_GetTableRightByContactOwnershipRequest)

 [Sentry\_GetTableRightByOwnershipRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sentry_GetTableRightByOwnershipRequest)

 [Sentry\_HasFunctionRightRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Sentry_HasFunctionRightRequest)

 [ServiceAuth](/en/api/reference/webapi/SuperOffice.WebApi.Data.ServiceAuth)

Carrier object for ServiceAuth.
Properties for service authentication

 [ShipmentMessageBlockEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.ShipmentMessageBlockEntity)

Carrier object for ShipmentMessageBlockEntity.
This entity contains information about design blocks that can be used in a shipment/mailing message

 [ShipmentMessageEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.ShipmentMessageEntity)

Carrier object for ShipmentMessageEntity.
A shipment message, that contains the actual message being sent out in a mailing

 [SignedPublicKey](/en/api/reference/webapi/SuperOffice.WebApi.Data.SignedPublicKey)

Carrier object for SignedPublicKey.
Represents all licenses for this owner at this site.

 [SmsConfig](/en/api/reference/webapi/SuperOffice.WebApi.Data.SmsConfig)

Carrier object for SmsConfig.
Sms settings for Customer Service sms providers.

 [SmtpTestResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.SmtpTestResult)

Carrier object for SmtpTestResult.
Carrier containing information about the result of a SMTP test

 [SoExceptionInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.SoExceptionInfo)

Carrier object for SoExceptionInfo.
Representation of a serialized exception that can be communicatyed over web services

 [SoExceptionInfoParameters](/en/api/reference/webapi/SuperOffice.WebApi.Data.SoExceptionInfoParameters)

Serializable dictionary.
This class works as a regular dictionary. The only restriction is that the string is a value type

 [SoExtraInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.SoExtraInfo)

Serializable dictionary.
This class works as a regular dictionary. The only restriction is that the string is a value type

 [SoPrincipalCarrier](/en/api/reference/webapi/SuperOffice.WebApi.Data.SoPrincipalCarrier)

Carrier object for SoPrincipalCarrier.
Representation of a serialized exception that can be communicatyed over web services

 [SoSystemInfoCarrier](/en/api/reference/webapi/SuperOffice.WebApi.Data.SoSystemInfoCarrier)

Carrier object for SoSystemInfoCarrier.
System Information - license, version, database type, language

 [SoTask](/en/api/reference/webapi/SuperOffice.WebApi.Data.SoTask)

Carrier object for SoTask.
Type of an appointment

 [UserPreferenceStrings.Sorting](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Sorting)

Sorting

 [Source](/en/api/reference/webapi/SuperOffice.WebApi.Data.Source)

Carrier object for Source.
Source of a sale

 [UserPreferenceStrings.StandardStartupScreenOptions](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.StandardStartupScreenOptions)

 [StatisticsDataSet](/en/api/reference/webapi/SuperOffice.WebApi.Data.StatisticsDataSet)

Carrier object for StatisticsDataSet.

 [StatisticsDataValue](/en/api/reference/webapi/SuperOffice.WebApi.Data.StatisticsDataValue)

Carrier object for StatisticsDataValue.
This class contains a single data value in a data set, used by CS for Status Screen reports

 [StatusMonitor](/en/api/reference/webapi/SuperOffice.WebApi.Data.StatusMonitor)

Carrier object for StatusMonitor.
Definition of all properties for a status monitor

 [StatusMonitorPeriods](/en/api/reference/webapi/SuperOffice.WebApi.Data.StatusMonitorPeriods)

Carrier object for StatusMonitorPeriods.
Obsolete, use SaintConfiguration carrier and methods instead. Was:Definition of the three perionds for status monitors

 [StringBoolDictionary](/en/api/reference/webapi/SuperOffice.WebApi.Data.StringBoolDictionary)

Serializable dictionary.
This class works as a regular dictionary. The only restriction is that the string is a value type

 [StringDictionary](/en/api/reference/webapi/SuperOffice.WebApi.Data.StringDictionary)

Serializable dictionary.
This class works as a regular dictionary. The only restriction is that the string is a value type

 [StringObjectDictionary](/en/api/reference/webapi/SuperOffice.WebApi.Data.StringObjectDictionary)

Serializable dictionary.
This class works as a regular dictionary. The only restriction is that the string is a value type

 [StructuredAddress](/en/api/reference/webapi/SuperOffice.WebApi.Data.StructuredAddress)

Carrier object for StructuredAddress.
Address with structured fields (Address1, City, Zip) - does not vary its layout by country, unliked the formatted address.

 [SuggestedAppointment](/en/api/reference/webapi/SuperOffice.WebApi.Data.SuggestedAppointment)

Carrier object for SuggestedAppointment.

 [SuggestedAppointmentEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.SuggestedAppointmentEntity)

Carrier object for SuggestedAppointmentEntity.

 [SuggestedDocumentEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.SuggestedDocumentEntity)

Carrier object for SuggestedDocumentEntity.

 [Summarizer](/en/api/reference/webapi/SuperOffice.WebApi.Data.Summarizer)

Carrier object for Summarizer.
Summary functions for various entities

 [UserPreferenceStrings.SuperMode](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.SuperMode)

SuperMode

 [SyncUserAccount](/en/api/reference/webapi/SuperOffice.WebApi.Data.SyncUserAccount)

Carrier object for SyncUserAccount.
Details for syncing email user accounts

 [UserPreferenceStrings.System](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.System)

System

 [SystemEventEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.SystemEventEntity)

Carrier object for SystemEventEntity.
Entity for system events

 [SystemInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.SystemInfo)

Carrier object for SystemInfo.
System information

 [SystemMessage](/en/api/reference/webapi/SuperOffice.WebApi.Data.SystemMessage)

Carrier object for SystemMessage.
System Message to be displayed to a user

 [UserPreferenceStrings.SystemNotification](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.SystemNotification)

 [SystemTemplateSettings](/en/api/reference/webapi/SuperOffice.WebApi.Data.SystemTemplateSettings)

Carrier object for SystemTemplateSettings.
System template settings item

 [TabOrder](/en/api/reference/webapi/SuperOffice.WebApi.Data.TabOrder)

Carrier object for TabOrder.

 [TableManager](/en/api/reference/webapi/SuperOffice.WebApi.Data.TableManager)

Carrier object for TableManager.
Operations related to table management

 [TableRecord](/en/api/reference/webapi/SuperOffice.WebApi.Data.TableRecord)

Carrier object for TableRecord.
The extra table entity contains meta data for an extra table (not implemented yet)

 [TableRight](/en/api/reference/webapi/SuperOffice.WebApi.Data.TableRight)

Carrier object for TableRight.
Wraps a table right together with an explanatory reason.

 [Tag](/en/api/reference/webapi/SuperOffice.WebApi.Data.Tag)

Carrier object for Tag.

 [TargetAssignment](/en/api/reference/webapi/SuperOffice.WebApi.Data.TargetAssignment)

Carrier object for TargetAssignment.
Target Period Assignment carrier

 [TargetChange](/en/api/reference/webapi/SuperOffice.WebApi.Data.TargetChange)

Carrier object for TargetChange.
Target field change

 [TargetDimension](/en/api/reference/webapi/SuperOffice.WebApi.Data.TargetDimension)

Carrier object for TargetDimension.
Target Dimension carrier

 [TargetGroup](/en/api/reference/webapi/SuperOffice.WebApi.Data.TargetGroup)

Carrier object for TargetGroup.
Target Group carrier

 [TargetRevision](/en/api/reference/webapi/SuperOffice.WebApi.Data.TargetRevision)

Carrier object for TargetRevision.
Target revision carrier

 [TargetRevisionHistory](/en/api/reference/webapi/SuperOffice.WebApi.Data.TargetRevisionHistory)

Carrier object for TargetRevisionHistory.
Target revision history carrier

 [UserPreferenceStrings.Targets](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Targets)

 [Targets\_CreateDefaultTargetAssignmentForCompanyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Targets_CreateDefaultTargetAssignmentForCompanyRequest)

 [Targets\_CreateDefaultTargetAssignmentForCompanyWithDimensionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Targets_CreateDefaultTargetAssignmentForCompanyWithDimensionRequest)

 [Targets\_CreateDefaultTargetAssignmentForCompanyWithYearAndDimensionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Targets_CreateDefaultTargetAssignmentForCompanyWithYearAndDimensionRequest)

 [Targets\_CreateDefaultTargetAssignmentForUserGroupRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Targets_CreateDefaultTargetAssignmentForUserGroupRequest)

 [Targets\_CreateDefaultTargetAssignmentForUserGroupWithDimensionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Targets_CreateDefaultTargetAssignmentForUserGroupWithDimensionRequest)

 [Targets\_CreateDefaultTargetAssignmentForUserGroupWithYearAndDimensionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Targets_CreateDefaultTargetAssignmentForUserGroupWithYearAndDimensionRequest)

 [Targets\_ExportTargetGroupToExcelRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Targets_ExportTargetGroupToExcelRequest)

 [Targets\_GetTargetAssignmentForYearAndDimensionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Targets_GetTargetAssignmentForYearAndDimensionRequest)

 [Targets\_GetTargetAssignmentForYearRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Targets_GetTargetAssignmentForYearRequest)

 [Targets\_GetTargetDimensionsForEntityTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Targets_GetTargetDimensionsForEntityTypeRequest)

 [Targets\_GetTargetGroupForYearAndDimensionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Targets_GetTargetGroupForYearAndDimensionRequest)

 [Targets\_GetTargetGroupForYearRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Targets_GetTargetGroupForYearRequest)

 [Targets\_GetTargetGroupIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Targets_GetTargetGroupIdsRequest)

 [Targets\_GetTargetRevisionHistoryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Targets_GetTargetRevisionHistoryRequest)

 [TaskListItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.TaskListItem)

Carrier object for TaskListItem.

 [TaskMenu](/en/api/reference/webapi/SuperOffice.WebApi.Data.TaskMenu)

Carrier object for TaskMenu.
Configurable Menu Items are items that will be merged into menus in given area and entity

 [TemplateInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.TemplateInfo)

Carrier object for TemplateInfo.
Information about a document template, i.e., a document that generally contains unresolved merge tags and that is used as the source for creating new documents

 [TemplateVariablesParameters](/en/api/reference/webapi/SuperOffice.WebApi.Data.TemplateVariablesParameters)

Carrier object for TemplateVariablesParameters.
Parameters specifying source and properties for template variable substitution.

 [TemporaryKeyInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.TemporaryKeyInfo)

Carrier object for TemporaryKeyInfo.
The fields returned when checking a temporary key

 [TenantStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.TenantStatus)

Online tenant status: Endpoint URL, State, IsRunning

 [TextServices](/en/api/reference/webapi/SuperOffice.WebApi.Data.TextServices)

Carrier object for TextServices.
Text generation functions for various entities

 [Ticket](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket)

Carrier object for Ticket.
The ticket carrier is used for representing a service request.

 [TicketBatchReplyData](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketBatchReplyData)

Carrier object for TicketBatchReplyData.
Data to add a batched ticket reply.

 [TicketCategory](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketCategory)

Carrier object for TicketCategory.
Ticket Category - light read-only version of TicketCategoryEntity.

 [TicketCategoryEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketCategoryEntity)

Carrier object for TicketCategoryEntity.
This entity describes the meta data for a ticket category, and provides special operations on it.

 [TicketCategoryMembershipEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketCategoryMembershipEntity)

Carrier object for TicketCategoryMembershipEntity.
This entity describes ticket category membership.

 [TicketEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketEntity)

Carrier object for TicketEntity.
The ticket entity is used for representing a service ticket

 [TicketFormSubmission](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketFormSubmission)

Carrier object for TicketFormSubmission.
Class representing form submission related to ticket.

 [TicketInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketInfo)

Carrier object for TicketInfo.
A small carrier that contains information about a ticket

 [TicketMessage](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketMessage)

Carrier object for TicketMessage.
The TicketMessage carrier is used for representing a service ticket message

 [TicketMessageContent](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketMessageContent)

Carrier object for TicketMessageContent.
Message content to fill a new message.

 [TicketMessageEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketMessageEntity)

Carrier object for TicketMessageEntity.
The ticket messages are e-mails, SMS or other messages attached to a ticket (aka request).

 [TicketMessageHeader](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketMessageHeader)

Carrier object for TicketMessageHeader.
Contains information about a header connected to a ticket message

 [TicketPriority](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketPriority)

Carrier object for TicketPriority.

 [TicketPriorityEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketPriorityEntity)

Carrier object for TicketPriorityEntity.
Entity for a ticket priority. This entity describes the meta data for a ticket priority, and provides special operations on it.

 [TicketPriorityEscalationLevelEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketPriorityEscalationLevelEntity)

Carrier object for TicketPriorityEscalationLevelEntity.
Entity for ticket priority escalation level. Entity describes properties/actions when escalation(alert) happens. Alert is raised according to TicketPriority timing intervals

 [TicketRelationAction](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketRelationAction)

Carrier object for TicketRelationAction.
Relation-driven actions applicable to a ticket, indicating whether to offer closing the parent or whether open children are blocking the ticket from being closed.

 [TicketRelationActionChild](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketRelationActionChild)

Carrier object for TicketRelationActionChild.
An unclosed child ticket affected by a relation-driven action, with the resolved contact e-mail address a close message would be sent to.

 [TicketRelationDefinitionEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketRelationDefinitionEntity)

Carrier object for TicketRelationDefinitionEntity.
Ticket relation definition entity

 [TicketRelationDefinitionResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketRelationDefinitionResult)

Carrier object for TicketRelationDefinitionResult.
Validation result for a single source/destination ticket type combination in a relation definition.

 [TicketRelationEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketRelationEntity)

Carrier object for TicketRelationEntity.
Relationship between two tickets, as described by the TicketRelationDefinition.

 [TicketRelationItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketRelationItem)

Carrier object for TicketRelationItem.
A ticket related to another ticket, including counts of how many further relations it has.

 [TicketRelationResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketRelationResult)

Carrier object for TicketRelationResult.
Result of a ticket relation operation, indicating success or the reason for failure.

 [TicketStateInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketStateInfo)

Carrier object for TicketStateInfo.
The sync state of one ticket: its current last-changed value on the server, whether the client's copy is out of date, and who last changed it.

 [TicketStatusEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketStatusEntity)

Carrier object for TicketStatusEntity.
Entity for a ticket status. This entity describes the meta data for a ticket status, and provides special operations on it.

 [TicketSummaryItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketSummaryItem)

Carrier object for TicketSummaryItem.
The ticket summary with heading, created date.

 [TicketType](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketType)

Carrier object for TicketType.
Ticket Type - light read-only version of TicketTypeEntity.

 [TicketTypeEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketTypeEntity)

Carrier object for TicketTypeEntity.
Entity for a ticket (Request) type. This enables having diversified tickets needed for a specific process.

 [TicketUsage](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketUsage)

Carrier object for TicketUsage.
Operations related to ticket usage

 [TicketVersionInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketVersionInfo)

Carrier object for TicketVersionInfo.
A ticket the client is showing, together with the last-changed value of the client's loaded copy. Sent to the server to ask whether that copy is out of date.

 [Ticket\_AcceptTicketsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_AcceptTicketsRequest)

 [Ticket\_AddAttachmentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_AddAttachmentsRequest)

 [Ticket\_BatchForwardRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_BatchForwardRequest)

 [Ticket\_BatchReplyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_BatchReplyRequest)

 [Ticket\_CalculateMessageRecipientsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_CalculateMessageRecipientsRequest)

 [Ticket\_ClearNotifyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_ClearNotifyRequest)

 [Ticket\_CopyFromCRMDocumentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_CopyFromCRMDocumentRequest)

 [Ticket\_CopyToTempFileRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_CopyToTempFileRequest)

 [Ticket\_CreateDefaultForTicketTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_CreateDefaultForTicketTypeRequest)

 [Ticket\_CreateTicketRelationsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_CreateTicketRelationsRequest)

 [Ticket\_DeleteMessageHeadersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_DeleteMessageHeadersRequest)

 [Ticket\_DeleteTicketEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_DeleteTicketEntityRequest)

 [Ticket\_DeleteTicketMessageEntityRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_DeleteTicketMessageEntityRequest)

 [Ticket\_DoEscalatingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_DoEscalatingRequest)

 [Ticket\_GetAttachmentInfoNonInlineRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetAttachmentInfoNonInlineRequest)

 [Ticket\_GetAttachmentInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetAttachmentInfoRequest)

 [Ticket\_GetAttachmentPreviewRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetAttachmentPreviewRequest)

 [Ticket\_GetAttachmentStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetAttachmentStreamRequest)

 [Ticket\_GetDefaultMessageContentFullRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetDefaultMessageContentFullRequest)

 [Ticket\_GetDefaultMessageContentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetDefaultMessageContentRequest)

 [Ticket\_GetDefaultMessageContentWithOptionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetDefaultMessageContentWithOptionsRequest)

 [Ticket\_GetForRmUiRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetForRmUiRequest)

 [Ticket\_GetHotlistEjUsersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetHotlistEjUsersRequest)

 [Ticket\_GetNextInQueueRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetNextInQueueRequest)

 [Ticket\_GetPreviewAttachmentStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetPreviewAttachmentStreamRequest)

 [Ticket\_GetRfcAttachmentStreamRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetRfcAttachmentStreamRequest)

 [Ticket\_GetTicketAttachmentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetTicketAttachmentsRequest)

 [Ticket\_GetTicketMessageEntitiesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetTicketMessageEntitiesRequest)

 [Ticket\_GetTicketMessageWithEmbeddedDataRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetTicketMessageWithEmbeddedDataRequest)

 [Ticket\_GetTicketMessageWithOptionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetTicketMessageWithOptionsRequest)

 [Ticket\_GetTicketParentIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetTicketParentIdRequest)

 [Ticket\_GetTicketRelationActionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetTicketRelationActionsRequest)

 [Ticket\_GetTicketRelationsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetTicketRelationsRequest)

 [Ticket\_GetTicketSummariesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetTicketSummariesRequest)

 [Ticket\_GetTicketsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_GetTicketsRequest)

 [Ticket\_Html2TextRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_Html2TextRequest)

 [Ticket\_MergeTicketsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_MergeTicketsRequest)

 [Ticket\_NotifyNewTicketMessageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_NotifyNewTicketMessageRequest)

 [Ticket\_NotifyNewTicketRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_NotifyNewTicketRequest)

 [Ticket\_ProcessTicketWhenReadRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_ProcessTicketWhenReadRequest)

 [Ticket\_RemoveMessageAttachmentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_RemoveMessageAttachmentsRequest)

 [Ticket\_ResolveTicketIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_ResolveTicketIdRequest)

 [Ticket\_SanitizeMailContentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_SanitizeMailContentRequest)

 [Ticket\_SanitizeMailContentWithOptionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_SanitizeMailContentWithOptionsRequest)

 [Ticket\_SanitizeMailContentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_SanitizeMailContentsRequest)

 [Ticket\_SaveTicketEntityWithNotifyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_SaveTicketEntityWithNotifyRequest)

 [Ticket\_SaveTicketMessageEntityWithNotifyRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_SaveTicketMessageEntityWithNotifyRequest)

 [Ticket\_SaveTicketMessageEntityWithOptionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_SaveTicketMessageEntityWithOptionsRequest)

 [Ticket\_SendTicketMessageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_SendTicketMessageRequest)

 [Ticket\_SendTicketMessageSmsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_SendTicketMessageSmsRequest)

 [Ticket\_SetDeletedStatusByIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_SetDeletedStatusByIdsRequest)

 [Ticket\_SetDeletedStatusByProviderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_SetDeletedStatusByProviderRequest)

 [Ticket\_SetSpamStatusByIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_SetSpamStatusByIdsRequest)

 [Ticket\_SetSpamStatusByProviderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_SetSpamStatusByProviderRequest)

 [Ticket\_SetTicketMessageImportantRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_SetTicketMessageImportantRequest)

 [Ticket\_SetTicketParentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_SetTicketParentRequest)

 [Ticket\_SetTicketReadByOwnerRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_SetTicketReadByOwnerRequest)

 [Ticket\_SplitTicketMessageRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_SplitTicketMessageRequest)

 [Ticket\_SplitTicketRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_SplitTicketRequest)

 [Ticket\_UndeleteByIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_UndeleteByIdsRequest)

 [Ticket\_UpdateTicketsReadStatusByProviderRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_UpdateTicketsReadStatusByProviderRequest)

 [Ticket\_UpdateTicketsReadStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_UpdateTicketsReadStatusRequest)

 [Ticket\_UploadAttachmentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_UploadAttachmentRequest)

 [Ticket\_ValidateAttachmentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_ValidateAttachmentsRequest)

 [Ticket\_ValidateTicketRelationsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Ticket_ValidateTicketRelationsRequest)

 [TileData](/en/api/reference/webapi/SuperOffice.WebApi.Data.TileData)

Carrier object for TileData.
Class representing data to be visualized in a dashboard tile.

 [TimeZoneData](/en/api/reference/webapi/SuperOffice.WebApi.Data.TimeZoneData)

Carrier object for TimeZoneData.
Information about a TimeZone

 [TimeZoneRule](/en/api/reference/webapi/SuperOffice.WebApi.Data.TimeZoneRule)

Carrier object for TimeZoneRule.
Information about a rule for a TimeZone

 [TimeZoneRuleDictionary](/en/api/reference/webapi/SuperOffice.WebApi.Data.TimeZoneRuleDictionary)

Serializable dictionary.
This class works as a regular dictionary. The only restriction is that the string is a value type

 [TimeZone\_CheckNewTimeZoneDataAvailableRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.TimeZone_CheckNewTimeZoneDataAvailableRequest)

 [TimeZone\_DeleteTimeZonesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.TimeZone_DeleteTimeZonesRequest)

 [TimeZone\_GetBaseTimeZoneIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.TimeZone_GetBaseTimeZoneIdRequest)

 [TimeZone\_GetDefaultTimeZoneInformationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.TimeZone_GetDefaultTimeZoneInformationRequest)

 [TimeZone\_GetDefaultTimeZonePreferenceRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.TimeZone_GetDefaultTimeZonePreferenceRequest)

 [TimeZone\_InitializeTimeZoneDataRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.TimeZone_InitializeTimeZoneDataRequest)

 [TimeZone\_SetActiveTimeZoneRowRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.TimeZone_SetActiveTimeZoneRowRequest)

 [TimeZone\_SetActiveTimeZonesByFilterRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.TimeZone_SetActiveTimeZonesByFilterRequest)

 [TimeZone\_SetBaseTimeZoneIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.TimeZone_SetBaseTimeZoneIdRequest)

 [TimeZone\_TimeOfLastTimeZoneUpdateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.TimeZone_TimeOfLastTimeZoneUpdateRequest)

 [TimeZone\_ToggleActiveTimeZoneRowRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.TimeZone_ToggleActiveTimeZoneRowRequest)

 [TimeZone\_UpdateTimeZoneDataRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.TimeZone_UpdateTimeZoneDataRequest)

 [TokenManagementInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.TokenManagementInfo)

Carrier object for TokenManagementInfo.
Token information

 [UserPreferenceStrings.ToolTips](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.ToolTips)

Tooltips in the application

 [Tooltips](/en/api/reference/webapi/SuperOffice.WebApi.Data.Tooltips)

Carrier object for Tooltips.
This service provides tooltips. It is based on 'tooltip hints', strings that may eiehter be a tooltip text (if provided by someone else), or a set of key/value pairs or other specification.&lt;para /&gt;The tooltip implementation is plugin based, and tooltip hints are passed to all registered plugin providers according to certain rules. It is therefore up to providers to parse and process tooltip hints as they see fit.

 [Tooltips\_GetTooltipRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Tooltips_GetTooltipRequest)

 [UserPreferenceStrings.Travel](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Travel)

Travel

 [TriggerScriptEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.TriggerScriptEntity)

Carrier object for TriggerScriptEntity.
CRM Trigger Script - The script runs when a given event fires.

 [TypicalSearch](/en/api/reference/webapi/SuperOffice.WebApi.Data.TypicalSearch)

Carrier object for TypicalSearch.
Complete information about a TypicalSearch (including criteria)

 [TypicalSearches](/en/api/reference/webapi/SuperOffice.WebApi.Data.TypicalSearches)

Carrier object for TypicalSearches.
Full set of all TypicalSearch elements maintained by one source (including criteria)

 [UiEvent](/en/api/reference/webapi/SuperOffice.WebApi.Data.UiEvent)

Carrier object for UiEvent.
An event that the user interface should know about. The origin may be something that happens in the user interface (in any session), or in the database (for instance, background process)

 [UntrustedCredentials](/en/api/reference/webapi/SuperOffice.WebApi.Data.UntrustedCredentials)

Carrier object for UntrustedCredentials.
Credentials for external authentication.

 [UpsertRowStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.UpsertRowStatus)

Carrier object for UpsertRowStatus.
Information about a row of the results of a mass-update operation

 [UsageStats](/en/api/reference/webapi/SuperOffice.WebApi.Data.UsageStats)

Carrier object for UsageStats.

 [User](/en/api/reference/webapi/SuperOffice.WebApi.Data.User)

Carrier object for User.
SuperOffice User, with login credentials and an associated person.

 [UserAndInstallationLicenceStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserAndInstallationLicenceStatus)

Carrier object for UserAndInstallationLicenceStatus.
Result of GetUserAndInstallationLicenceStatus

 [UserCandidate](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserCandidate)

Carrier object for UserCandidate.
The UserCandidate carrier is used to represent a Customer Centre User.

 [UserConnectedState](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserConnectedState)

Carrier object for UserConnectedState.
Details about the users connected state.

 [UserDefinedFieldInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo)

Carrier object for UserDefinedFieldInfo.
Return information about the user defined fields. The information can be used to mirror the layout of the user-defined fields in the user interface.

 [UserDefinedFieldInfo\_ChangeFieldTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_ChangeFieldTypeRequest)

 [UserDefinedFieldInfo\_ClearCachesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_ClearCachesRequest)

 [UserDefinedFieldInfo\_CreateUserDefinedFieldInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_CreateUserDefinedFieldInfoRequest)

 [UserDefinedFieldInfo\_DetectColumnIdChangesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_DetectColumnIdChangesRequest)

 [UserDefinedFieldInfo\_DetectUnpublishedChangesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_DetectUnpublishedChangesRequest)

 [UserDefinedFieldInfo\_FigureOutListIdFromListTableIdAndUDListDefinitionIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_FigureOutListIdFromListTableIdAndUDListDefinitionIdRequest)

 [UserDefinedFieldInfo\_GetChangedPageOneFieldsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_GetChangedPageOneFieldsRequest)

 [UserDefinedFieldInfo\_GetCustomFieldInfoListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_GetCustomFieldInfoListRequest)

 [UserDefinedFieldInfo\_GetCustomFieldInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_GetCustomFieldInfoRequest)

 [UserDefinedFieldInfo\_GetPublishedUserDefinedFieldFromFieldLabelRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_GetPublishedUserDefinedFieldFromFieldLabelRequest)

 [UserDefinedFieldInfo\_GetPublishedUserDefinedFieldFromProgIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_GetPublishedUserDefinedFieldFromProgIdRequest)

 [UserDefinedFieldInfo\_GetPublishedUserDefinedFieldListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_GetPublishedUserDefinedFieldListRequest)

 [UserDefinedFieldInfo\_GetPublishedUserDefinedPageOneFieldsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_GetPublishedUserDefinedPageOneFieldsRequest)

 [UserDefinedFieldInfo\_GetUserDefinedFieldFromFieldLabelRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_GetUserDefinedFieldFromFieldLabelRequest)

 [UserDefinedFieldInfo\_GetUserDefinedFieldFromIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_GetUserDefinedFieldFromIdsRequest)

 [UserDefinedFieldInfo\_GetUserDefinedFieldFromProgIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_GetUserDefinedFieldFromProgIdRequest)

 [UserDefinedFieldInfo\_GetUserDefinedFieldFromProgIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_GetUserDefinedFieldFromProgIdsRequest)

 [UserDefinedFieldInfo\_GetUserDefinedFieldListRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_GetUserDefinedFieldListRequest)

 [UserDefinedFieldInfo\_GetUserDefinedPageOneFieldsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_GetUserDefinedPageOneFieldsRequest)

 [UserDefinedFieldInfo\_IsAnyPublishEventActiveRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_IsAnyPublishEventActiveRequest)

 [UserDefinedFieldInfo\_IsPublishEventActiveRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_IsPublishEventActiveRequest)

 [UserDefinedFieldInfo\_PublishRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_PublishRequest)

 [UserDefinedFieldInfo\_RevertRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_RevertRequest)

 [UserDefinedFieldInfo\_SaveUserDefinedFieldInfosRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_SaveUserDefinedFieldInfosRequest)

 [UserDefinedFieldInfo\_SetListTableIdAndUDListDefinitionIdFromSelectedListIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_SetListTableIdAndUDListDefinitionIdFromSelectedListIdRequest)

 [UserDefinedFieldInfo\_SetPageOneFieldGroupingRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_SetPageOneFieldGroupingRequest)

 [UserDefinedFieldInfo\_SetPublishStartSystemEventRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_SetPublishStartSystemEventRequest)

 [UserDefinedFieldInfo\_SetRankOnFieldsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_SetRankOnFieldsRequest)

 [UserDefinedFieldInfo\_SetUserDefinedPageOneFieldRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserDefinedFieldInfo_SetUserDefinedPageOneFieldRequest)

 [UserGroup](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserGroup)

Carrier object for UserGroup.
The main user group that this user belongs to

 [UserInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserInfo)

Carrier object for UserInfo.

 [UserPreferenceStrings](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings)

Helper class that exposes section and key names for the UserPreference table. The top level
members of this class are classes that represent sections; use the .Section member to get the actual
section name. Inner members represent the keys within each section.. The PreferenceLevel enum contains the
values for the DefLevel element of the UserPreference table.

 [UserPresenceStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPresenceStatus)

Carrier object for UserPresenceStatus.
Manage present/absent status for a Service user account (influences ticket assignments).

 [UserValidationResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserValidationResult)

Carrier object for UserValidationResult.

 [User\_AddOwnerContactRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_AddOwnerContactRequest)

 [User\_CanChangePasswordRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_CanChangePasswordRequest)

 [User\_ChangeOwnPasswordRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_ChangeOwnPasswordRequest)

 [User\_ChangePasswordFromNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_ChangePasswordFromNameRequest)

 [User\_ChangePasswordRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_ChangePasswordRequest)

 [User\_ChangeUserTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_ChangeUserTypeRequest)

 [User\_CheckAccessGatewayRegistrationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_CheckAccessGatewayRegistrationRequest)

 [User\_CreateDefaultRoleEntityFromTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_CreateDefaultRoleEntityFromTypeRequest)

 [User\_CreateDefaultUserFromUserTypeAndCredentialRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_CreateDefaultUserFromUserTypeAndCredentialRequest)

 [User\_CreateDefaultUserFromUserTypeAndPersonIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_CreateDefaultUserFromUserTypeAndPersonIdRequest)

 [User\_CreateDefaultUserFromUserTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_CreateDefaultUserFromUserTypeRequest)

 [User\_CreateExternalUserRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_CreateExternalUserRequest)

 [User\_CreateUserGroupRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_CreateUserGroupRequest)

 [User\_DecryptAndSaveExternalTokensRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_DecryptAndSaveExternalTokensRequest)

 [User\_DeleteCredentialRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_DeleteCredentialRequest)

 [User\_DeleteExternalUserRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_DeleteExternalUserRequest)

 [User\_DeleteRoleRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_DeleteRoleRequest)

 [User\_DeleteUserFromNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_DeleteUserFromNameRequest)

 [User\_DeleteUserGroupRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_DeleteUserGroupRequest)

 [User\_ExchangeCodeAndSaveTokensRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_ExchangeCodeAndSaveTokensRequest)

 [User\_FindCredentialUsersRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_FindCredentialUsersRequest)

 [User\_FindCredentialsGroupsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_FindCredentialsGroupsRequest)

 [User\_FindRolesWithFunctionalRightRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_FindRolesWithFunctionalRightRequest)

 [User\_FindRolesWithFunctionalRightsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_FindRolesWithFunctionalRightsRequest)

 [User\_FindRolesWithoutFunctionalRightsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_FindRolesWithoutFunctionalRightsRequest)

 [User\_GenerateNewPasswordForExternalUserRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GenerateNewPasswordForExternalUserRequest)

 [User\_GetAccessGatewayInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetAccessGatewayInfoRequest)

 [User\_GetAccessTokenRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetAccessTokenRequest)

 [User\_GetAllDataRightsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetAllDataRightsRequest)

 [User\_GetAllFunctionalRightsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetAllFunctionalRightsRequest)

 [User\_GetAllRolesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetAllRolesRequest)

 [User\_GetAllUserGroupsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetAllUserGroupsRequest)

 [User\_GetCredentialTypesForUserTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetCredentialTypesForUserTypeRequest)

 [User\_GetCredentialTypesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetCredentialTypesRequest)

 [User\_GetCredentialUsersInGroupRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetCredentialUsersInGroupRequest)

 [User\_GetDataRightRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetDataRightRequest)

 [User\_GetDataRightsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetDataRightsRequest)

 [User\_GetDefaultAssociateUserNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetDefaultAssociateUserNameRequest)

 [User\_GetExternalTokensRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetExternalTokensRequest)

 [User\_GetFunctionalRightsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetFunctionalRightsRequest)

 [User\_GetOrRegisterAccessGatewayInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetOrRegisterAccessGatewayInfoRequest)

 [User\_GetSupportedChangePasswordTypesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetSupportedChangePasswordTypesRequest)

 [User\_GetUntrustedCredentialsForAssociateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetUntrustedCredentialsForAssociateRequest)

 [User\_GetUntrustedCredentialsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetUntrustedCredentialsRequest)

 [User\_GetUserFromEjUserIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetUserFromEjUserIdRequest)

 [User\_GetUserFromNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetUserFromNameRequest)

 [User\_GetUserFromPersonIdRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetUserFromPersonIdRequest)

 [User\_GetUserPresenceStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetUserPresenceStatusRequest)

 [User\_GetValidUserNameForNewUserRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetValidUserNameForNewUserRequest)

 [User\_GetValidUserNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_GetValidUserNameRequest)

 [User\_IsNickNameUniqueRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_IsNickNameUniqueRequest)

 [User\_IsPasswordValidRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_IsPasswordValidRequest)

 [User\_IsPasswordValidWithReasonRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_IsPasswordValidWithReasonRequest)

 [User\_IsUserNameValidRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_IsUserNameValidRequest)

 [User\_IsUserNameValidWithReasonRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_IsUserNameValidWithReasonRequest)

 [User\_MakeRetiredRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_MakeRetiredRequest)

 [User\_RegisterWithAccessGatewayRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_RegisterWithAccessGatewayRequest)

 [User\_RemoveLicensesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_RemoveLicensesRequest)

 [User\_RemoveOwnerContactRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_RemoveOwnerContactRequest)

 [User\_RemoveUntrustedCredentialsForAssociateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_RemoveUntrustedCredentialsForAssociateRequest)

 [User\_RemoveUntrustedCredentialsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_RemoveUntrustedCredentialsRequest)

 [User\_ResolveUserFromInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_ResolveUserFromInfoRequest)

 [User\_SaveCredentialRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_SaveCredentialRequest)

 [User\_SaveExternalTokensRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_SaveExternalTokensRequest)

 [User\_SaveUntrustedCredentialsForAssociateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_SaveUntrustedCredentialsForAssociateRequest)

 [User\_SaveUntrustedCredentialsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_SaveUntrustedCredentialsRequest)

 [User\_SaveUserFromNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_SaveUserFromNameRequest)

 [User\_SaveUserGroupRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_SaveUserGroupRequest)

 [User\_SendChangePasswordEMailRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_SendChangePasswordEMailRequest)

 [User\_SetDataRightRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_SetDataRightRequest)

 [User\_SetExternalUserInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_SetExternalUserInfoRequest)

 [User\_SetFunctionalRightsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_SetFunctionalRightsRequest)

 [User\_SetGeneratedPasswordFromNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_SetGeneratedPasswordFromNameRequest)

 [User\_SetGeneratedPasswordRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_SetGeneratedPasswordRequest)

 [User\_SetPasswordFromNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_SetPasswordFromNameRequest)

 [User\_SetPasswordRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_SetPasswordRequest)

 [User\_SetUserPresenceStatusRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_SetUserPresenceStatusRequest)

 [User\_ValidateUserRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.User_ValidateUserRequest)

 [UtmParameters](/en/api/reference/webapi/SuperOffice.WebApi.Data.UtmParameters)

Carrier object for UtmParameters.
Utm parameters for a given person.

 [VersionInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.VersionInfo)

Carrier object for VersionInfo.
An instance of this class represents information about one version of one document

 [VideoMeetingReservation](/en/api/reference/webapi/SuperOffice.WebApi.Data.VideoMeetingReservation)

Carrier object for VideoMeetingReservation.
VideoMeetingReservation - Create or delete a VideoMeetingReservation. The VideoMeetingReservation.MeetingId can be linked to an Appointment

 [UserPreferenceStrings.VideoMeetings](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.VideoMeetings)

 [ViewState\_DeleteHistoryRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ViewState_DeleteHistoryRequest)

 [ViewState\_GetCurrentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ViewState_GetCurrentRequest)

 [ViewState\_GetHistoriesByNameRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ViewState_GetHistoriesByNameRequest)

 [ViewState\_GetHistoriesByNamesAndIdsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ViewState_GetHistoriesByNamesAndIdsRequest)

 [ViewState\_GetHistoriesByNamesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ViewState_GetHistoriesByNamesRequest)

 [ViewState\_GetHistoriesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ViewState_GetHistoriesRequest)

 [ViewState\_GetHistoryLengthPrefValueRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ViewState_GetHistoryLengthPrefValueRequest)

 [ViewState\_GetLiveUiConfigRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ViewState_GetLiveUiConfigRequest)

 [ViewState\_GetNextCurrentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ViewState_GetNextCurrentRequest)

 [ViewState\_GetPreviousCurrentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ViewState_GetPreviousCurrentRequest)

 [ViewState\_PublishAndRetrieveUiEventsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ViewState_PublishAndRetrieveUiEventsRequest)

 [ViewState\_PublishAndSyncUiEventsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ViewState_PublishAndSyncUiEventsRequest)

 [ViewState\_SaveCurrentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ViewState_SaveCurrentRequest)

 [ViewState\_SaveCurrentsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ViewState_SaveCurrentsRequest)

 [ViewState\_SaveHistoriesRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ViewState_SaveHistoriesRequest)

 [ViewState\_SetHistoryLengthPrefValueRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ViewState_SetHistoryLengthPrefValueRequest)

 [ViewState\_UpsertRecentNavigationRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.ViewState_UpsertRecentNavigationRequest)

 [VisibleFor](/en/api/reference/webapi/SuperOffice.WebApi.Data.VisibleFor)

Carrier object for VisibleFor.

 [UserPreferenceStrings.Visual](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Visual)

Visual effects

 [WebAppUsage](/en/api/reference/webapi/SuperOffice.WebApi.Data.WebAppUsage)

Carrier object for WebAppUsage.

 [WebPanelEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.WebPanelEntity)

Carrier object for WebPanelEntity.
The web panel entity contains information on a web panel

 [UserPreferenceStrings.WebServer](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.WebServer)

Web server

 [Webhook](/en/api/reference/webapi/SuperOffice.WebApi.Data.Webhook)

Carrier object for Webhook.
Webhook definitions. Webhooks broadcast events from NetServer to remote servers.

 [WebhookResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.WebhookResult)

Carrier object for WebhookResult.
Result of calling a webhook

 [Webhook\_GetAllWebhooksRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Webhook_GetAllWebhooksRequest)

 [Webhook\_GetLastErrorRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Webhook_GetLastErrorRequest)

 [Webhook\_SignalEventRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Webhook_SignalEventRequest)

 [Webhook\_TestWebhookRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Webhook_TestWebhookRequest)

 [WindowPosSize](/en/api/reference/webapi/SuperOffice.WebApi.Data.WindowPosSize)

Carrier object for WindowPosSize.
Window and dialog position and size settings

 [UserPreferenceStrings.Wizard](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.Wizard)

Wizard

 [WorkflowContentInfo](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowContentInfo)

Carrier object for WorkflowContentInfo.
Info about workflow content like messages, forms and links.

 [WorkflowEvent](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowEvent)

Carrier object for WorkflowEvent.
Workflow event carrier

 [WorkflowEventResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowEventResult)

Carrier object for WorkflowEventResult.
Workflow event result carrier

 [WorkflowFilter](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowFilter)

Carrier object for WorkflowFilter.
Workflow filter carrier

 [WorkflowGoal](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowGoal)

Carrier object for WorkflowGoal.
Workflow goal carrier

 [WorkflowStepAddToList](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepAddToList)

Carrier object for WorkflowStepAddToList.
Step in workflow.

 [WorkflowStepBase](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepBase)

Carrier object for WorkflowStepBase.
Workflow step carrier.

 [WorkflowStepCreateFollowUp](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepCreateFollowUp)

Carrier object for WorkflowStepCreateFollowUp.
Step in workflow.

 [WorkflowStepCreateRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepCreateRequest)

Carrier object for WorkflowStepCreateRequest.
Step in workflow.

 [WorkflowStepCreateSale](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepCreateSale)

Carrier object for WorkflowStepCreateSale.
Step in workflow.

 [WorkflowStepDistributeToOwner](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepDistributeToOwner)

Carrier object for WorkflowStepDistributeToOwner.
Step in workflow.

 [WorkflowStepNotifyByEmail](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepNotifyByEmail)

Carrier object for WorkflowStepNotifyByEmail.
Step in workflow.

 [WorkflowStepNotifyBySMS](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepNotifyBySMS)

Carrier object for WorkflowStepNotifyBySMS.
Step in workflow.

 [WorkflowStepOptionBase](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepOptionBase)

Carrier object for WorkflowStepOptionBase.
Workflow step split option carrier base carrier class.

 [WorkflowStepOptionFormData](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepOptionFormData)

Carrier object for WorkflowStepOptionFormData.
Option that splits on matching data on a specific form submitted

 [WorkflowStepOptionFormSubmission](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepOptionFormSubmission)

Carrier object for WorkflowStepOptionFormSubmission.
Option that splits on a form is submitted or not

 [WorkflowStepOptionLinkClicked](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepOptionLinkClicked)

Carrier object for WorkflowStepOptionLinkClicked.
Option that splits on if a link is clicked or not

 [WorkflowStepOptionPersonData](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepOptionPersonData)

Carrier object for WorkflowStepOptionPersonData.
Option that splits on matching person criteria

 [WorkflowStepOptionProjectMembership](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepOptionProjectMembership)

Carrier object for WorkflowStepOptionProjectMembership.
Option that splits on project membership

 [WorkflowStepOptionSelectionMembership](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepOptionSelectionMembership)

Carrier object for WorkflowStepOptionSelectionMembership.
Option that splits on selection membership

 [WorkflowStepRemoveFromList](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepRemoveFromList)

Carrier object for WorkflowStepRemoveFromList.
Step in workflow.

 [WorkflowStepRunScript](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepRunScript)

Carrier object for WorkflowStepRunScript.
Step in workflow.

 [WorkflowStepSendEmail](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepSendEmail)

Carrier object for WorkflowStepSendEmail.
Step in workflow that waits for time.

 [WorkflowStepSendSMS](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepSendSMS)

Carrier object for WorkflowStepSendSMS.
Step in workflow.

 [WorkflowStepSplit](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepSplit)

Carrier object for WorkflowStepSplit.
Step in workflow.

 [WorkflowStepUpdateParticipant](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepUpdateParticipant)

Carrier object for WorkflowStepUpdateParticipant.
Step in workflow.

 [WorkflowStepWaitForAction](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepWaitForAction)

Carrier object for WorkflowStepWaitForAction.
Step in workflow.

 [WorkflowStepWaitForTime](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepWaitForTime)

Carrier object for WorkflowStepWaitForTime.
Step in workflow that waits for time.

 [WorkflowTrigger](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowTrigger)

Carrier object for WorkflowTrigger.
Workflow trigger carrier

 [WorkflowWaitForAction](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowWaitForAction)

Carrier object for WorkflowWaitForAction.
Workflow action info carrier

 [Workflow\_AddAfterWorkflowStepRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Workflow_AddAfterWorkflowStepRequest)

 [Workflow\_ConnectEmailFlowContentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Workflow_ConnectEmailFlowContentRequest)

 [Workflow\_CopyEmailFlowContentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Workflow_CopyEmailFlowContentRequest)

 [Workflow\_CopyEmailFlowRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Workflow_CopyEmailFlowRequest)

 [Workflow\_CreateDefaultWorkflowStepFromTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Workflow_CreateDefaultWorkflowStepFromTypeRequest)

 [Workflow\_CreateDefaultWorkflowStepOptionFromTypeRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Workflow_CreateDefaultWorkflowStepOptionFromTypeRequest)

 [Workflow\_CreateEmailFlowContentRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Workflow_CreateEmailFlowContentRequest)

 [Workflow\_GetWorkflowFencingInfoRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Workflow_GetWorkflowFencingInfoRequest)

 [Workflow\_GetWorkflowStepOptionRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Workflow_GetWorkflowStepOptionRequest)

 [Workflow\_GetWorkflowStepRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Workflow_GetWorkflowStepRequest)

 [Workflow\_RemoveParticipantsFromEmailFlowRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Workflow_RemoveParticipantsFromEmailFlowRequest)

 [Workflow\_RunRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Workflow_RunRequest)

 [Workflow\_SendEventRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Workflow_SendEventRequest)

 [Workflow\_SetStatusOnWorkflowRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Workflow_SetStatusOnWorkflowRequest)

 [Workflow\_TryAddPersonsToEmailFlowRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Workflow_TryAddPersonsToEmailFlowRequest)

 [Workflow\_UpdateFormSubmissionsRequest](/en/api/reference/webapi/SuperOffice.WebApi.Data.Workflow_UpdateFormSubmissionsRequest)

 [XsrfValue](/en/api/reference/webapi/SuperOffice.WebApi.Data.XsrfValue)

Carrier object for XsrfValue.
XSRF token value and last update time

 [UserPreferenceStrings.ZFD](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserPreferenceStrings.ZFD)

Preferences for Zero Footprint Documents

### Enums

 [ActivityStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.ActivityStatus)

Completed status enum common to appointments, documents and sales

 [AddressType](/en/api/reference/webapi/SuperOffice.WebApi.Data.AddressType)

The type of Address
This Enum is used by:  address.atype_idx

 [AiTextStyle](/en/api/reference/webapi/SuperOffice.WebApi.Data.AiTextStyle)

AI Text reformmating styles

 [AppointmentCautionWarning](/en/api/reference/webapi/SuperOffice.WebApi.Data.AppointmentCautionWarning)

Appointment and invitation synchronization info
This Enum is used by:  appointment.cautionWarning

 [AppointmentExternalOwner](/en/api/reference/webapi/SuperOffice.WebApi.Data.AppointmentExternalOwner)

An external owner system for appointments (Outlook, Google...)
This Enum is used by:  appointment.owned_externally

 [AppointmentPrivate](/en/api/reference/webapi/SuperOffice.WebApi.Data.AppointmentPrivate)

Value for field 'private' in table 'appointment'.
This Enum is used by:  appointment.private

 [AppointmentStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.AppointmentStatus)

Value for field 'status' in table 'appointment'.
This Enum is used by:  appointment.status

 [AppointmentType](/en/api/reference/webapi/SuperOffice.WebApi.Data.AppointmentType)

Value for field 'type' in table 'appointment'.
This Enum is used by:  appointment.type

 [ArchiveBehaviour](/en/api/reference/webapi/SuperOffice.WebApi.Data.ArchiveBehaviour)

Behaviour in archives
This Enum is used by:  TaskMenu.archiveBehaviour
This Enum consists of flag values that can be combined.

 [AssignmentStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.AssignmentStatus)

Status if this appointment is in the process of being assigned to someone else

 [AssociateSourceType](/en/api/reference/webapi/SuperOffice.WebApi.Data.AssociateSourceType)

Describes what source the associates should be retrieved from. 
This Enum consists of flag values that can be combined.

 [BadgeType](/en/api/reference/webapi/SuperOffice.WebApi.Data.BadgeType)

Value for field 'badge' in table 'ej_message'. Defines the initial source of a message.
This Enum is used by:  ej_message.badge

 [BatchTaskCancellationBehaviour](/en/api/reference/webapi/SuperOffice.WebApi.Data.BatchTaskCancellationBehaviour)

Batch task cancel support

 [BatchTaskState](/en/api/reference/webapi/SuperOffice.WebApi.Data.BatchTaskState)

 [BlobLinkType](/en/api/reference/webapi/SuperOffice.WebApi.Data.BlobLinkType)

The link type, often synonymous with the blob type, of a BinaryObjectLink row
This Enum is used by:  BinaryObjectLink.linkType

 [BoardViewBadgeType](/en/api/reference/webapi/SuperOffice.WebApi.Data.BoardViewBadgeType)

Board view badge type

 [BoardViewEntityType](/en/api/reference/webapi/SuperOffice.WebApi.Data.BoardViewEntityType)

Board view entity type
This Enum is used by:  board_view_settings.entity_type

 [BookingType](/en/api/reference/webapi/SuperOffice.WebApi.Data.BookingType)

The type of booking the appointment represents

 [CalMethod](/en/api/reference/webapi/SuperOffice.WebApi.Data.CalMethod)

Valid iCal methods

 [CalReplyStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.CalReplyStatus)

iCal reply status

 [CategoryGroup](/en/api/reference/webapi/SuperOffice.WebApi.Data.CategoryGroup)

5 different type a category can be grouped as
This Enum is used by:  Category.category_group

 [CategoryUpdateTrigger](/en/api/reference/webapi/SuperOffice.WebApi.Data.CategoryUpdateTrigger)

Enum for type of change to trigger update of category 
This Enum is used by:  AutomatedCategoryUpdate.trigger_type

 [ChangePasswordType](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChangePasswordType)

Different methods the user may use to change password

 [ChatMessageSpecialType](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChatMessageSpecialType)

Chat Message Special type
This Enum is used by:  chat_message.special_type

 [ChatMessageType](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChatMessageType)

Chat Message type
This Enum is used by:  chat_message.type

 [ChatSessionFlags](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChatSessionFlags)

Value for field 'flags' in table 'chat_session'.
This Enum is used by:  chat_session.flags
This Enum consists of flag values that can be combined.

 [ChatSessionStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChatSessionStatus)

Chat Session status
This Enum is used by:  chat_session.status

 [ChatWidgetSize](/en/api/reference/webapi/SuperOffice.WebApi.Data.ChatWidgetSize)

Chat widget size: normal or large

 [CheckoutState](/en/api/reference/webapi/SuperOffice.WebApi.Data.CheckoutState)

Enum listing the possible checkout states of a document, as seen from outside the document plugin

 [ColorIndex](/en/api/reference/webapi/SuperOffice.WebApi.Data.ColorIndex)

Color index of tasks

 [CommandAction](/en/api/reference/webapi/SuperOffice.WebApi.Data.CommandAction)

Command action

 [CommandActionResult](/en/api/reference/webapi/SuperOffice.WebApi.Data.CommandActionResult)

Result of an action

 [ConfigType](/en/api/reference/webapi/SuperOffice.WebApi.Data.ConfigType)

Value for field 'type' in table 'cust_config'.
This Enum is used by:  cust_config.type

 [ContactAction](/en/api/reference/webapi/SuperOffice.WebApi.Data.ContactAction)

Describes what type of activity/action there has been on a contact
This Enum consists of flag values that can be combined.

 [ContactPersonBoardEntity](/en/api/reference/webapi/SuperOffice.WebApi.Data.ContactPersonBoardEntity)

Contact/Person board view entity

 [ContactSourceType](/en/api/reference/webapi/SuperOffice.WebApi.Data.ContactSourceType)

Describes what source the contacts should be retrieved from.
This Enum consists of flag values that can be combined.

 [CopilotDataSourceType](/en/api/reference/webapi/SuperOffice.WebApi.Data.CopilotDataSourceType)

Copilot data source type
This Enum is used by:  copilot_data_source.copilot_data_source_type

 [CredentialControlType](/en/api/reference/webapi/SuperOffice.WebApi.Data.CredentialControlType)

Types of control used with user administration work with credentials

 [CrmActorType](/en/api/reference/webapi/SuperOffice.WebApi.Data.CrmActorType)

Actor type within the CRM, system, a subset of the SuperOffice Entity types
This Enum is used by:  ErpConnectionActorType.ActorTypeCrm

 [CtaFlavor](/en/api/reference/webapi/SuperOffice.WebApi.Data.CtaFlavor)

News feed CTA flavor
This Enum is used by:  NewsFeedItemCta.CtaFlavor

 [CtaType](/en/api/reference/webapi/SuperOffice.WebApi.Data.CtaType)

News feed CTA type
This Enum is used by:  NewsFeedItemCta.CtaType

 [CustomFieldType](/en/api/reference/webapi/SuperOffice.WebApi.Data.CustomFieldType)

Custom Field type: 1 = Number, 2 = Short text, 3 = Long text, 4 = Date, 5 = Unlimited date, 6 = Checkbox, 7 = Drop-down listbox, 8 = Decimal, 9 = DateTime, 10 = Time, 11 = TimeSpan, 12 = Relation, 13 = Attachment

 [DashTileCurrencyMode](/en/api/reference/webapi/SuperOffice.WebApi.Data.DashTileCurrencyMode)

Dashboard tile currency mode
This Enum is used by:  dashboard_tile_definition.currency_mode

 [DashTileEntityType](/en/api/reference/webapi/SuperOffice.WebApi.Data.DashTileEntityType)

Dashboard tile entity type (V2)
This Enum is used by:  dashboard_tile_definition.entity_type

 [DashTileMeasure](/en/api/reference/webapi/SuperOffice.WebApi.Data.DashTileMeasure)

Dashboard tile measure
This Enum is used by:  dashboard_tile_definition.measure

 [DashTileType](/en/api/reference/webapi/SuperOffice.WebApi.Data.DashTileType)

Dashboard tile chart type (V2)
This Enum is used by:  dashboard_tile_definition.tile_type

 [DashTileUsage](/en/api/reference/webapi/SuperOffice.WebApi.Data.DashTileUsage)

Defines where a DashTileDefinition can be used
This Enum is used by:  dashboard_tile_definition.usage
This Enum consists of flag values that can be combined.

 [DashboardLayout](/en/api/reference/webapi/SuperOffice.WebApi.Data.DashboardLayout)

Dashboard layout

 [DashboardTileEntityType](/en/api/reference/webapi/SuperOffice.WebApi.Data.DashboardTileEntityType)

Dashboard tile entity type

 [DashboardTileOptionType](/en/api/reference/webapi/SuperOffice.WebApi.Data.DashboardTileOptionType)

Dashboard tile option type

 [DashboardTileType](/en/api/reference/webapi/SuperOffice.WebApi.Data.DashboardTileType)

Dashboard tile type

 [DeltaState](/en/api/reference/webapi/SuperOffice.WebApi.Data.DeltaState)

State of delta
This Enum is used by:  ConfigurableScreenDelta.deltaState

 [DeltaType](/en/api/reference/webapi/SuperOffice.WebApi.Data.DeltaType)

Type and content of delta 
This Enum is used by:  ConfigurableScreenDelta.deltaType

 [DesignType](/en/api/reference/webapi/SuperOffice.WebApi.Data.DesignType)

Indicates what design type this message is created with
This Enum is used by:  s_message.designtype

 [DocTmplDirection](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocTmplDirection)

Value for field 'direction' in table 'doctmpl'.
This Enum is used by:  DocTmpl.direction

 [DocTmplInvitationType](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocTmplInvitationType)

Is this document template some kind of appointment document, and if so what
This Enum is used by:  DocTmpl.invitationDocType

 [DocTmplPrivacyType](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocTmplPrivacyType)

Is this document template some kind of quote document, and if so what
This Enum is used by:  DocTmpl.privacyDocType

 [DocTmplQuoteType](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocTmplQuoteType)

Is this document template some kind of quote document, and if so what
This Enum is used by:  DocTmpl.quoteDocType

 [DocTmplType](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocTmplType)

Value for field 'record_type' in table 'doctmpl'.
This Enum is used by:  DocTmpl.record_type

 [DocumentLockSemantics](/en/api/reference/webapi/SuperOffice.WebApi.Data.DocumentLockSemantics)

Locking semantics requested/applied to a document

 [Domain](/en/api/reference/webapi/SuperOffice.WebApi.Data.Domain)

Hierarchy domain

 [DurationUnit](/en/api/reference/webapi/SuperOffice.WebApi.Data.DurationUnit)

Units of duration - from seconds to centuries
This Enum is used by:  ProjType.durationUnit; SaleType.durationUnit

 [EFieldRight](/en/api/reference/webapi/SuperOffice.WebApi.Data.EFieldRight)

Access levels to a single field. Read and/or write.
This Enum consists of flag values that can be combined.

 [EMailFlags](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMailFlags)

This Enum is used by:  email_item.flags
This Enum consists of flag values that can be combined.

 [EMailPriority](/en/api/reference/webapi/SuperOffice.WebApi.Data.EMailPriority)

This Enum is used by:  email_item.priority; NewsFeedItem.Priority

 [ETableRight](/en/api/reference/webapi/SuperOffice.WebApi.Data.ETableRight)

Table right is a combination of bits representing permissions on a row.
This Enum consists of flag values that can be combined.

 [EjScriptVerb](/en/api/reference/webapi/SuperOffice.WebApi.Data.EjScriptVerb)

Flag values for field 'blocked_verbs' in table 'ejscript'.
This Enum is used by:  ejscript.blocked_verbs
This Enum consists of flag values that can be combined.

 [EjUserStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.EjUserStatus)

 [EmailFromType](/en/api/reference/webapi/SuperOffice.WebApi.Data.EmailFromType)

Email/Mailing From field address algorithm
This Enum is used by:  email_flow.from_type

 [EmailReplyToType](/en/api/reference/webapi/SuperOffice.WebApi.Data.EmailReplyToType)

Email/Mailing ReplyTo field address algorithm
This Enum is used by:  email_flow.reply_to_type

 [ErpActorType](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpActorType)

Actor type within the ERP system, related but not identitcal to SuperOffice Entity type
This Enum is used by:  ErpConnectionActorType.ActorTypeErp; ErpExternalKey.ActorType

 [ErpSyncResponseCode](/en/api/reference/webapi/SuperOffice.WebApi.Data.ErpSyncResponseCode)

Response codes used by ErpSync services

 [EventHandlerType](/en/api/reference/webapi/SuperOffice.WebApi.Data.EventHandlerType)

CrmScript Event triggers - when CRMScript is run based on user or system actions.

 [ExecuteOnEvent](/en/api/reference/webapi/SuperOffice.WebApi.Data.ExecuteOnEvent)

Value for the executeOnEvent field in table externalapplication
This Enum is used by:  ExtApp.executeOnEvent

 [ExternalUserInfoModification](/en/api/reference/webapi/SuperOffice.WebApi.Data.ExternalUserInfoModification)

Describes what part of the external user should be changed.
This Enum consists of flag values that can be combined.

 [ExternallyOwnedStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.ExternallyOwnedStatus)

The status for a participant of an externally owned appointment

 [ExternallyOwnedType](/en/api/reference/webapi/SuperOffice.WebApi.Data.ExternallyOwnedType)

Type of participant of an externally owned appointment

 [ExtraDataFieldType](/en/api/reference/webapi/SuperOffice.WebApi.Data.ExtraDataFieldType)

String, int, decimal, image, url, etc. How should the value be interpreted.

 [FieldAccess](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldAccess)

Access restrictions and mandatory status, if any.
This Enum is used by:  ErpField.Access

 [FieldMetadataType](/en/api/reference/webapi/SuperOffice.WebApi.Data.FieldMetadataType)

Describes the different types of controls that can appear in the Configure connection dialog
This Enum is used by:  ErpField.FieldType

 [FormFieldRestrictionType](/en/api/reference/webapi/SuperOffice.WebApi.Data.FormFieldRestrictionType)

Possible restriction for fields used in forms
This Enum is used by:  form_field_restriction.field_restriction

 [FormSubmissionStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.FormSubmissionStatus)

What is the status of this submission
This Enum is used by:  form_submission.status

 [FormType](/en/api/reference/webapi/SuperOffice.WebApi.Data.FormType)

What kind of form is this
This Enum is used by:  form.type

 [FormsRecaptchaMode](/en/api/reference/webapi/SuperOffice.WebApi.Data.FormsRecaptchaMode)

Forms recaptcha mode

 [FreeBusy](/en/api/reference/webapi/SuperOffice.WebApi.Data.FreeBusy)

What kind of time is this: 0 = Busy, 1 = Free
This Enum is used by:  appointment.freeBusy; Task.defaultFreeBusy

 [FreeTextOperator](/en/api/reference/webapi/SuperOffice.WebApi.Data.FreeTextOperator)

Freetext operator

 [GeneratorEncoding](/en/api/reference/webapi/SuperOffice.WebApi.Data.GeneratorEncoding)

 [HtmlSanitizerOptions](/en/api/reference/webapi/SuperOffice.WebApi.Data.HtmlSanitizerOptions)

HTML sanitizer options
This Enum consists of flag values that can be combined.

 [ImageEmbedType](/en/api/reference/webapi/SuperOffice.WebApi.Data.ImageEmbedType)

Embedded images type

 [ImportAction](/en/api/reference/webapi/SuperOffice.WebApi.Data.ImportAction)

Action being done for the import row
This Enum consists of flag values that can be combined.

 [ImportEntityType](/en/api/reference/webapi/SuperOffice.WebApi.Data.ImportEntityType)

Entitys type for import
This Enum consists of flag values that can be combined.

 [InterRestrictionOperator](/en/api/reference/webapi/SuperOffice.WebApi.Data.InterRestrictionOperator)

Operators to be used between restrictions, describes how this restriction is related to the next one in an array

 [InvitationStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.InvitationStatus)

Status if this appointment represents an invitation

 [KbAccessLevel](/en/api/reference/webapi/SuperOffice.WebApi.Data.KbAccessLevel)

Describes access level to get FAQs

 [LandingPageTabType](/en/api/reference/webapi/SuperOffice.WebApi.Data.LandingPageTabType)

What kind of content a landing page tab shows
This Enum is used by:  landing_page_tab.tab_type

 [LicenseStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.LicenseStatus)

Used in the CheckLicenseStatusResult to describe how SOADMIN should handle

 [LicenseType](/en/api/reference/webapi/SuperOffice.WebApi.Data.LicenseType)

Entire system, per database, or per associate
This Enum is used by:  ModuleLicense.licenseType

 [LocalizedTextType](/en/api/reference/webapi/SuperOffice.WebApi.Data.LocalizedTextType)

The type of the locale text strings

 [LogEvent](/en/api/reference/webapi/SuperOffice.WebApi.Data.LogEvent)

Log events
This Enum is used by:  target_revision.log_event

 [MailboxType](/en/api/reference/webapi/SuperOffice.WebApi.Data.MailboxType)

Service mailbox type
This Enum is used by:  mail_in_filter.server_type

 [MessageActionType](/en/api/reference/webapi/SuperOffice.WebApi.Data.MessageActionType)

Ticket message actions

 [MessageHeaderStdItem](/en/api/reference/webapi/SuperOffice.WebApi.Data.MessageHeaderStdItem)

Standard/predefined values for ticket message header

 [MessageHeaderStdItemCol](/en/api/reference/webapi/SuperOffice.WebApi.Data.MessageHeaderStdItemCol)

Ticket message header modification options

 [Navigation](/en/api/reference/webapi/SuperOffice.WebApi.Data.Navigation)

Value for the navigation field in table externalapplication
This Enum is used by:  ExtApp.navigation

 [NetServerBuildType](/en/api/reference/webapi/SuperOffice.WebApi.Data.NetServerBuildType)

 [NewsFeedItemStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.NewsFeedItemStatus)

News feed item status - indicates if the item is being handled by a CTA
This Enum is used by:  NewsFeedItem.Status

 [NotificationEventType](/en/api/reference/webapi/SuperOffice.WebApi.Data.NotificationEventType)

Types of events that are sent through the Pocket Notification API, where they are paired with an entity id

 [NotificationMessageType](/en/api/reference/webapi/SuperOffice.WebApi.Data.NotificationMessageType)

Defines what type of content the notification contains

 [NotificationPlatform](/en/api/reference/webapi/SuperOffice.WebApi.Data.NotificationPlatform)

Value for field 'DevicePlatform' in table 'PushNotificationService'
This Enum is used by:  PushNotificationService.DevicePlatform

 [NotifyChannel](/en/api/reference/webapi/SuperOffice.WebApi.Data.NotifyChannel)

The different channels to send notifications over

 [NotifyType](/en/api/reference/webapi/SuperOffice.WebApi.Data.NotifyType)

Value for field 'Type' in table 'notify'. Defines the notification type.
This Enum consists of flag values that can be combined.

 [OrderBySortType](/en/api/reference/webapi/SuperOffice.WebApi.Data.OrderBySortType)

How to sort the Order by statement.

 [PrefDescAccessFlags](/en/api/reference/webapi/SuperOffice.WebApi.Data.PrefDescAccessFlags)

Value for field 'accessflags' in table 'prefdesc'.
This Enum is used by:  PrefDesc.accessFlags
This Enum consists of flag values that can be combined.

 [PrefDescValueType](/en/api/reference/webapi/SuperOffice.WebApi.Data.PrefDescValueType)

Value for field 'valueType' in table 'prefdesc'.
This Enum is used by:  PrefDesc.valueType

 [PreferenceLevel](/en/api/reference/webapi/SuperOffice.WebApi.Data.PreferenceLevel)

Value for field 'deflevel' in table 'userpreference'.
This Enum is used by:  UserPreference.deflevel; UserPreference.maxlevel; PrefDesc.maxLevel; PrefDesc.sysMaxLevel

 [ProjectMemberEventParticipantStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.ProjectMemberEventParticipantStatus)

Enum for project member event participant status from Lyyti.
This Enum is used by:  projectmember.event_participant_status

 [PublishType](/en/api/reference/webapi/SuperOffice.WebApi.Data.PublishType)

Should this field be published by default?
This Enum is used by:  Task.defaultPublishType; DocTmpl.defaultPublishType; Publish.publishType

 [QuoteStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuoteStatus)

The state of a quote-alternative or line: Ok / OkWithInfo / Warn / Error
This Enum is used by:  QuoteVersion.Status; QuoteAlternative.Status; QuoteLine.Status

 [QuoteVersionButtonAction](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuoteVersionButtonAction)

Possible actions for QuoteVersionButton states.

 [QuoteVersionState](/en/api/reference/webapi/SuperOffice.WebApi.Data.QuoteVersionState)

State of a Quote Version
This Enum is used by:  QuoteVersion.State; QuoteVersion.ArchivedState

 [RagType](/en/api/reference/webapi/SuperOffice.WebApi.Data.RagType)

AI RAG Content types

 [RecipientType](/en/api/reference/webapi/SuperOffice.WebApi.Data.RecipientType)

Recipient type

 [RecurrenceDailyPattern](/en/api/reference/webapi/SuperOffice.WebApi.Data.RecurrenceDailyPattern)

Enumerator describing the different patterns for a daily recurrence

 [RecurrenceEndType](/en/api/reference/webapi/SuperOffice.WebApi.Data.RecurrenceEndType)

Enumerator describing how the series of recurring activities are terminated.

 [RecurrenceMonthlyPattern](/en/api/reference/webapi/SuperOffice.WebApi.Data.RecurrenceMonthlyPattern)

Enumerator describing the different patterns for a monthly recurrence

 [RecurrencePattern](/en/api/reference/webapi/SuperOffice.WebApi.Data.RecurrencePattern)

Enumerator describing the main pattern of recurrence.
This Enum is used by:  RecurrenceRule.pattern

 [RecurrenceUpdateMode](/en/api/reference/webapi/SuperOffice.WebApi.Data.RecurrenceUpdateMode)

Appointment recurrence change mode: only this, this and forward, stop

 [RecurrenceYearlyPattern](/en/api/reference/webapi/SuperOffice.WebApi.Data.RecurrenceYearlyPattern)

Enumerator describing the different patterns for a yearly recurrence

 [RelationTarget](/en/api/reference/webapi/SuperOffice.WebApi.Data.RelationTarget)

Target type of relation. (Contact, person or both)

 [ReportPaperOrientation](/en/api/reference/webapi/SuperOffice.WebApi.Data.ReportPaperOrientation)

Describes the orientation of the paper when printing a report. The report layout must be "Label"

 [ReturnType](/en/api/reference/webapi/SuperOffice.WebApi.Data.ReturnType)

Type of responce

 [RoleRelationToOwner](/en/api/reference/webapi/SuperOffice.WebApi.Data.RoleRelationToOwner)

Describes the available relation to owner types available for roles

 [RoleType](/en/api/reference/webapi/SuperOffice.WebApi.Data.RoleType)

0 = employee, 1 = external user, 2 = anonymous
This Enum is used by:  Role.roleType

 [SaleStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.SaleStatus)

Value for the 'status' field in the sale table
This Enum is used by:  sale.status; SaleHist.status; CounterValue.sale_status

 [ScriptType](/en/api/reference/webapi/SuperOffice.WebApi.Data.ScriptType)

Value for field 'Type' in table 'ej_script'. Indicates what type of script this is.
This Enum is used by:  ejscript.type

 [SelectionType](/en/api/reference/webapi/SuperOffice.WebApi.Data.SelectionType)

Selection type - static/dynamic/combined. Works together with targetTable to define what kind of Selection this is.

 [SelectionUnionType](/en/api/reference/webapi/SuperOffice.WebApi.Data.SelectionUnionType)

How the selections in a combined selections should be put together
This Enum is used by:  selection.combinationType

 [SenderMailMode](/en/api/reference/webapi/SuperOffice.WebApi.Data.SenderMailMode)

Sender email mode
This Enum is used by:  DocTmpl.senderEmailMode

 [ShipmentAddrStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.ShipmentAddrStatus)

Status for sending to a particular recipient
This Enum is used by:  s_shipment_addr.status

 [ShipmentMessageType](/en/api/reference/webapi/SuperOffice.WebApi.Data.ShipmentMessageType)

Bitmask defining what type of shipment message this is
This Enum is used by:  s_message.flags
This Enum consists of flag values that can be combined.

 [ShipmentStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.ShipmentStatus)

Status for shipment (started, cancelled etc)
This Enum is used by:  s_shipment.status

 [ShowState](/en/api/reference/webapi/SuperOffice.WebApi.Data.ShowState)

Value for the showState field in table externalapplication
This Enum is used by:  ExtApp.showState
This Enum consists of flag values that can be combined.

 [ShowTaskItemInClient](/en/api/reference/webapi/SuperOffice.WebApi.Data.ShowTaskItemInClient)

Show Task Item In Client
This Enum is used by:  TaskMenu.showInClient
This Enum consists of flag values that can be combined.

 [ShowWindowState](/en/api/reference/webapi/SuperOffice.WebApi.Data.ShowWindowState)

Normal = 0, Maximized = 1, Minimized = 2
This Enum is used by:  WinPosSize.state

 [StatusScreenPanelType](/en/api/reference/webapi/SuperOffice.WebApi.Data.StatusScreenPanelType)

 [StringSearchType](/en/api/reference/webapi/SuperOffice.WebApi.Data.StringSearchType)

Enumeration that exposes what search types that are supported.

 [SystemEventScope](/en/api/reference/webapi/SuperOffice.WebApi.Data.SystemEventScope)

Value for field 'scope' in table 'systemevent'.

 [SystemMessageType](/en/api/reference/webapi/SuperOffice.WebApi.Data.SystemMessageType)

System Message Type
This Enum is used by:  Message.type
This Enum consists of flag values that can be combined.

 [TargetAssignementLevel](/en/api/reference/webapi/SuperOffice.WebApi.Data.TargetAssignementLevel)

Target assignement levels
This Enum is used by:  target_dimension.assignment_level
This Enum consists of flag values that can be combined.

 [TargetEntityType](/en/api/reference/webapi/SuperOffice.WebApi.Data.TargetEntityType)

Target entity type
This Enum is used by:  target_group.entity_type; target_revision_history.entity_type; target_dimension.entity_type

 [TargetLevel](/en/api/reference/webapi/SuperOffice.WebApi.Data.TargetLevel)

Target level

 [TargetMeasurementUnit](/en/api/reference/webapi/SuperOffice.WebApi.Data.TargetMeasurementUnit)

Target measurement unit
This Enum is used by:  target_dimension.measurement_unit

 [TargetPeriodType](/en/api/reference/webapi/SuperOffice.WebApi.Data.TargetPeriodType)

Target period type
This Enum is used by:  target_group.period_type

 [TaskDirection](/en/api/reference/webapi/SuperOffice.WebApi.Data.TaskDirection)

Value for field 'direction' in table 'task'. Controls icons used in GUI
This Enum is used by:  Task.direction

 [TaskListItemType](/en/api/reference/webapi/SuperOffice.WebApi.Data.TaskListItemType)

Task List Item Type
This Enum is used by:  TaskMenu.taskType

 [TaskType](/en/api/reference/webapi/SuperOffice.WebApi.Data.TaskType)

Value for field 'record_type' in table 'task'. Controls icons used in GUI
This Enum is used by:  Task.record_type

 [TemporaryKeyDomain](/en/api/reference/webapi/SuperOffice.WebApi.Data.TemporaryKeyDomain)

Domains for temporary keys.
This Enum is used by:  TemporaryKey.domain

 [TenantState](/en/api/reference/webapi/SuperOffice.WebApi.Data.TenantState)

Is the Online Tenant running, or down for maintenance?

 [TicketAlertAction](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketAlertAction)

Bitmask available ticket alert action
This Enum consists of flag values that can be combined.

 [TicketBaseStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketBaseStatus)

Status of a ticket/request
This Enum is used by:  ticket.status; ticket_status.status

 [TicketCategoryClosingStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketCategoryClosingStatus)

Default status for requests. Note that this is a preference which should be respected if making UI. It is not enforced by the APIs

 [TicketCategoryDelegateMethod](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketCategoryDelegateMethod)

The delegate method used for request assigned to this category
This Enum is used by:  ej_category.delegate_method

 [TicketCategoryFlags](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketCategoryFlags)

Flags for the ticket category
This Enum is used by:  ej_category.flags
This Enum consists of flag values that can be combined.

 [TicketMessageCategory](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketMessageCategory)

Classification of ticket messages
This Enum is used by:  ej_message.message_category

 [TicketMessageType](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketMessageType)

Indicates if the field body contains plain or html text
This Enum is used by:  ej_message.type

 [TicketOrigin](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketOrigin)

Communication channel leading to ticket being created
This Enum is used by:  ticket.origin

 [TicketPriorityEscalateAction](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketPriorityEscalateAction)

The different types of escalate actions that can be set

 [TicketPriorityEscalateEvent](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketPriorityEscalateEvent)

The different types of escalate events that can be set
This Enum is used by:  ticket_priority.ticket_read; ticket_priority.changed_owner; ticket_priority.ticket_newinfo; ticket_priority.ticket_closed; ticket_priority.ticket_changed_priority; ticket_priority.ticket_new

 [TicketPriorityFlags](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketPriorityFlags)

Flags for the ticket priority
This Enum is used by:  ticket_priority.flags
This Enum consists of flag values that can be combined.

 [TicketPriorityStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketPriorityStatus)

Status of the ticket priority
This Enum is used by:  ticket_priority.status

 [TicketReadStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketReadStatus)

Whether the owner has read the ticket or not (red, yellow, green)
This Enum is used by:  ticket.read_status

 [TicketRelationError](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketRelationError)

Describes why a ticket relation is invalid.

 [TicketRelationType](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketRelationType)

Type of relation between tickets
This Enum is used by:  ticket_relation_def.relation_type

 [TicketSecurityLevel](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketSecurityLevel)

Indicates if a ticket is external or internal
This Enum is used by:  ej_message.slevel; ticket.slevel

 [TicketStatusTimeCounter](/en/api/reference/webapi/SuperOffice.WebApi.Data.TicketStatusTimeCounter)

Which field in ticket we count time spent on
This Enum is used by:  ticket_status.time_counter

 [UDefFieldType](/en/api/reference/webapi/SuperOffice.WebApi.Data.UDefFieldType)

Field type: 1 = Number, 2 = Short text, 3 = Long text, 4 = Date, 5 = Unlimited date, 6 = Checkbox, 7 = Drop-down listbox, 8 = Decimal

 [UDefType](/en/api/reference/webapi/SuperOffice.WebApi.Data.UDefType)

Same as the EUDefType enum in the C++ client, this is the DATABASE value used for udef definitions

 [UdefJustification](/en/api/reference/webapi/SuperOffice.WebApi.Data.UdefJustification)

Justification - 0 = default, left, right, center
This Enum is used by:  UDefField.justification

 [UpsertNomatchAction](/en/api/reference/webapi/SuperOffice.WebApi.Data.UpsertNomatchAction)

Upsert: Action to take on target table rows that do not match any incoming keys

 [UpsertRowActionStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.UpsertRowActionStatus)

Upsert: Action status for each row

 [UrlEncoding](/en/api/reference/webapi/SuperOffice.WebApi.Data.UrlEncoding)

Url encoding
This Enum is used by:  TaskMenu.encoding

 [UserType](/en/api/reference/webapi/SuperOffice.WebApi.Data.UserType)

Type of user (or resource). Mapps functionally to associate type - but with different binary values.

 [ValueOverride](/en/api/reference/webapi/SuperOffice.WebApi.Data.ValueOverride)

Has a pre-calculated value been overriden in a QuoteAlternative or QuoteLine, and in that case what &amp; how?
This Enum is used by:  QuoteAlternative.UserValueOverride; QuoteLine.UserValueOverride

 [VideoMeetingStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.VideoMeetingStatus)

Default video-meeting status for meetings created in SuperOffice CRM
This Enum is used by:  Task.default_videomeeting_status

 [Visibility](/en/api/reference/webapi/SuperOffice.WebApi.Data.Visibility)

The visibility of the record

 [WebhookNotifyVia](/en/api/reference/webapi/SuperOffice.WebApi.Data.WebhookNotifyVia)

How to notify recipients when a webhook changes state
This Enum is used by:  Webhook.notify_via
This Enum consists of flag values that can be combined.

 [WebhookNotifyWhen](/en/api/reference/webapi/SuperOffice.WebApi.Data.WebhookNotifyWhen)

How to notify recipients when a webhook changes state
This Enum is used by:  Webhook.notify_when
This Enum consists of flag values that can be combined.

 [WebhookState](/en/api/reference/webapi/SuperOffice.WebApi.Data.WebhookState)

Webhook status: active, stopped, or tooManyErrors
This Enum is used by:  Webhook.state

 [WeekOfMonth](/en/api/reference/webapi/SuperOffice.WebApi.Data.WeekOfMonth)

Enumerator representing a week of the month

 [Weekday](/en/api/reference/webapi/SuperOffice.WebApi.Data.Weekday)

Enumerator for the days of the week
This Enum is used by:  email_flow.selected_days
This Enum consists of flag values that can be combined.

 [WidgetRequiredFields](/en/api/reference/webapi/SuperOffice.WebApi.Data.WidgetRequiredFields)

Chat Topic required fields in pre-form.
This Enum is used by:  chat_topic.widget_offline_fields; chat_topic.widget_required_fields
This Enum consists of flag values that can be combined.

 [WidgetTheme](/en/api/reference/webapi/SuperOffice.WebApi.Data.WidgetTheme)

Chat Topic widget_theme: classic(0) or modern(1).
This Enum is used by:  chat_topic.widget_theme

 [WorkflowActionType](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowActionType)

Workflow action type

 [WorkflowContentType](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowContentType)

Workflow event type

 [WorkflowDefinitionStatus](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowDefinitionStatus)

Workflow definition status
This Enum is used by:  workflow.status

 [WorkflowEventType](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowEventType)

Workflow event type

 [WorkflowGoalType](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowGoalType)

Workflow Goal Type
This Enum is used by:  workflow_goal.goal_type

 [WorkflowNotifyEmailType](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowNotifyEmailType)

Workflow notify email type

 [WorkflowNotifySMSType](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowNotifySMSType)

Workflow notify sms type

 [WorkflowOwnerType](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowOwnerType)

Workflow owner type

 [WorkflowSplitOptionType](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowSplitOptionType)

Workflow Split Option type

 [WorkflowStepType](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowStepType)

Workflow Step Type
This Enum is used by:  workflow_step.step_type

 [WorkflowTimeWaitAlgorithm](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowTimeWaitAlgorithm)

Workflow waiting step wait algorithm

 [WorkflowTimeWaitIntervalType](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowTimeWaitIntervalType)

Workflow waiting time interval type

 [WorkflowTriggerType](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowTriggerType)

Workflow Trigger Type
This Enum is used by:  workflow_trigger.trigger_type

 [WorkflowWaitForActionType](/en/api/reference/webapi/SuperOffice.WebApi.Data.WorkflowWaitForActionType)

Workflow action type
This Enum is used by:  workflow_wait_for_action.ActionType

