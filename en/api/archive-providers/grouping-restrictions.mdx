...
The `Or` sits on the first `recipientStatus` element because that's the element _before_ the OR. The last element's `InterOperator` is never used — there is nothing after it to connect to.
