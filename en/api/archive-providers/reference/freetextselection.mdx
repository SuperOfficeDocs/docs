...
description: "Archive provider for freetext search on the Selection entity. . Use the 'searchwords' restriction with one or more words to specify what to search for. The operator is ignored, the freetext search will apply the 'begins' operator to each word and 'AND' between them. . It is possible to combine freetext search words with other restrictions and criteria native to the entity and its standard subproviders. Note however that this is not the same search as provided by the 'Find' providers, since they apply various special rules not present here. . Freetext search will limit the number of hits to the first 1000. This clipping is applied before any other criteria, as well as before any ORDER BY, are applied."
...
```http!
GET /api/v1/archive/FreetextSelection?$select=associateId,registeredBy,selectionAssociate/ejDisplayName,selectionAssociate/portraitThumbnail,visibleFor
