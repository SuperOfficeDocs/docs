...
This provider name is implemented by the class `SuperOffice.CRM.ArchiveLists.SaleTypeQuoteAttachmentProvider` inside NetServer's SODatabase assembly.
...
```http!
GET /api/v1/archive/SaleTypeQuoteAttachment?$select=defaultIncluded,document/associate/lastName,document/documentPublish/isPublished,document/projectId,saleTypeQuoteAttachmentId
