...
This provider name is implemented by the class `SuperOffice.CRM.ArchiveLists.SimpleDocumentsProvider` inside NetServer's SODatabase assembly.
...
```http!
GET /api/v1/archive/SimpleDocument?$select=associate/assocName,contact/restrictionAddress/zip,person/personCategoryGroup,project/projectPublish/publishedTo,yourref
