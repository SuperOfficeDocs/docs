---
title: "Forms"
description: "Archive provider for the list of forms (the form table). Backs the Forms pane's flat tabs — Recent (via `FormsHistoryExtender`'s historyRank), My forms (registeredBy) and Active forms (formActive) — and, as a sub-provider, the folder-aware `FormsAndFoldersProvider`. No restriction on form.type is applied here: the type is exposed as the filter-only `ColFormType` column so the Forms pane can ask for Normal forms and the Form-templates pane for Templates, both from this one provider."
...
This provider name is implemented by the class `SuperOffice.CRM.ArchiveLists.FormsProvider` inside NetServer's SODatabase assembly.

Archive provider for the list of forms (the form table). Backs the Forms
pane's flat tabs — Recent (via `FormsHistoryExtender`'s
historyRank), My forms (registeredBy) and Active forms (formActive) — and,
as a sub-provider, the folder-aware `FormsAndFoldersProvider`.

No restriction on form.type is applied here: the type is exposed as the
filter-only `ColFormType` column so the Forms
...
```http!
GET /api/v1/archive/Forms?$select=emailFlow/emailFlowId,emailFlow/thumbnail,emailFlow/workflowInProgressCount,formName,updatedDate
