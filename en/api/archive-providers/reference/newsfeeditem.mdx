---
title: "NewsFeedItem"
description: "Archive provider for the user's list of newsfeed items. Handled and expired items are filtered out of the list. . Name: \"NewsFeedItem\" . Entity name: \"newsfeeditem\""
...
|location|string|Location: Location| x |
...
|ratingOf5| *None* |Rating 0-5: Rating 0-5| x |
|ratingPercent|int|Rating %: Rating %| x |
...
|sourceSoProtocol|string|SuperOffice Protocol Source link: SOProtocol link to source within SuperOffice| x |
|status|listAny|Status: Newsfeed item processing status| x |
|summary|string|Summary: Short text| x |
|title|string|Title: Headline| x |
|type|string|Type: Item type| x |
...
```http!
GET /api/v1/archive/NewsFeedItem?$select=applicationId,location,person/personUrl/URLAddress,sale/completed,updatedDate
