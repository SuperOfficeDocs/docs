...
This provider name is implemented by the class `SuperOffice.CRM.ArchiveLists.RelatedTicketsProvider` inside NetServer's SODatabase assembly.
...
|relationType|listAny|Relation type: Relation type| x |
|sourceTicketId|int|| x |
|comment|string|Comment: Comment on the relation| x |

## Sample

```http!
GET /api/v1/archive/RelatedTickets?$select=ownedBy/usergroupId,ownedBy/simultaneousEjUser,person/restrictionAddress/county,contact/category,contact/contactUdef/SuperOffice:2
