...
This provider name is implemented by the class `SuperOffice.CRM.ArchiveLists.Archive.DocumentShadowSelectionDynamicProvider` inside NetServer's SODatabase assembly.
...
```http!
GET /api/v1/archive/DocumentShadowDynamicSelection?$select=associate/assocName,contact/restrictionAddress/zip,person/personCountry,project/projectUdef/SuperOffice:1,yourref
