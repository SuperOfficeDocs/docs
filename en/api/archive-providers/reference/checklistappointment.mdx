...
description: "Archive provider for Appointment activities, of the Diary type (not followup or booking). This provider can be used on its own, or can form part of the `ActivityArchiveProvider`, which mixes Appointments, appointments, tasks and documents."
...
`ActivityArchiveProvider`, which mixes Appointments, appointments, tasks and documents.
...
```http!
GET /api/v1/archive/ChecklistAppointment?$select=alarm,contact/postAddress/county,person/personAssociate/usergroup,project/projectId,who
