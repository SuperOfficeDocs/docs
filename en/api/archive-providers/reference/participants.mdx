...
This provider, like the `LinksProvider`, offers two alternative ways of accessing
...
* **dateInformation** - Dates of multiple appointments, override the date part of startTimeInformation and endTimeInformation. Needed when fetching data through the explicit restrictions (not appointmentRestrictionId) and if you want collision detection on multiple (such as recurring) occasions.
...
```http!
GET /api/v1/archive/Participants?$select=assocName,fullName,personContact/contactDeleted,personNoMail,whenUpdatedByWorkflow
