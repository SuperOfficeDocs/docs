...
description: Appointment selection archive with OR-able selection groups. Each group is represented with the AppointmentSelectionDynamicProviderSingleCriteriaGroup.
...
Appointment selection archive with OR-able selection groups. Each group is represented with the [AppointmentSelectionDynamicProviderSingleCriteriaGroup](./appointmentdynamicselectionsinglecriteriagroup).
...
```http!
GET /api/v1/archive/AppointmentDynamicSelectionV2?$select=contact/NumberOfActivities,contact/saintTicketCategory,person/personExtra/x_person_float,person/correspondingAssociate/locationAddress,project/updatedDate
