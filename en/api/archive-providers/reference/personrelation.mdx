...
description: "Data provider class for the 'Relation' archive under the person card. This class combines output from the eight basic sub-providers (CC, PC, PP and CP plus their reversed cousins) into one data stream."
...
```http!
GET /api/v1/archive/PersonRelation?$select=assocName,source/contactUdef/SuperOffice:2,source/supportAssociate,target/fullName,usergroupId
