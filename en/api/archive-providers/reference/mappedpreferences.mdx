...
description: "Get all the mapped ('fake') Service preferences known to the `ServicePreferenceMapper`. **NOTE** that only **section** and **key** are supported as restrictions, and only with the **equals** operator. Anything else needs to be coded!"
...
```http!
GET /api/v1/archive/MappedPreferences?$select=deflevel,maxlevel,ownerId,prefsection,value
