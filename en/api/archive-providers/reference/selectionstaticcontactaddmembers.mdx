...
just a facade for the [FindContactProvider](./findcontact), but a different name is required to avoid stepping
...
```http!
GET /api/v1/archive/SelectionStaticContactAddMembers?$select=business,personEmail/emailBounceCount,workflowInstance/emailFlow/workflowDescription,request/firstReadByUser,request/createdBy/locationAddress
