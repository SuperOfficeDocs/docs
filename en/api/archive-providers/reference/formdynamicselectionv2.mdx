...
Form selection archive with OR-able selection groups. Each group is represented with the `FormSelectionDynamicProviderSingleCriteriaGroup`.
...
```http!
GET /api/v1/archive/FormDynamicSelectionV2?$select=emailFlow/emailFlowId,emailFlow/thumbnail,emailFlow/workflowInProgressCount,formSubmissionCount,updatedDate
