...
description: Ticket selection archive with OR-able selection groups. Each group is represented with the TicketSelectionDynamicProviderSingleCriteriaGroup.
...
Ticket selection archive with OR-able selection groups. Each group is represented with the [TicketSelectionDynamicProviderSingleCriteriaGroup](./ticketdynamicselectionsinglecriteriagroup).
...
```http!
GET /api/v1/archive/TicketDynamicSelectionV2?$select=categoryFullName,ownedBy/simultaneousEjUser,person/mrMrs,person/personDeletedDate,person/restrictionAddress/zip
