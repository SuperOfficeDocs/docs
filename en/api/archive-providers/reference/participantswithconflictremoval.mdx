...
Archive provider that wraps the [ParticipantsProvider](./participants), and adds a pseudo entity used to hide associates with conflicts.
...
```http!
GET /api/v1/archive/ParticipantsWithConflictRemoval?$select=hasCompany,fullNameWithContact,ticketPriority
