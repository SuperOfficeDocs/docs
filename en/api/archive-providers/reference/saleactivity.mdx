...
This provider name is implemented by the class `SuperOffice.CRM.ArchiveLists.SaleActivityArchiveProvider` inside NetServer's SODatabase assembly.
...
```http!
GET /api/v1/archive/SaleActivity?$select=alarm,contact/postAddress/wgs84latitude,person/personAssociate/locationAddress,project/projectEvent/hasSignOn,yourref
