...
description: Sale selection archive with OR-able selection groups. Each group is represented with the SaleSelectionDynamicProviderSingleCriteriaGroup.
...
Sale selection archive with OR-able selection groups. Each group is represented with the [SaleSelectionDynamicProviderSingleCriteriaGroup](./saledynamicselectionsinglecriteriagroup).
...
```http!
GET /api/v1/archive/SaleDynamicSelectionV2?$select=originalStage,person/personRegisteredBy,person/personExtra/y_rental/x_amount,contact/postAddress/line3,contact/postAddress/formattedAddress
