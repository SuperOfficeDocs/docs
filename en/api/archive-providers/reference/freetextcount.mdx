...
the `FreetextContactResultProvider`.



The 'OwnerEntityName' column accepts a restriction in the form of an array of strings or table names. If
provided, this restriction will limit the entities searched. The `FreetextSearchEntityProvider`
...
```http!
GET /api/v1/archive/FreetextCount?$select=count,entityLocalizedName,ownerEntityName
