...
description: ChatConversation shadow selection archive with OR-able selection groups. Each group is represented with the ChatConversationShadowSelectionDynamicProviderSingleCriteriaGroup.
...
ChatConversation shadow selection archive with OR-able selection groups. Each group is represented with the [ChatConversationShadowSelectionDynamicProviderSingleCriteriaGroup](./chatconversationshadowdynamicselectionsinglecriteriagroup).
...
```http!
GET /api/v1/archive/ChatConversationShadowDynamicSelectionV2?$select=person/correspondingAssociate/ejStatus,contact/url/URLAddress,person/personAddress/formattedAddress
