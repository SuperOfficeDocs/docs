...
description: "Document selection archive with OR-able selection groups. Each group is represented with the `DocumentSelectionDynamicProviderSingleCriteriaGroup`."
...
This provider name is implemented by the class `SuperOffice.CRM.ArchiveLists.Archive.DocumentSelectionDynamicProviderV2` inside NetServer's SODatabase assembly.
...
```http!
GET /api/v1/archive/DocumentDynamicSelectionV2?$select=associate/assocName,contact/restrictionAddress/zip,person/personCountry,project/projectUdef/SuperOffice:1,yourref
