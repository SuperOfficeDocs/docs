...
This class instantiates a [DiaryAppointmentProvider](./diaryappointment) and a [ChecklistAppointmentProvider](./checklistappointment) as
subchannels, each split by a DoneNotDoneSplitter to get appointments that are not done, or done today
...
```http!
GET /api/v1/archive/Appointment?$select=contact/companyCategoryRank,person/personAssociate/credentialDisplayValue,person/correspondingAssociate/simultaneousEjUser,project/projectAssociate/contactDepartment,associate/usergroupId
