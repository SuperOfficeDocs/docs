...
This provider name is implemented by the class <see cref="T:SuperOffice.CRM.ArchiveLists.Archive.ChatConversationSelectionDynamicProviderSingleCriteriaGroup">SuperOffice.CRM.ArchiveLists.Archive.ChatConversationSelectionDynamicProviderSingleCriteriaGroup</see> inside NetServer's SODatabase assembly.
...
```http!
GET /api/v1/archive/ChatConversationDynamicSelectionSingleCriteriaGroup?$select=associate/assocTooltip,person/personAssociate/personEmail,person/correspondingAssociate/title,contact/updatedDate,contact/contactExtra/x_contact_float
