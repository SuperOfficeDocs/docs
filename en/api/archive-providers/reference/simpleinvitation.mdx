...
A simple invitation is one that is not recurring and has the status Booking.
...
```http!
GET /api/v1/archive/SimpleInvitation?$select=contact/deletedDate,contact/email/emailLastSent,contact/streetAddress/state,person/correspondingAssociate/contactCategory
