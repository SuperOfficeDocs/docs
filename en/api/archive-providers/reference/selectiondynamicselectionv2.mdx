...
description: Selection selection archive with OR-able selection groups. Each group is represented with the SelectionSelectionDynamicProviderSingleCriteriaGroup.
...
Selection selection archive with OR-able selection groups. Each group is represented with the [SelectionSelectionDynamicProviderSingleCriteriaGroup](./selectiondynamicselectionsinglecriteriagroup).
...
```http!
GET /api/v1/archive/SelectionDynamicSelectionV2?$select=updatedByFullName,lastLoadedByFullName,selectionAssociate/contactId
