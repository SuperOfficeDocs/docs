---
title: "Invitation"
description: "Invitation provider multichannel combiner, consisting of simple, cancelled, rejected, repeating and response invitations. You can also call up appointments explicitly through the Explicit provider."
...
`SimpleInvitationProvider``ResponseInvitationProvider``CancelledInvitationProvider``RepeatingInvitationProvider``RejectedInvitationProvider``ExplicitInvitationProvider`
...
```http!
GET /api/v1/archive/Invitation?$select=alarm,contact/postAddress/county,person/personAssociate/userName,project/projectId,who
