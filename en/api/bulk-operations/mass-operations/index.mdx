...
|-----------|--------------------|---------------------------------|
| [Delete](/en/api/bulk-operations/mass-operations/delete)    | Delete multiple rows by primary key | More efficient than deleting rows one by one but less efficient than truncate. |
| [Insert](/en/api/bulk-operations/mass-operations/insert)    | Add new rows.      | No primary key needs to be specified, all rows are simply added. Any collision with unique indexes causes an exception. |
| [Truncate](/en/api/bulk-operations/mass-operations/truncate)  | Delete all rows in a table. | Unconditional, non-recoverable truncation of the table. Very fast, near-instant. Resets primary key value to 1. |
| [Upsert](/en/api/bulk-operations/mass-operations/upsert)    | Add or update rows by key | Key column(s) designate target rows. Rows that have *no* key match are treated as **insert** rows. **No support for Oracle**. |
...
[1]: /en/api/authentication/online/auth-application/index
[2]: /en/database/index
[3]: /en/database/tables/index
[4]: /en/api/bulk-operations/mass-operations/protected-tables
[5]: /en/api/localization/culture/culturedataformatter
[6]: /en/automation/webhook/dev/index
