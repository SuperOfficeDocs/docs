...
|-----------|--------------------|---------------------------------|
| [Delete](./delete)    | Delete multiple rows by primary key | More efficient than deleting rows one by one but less efficient than truncate. |
| [Insert](./insert)    | Add new rows.      | No primary key needs to be specified, all rows are simply added. Any collision with unique indexes causes an exception. |
| [Truncate](./truncate)  | Delete all rows in a table. | Unconditional, non-recoverable truncation of the table. Very fast, near-instant. Resets primary key value to 1. |
| [Upsert](./upsert)    | Add or update rows by key | Key column(s) designate target rows. Rows that have *no* key match are treated as **insert** rows. **No support for Oracle**. |
...
[2]: ../../../database/index
[3]: ../../../database/tables/index
[4]: ./protected-tables
[5]: ../../localization/culture/culturedataformatter
[6]: ../../../automation/webhook/dev/index
