...
passed in as the extraInfo parameter, and is generally one of the constants in `ColumnTypes`.
The `RestrictionOperations` class is the source of the data, so this list will change if that class is extended.

Implemented by the `SuperOffice.CRM.ArchiveLists.OperatorProvider` class.
