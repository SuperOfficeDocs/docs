...
The `RelationType` enum can be used to avoid strings when specifying the relation type.



The extraInfo field in the items contains the passive (reverse) text, useful for display.

Implemented by the `SuperOffice.CRM.Lists.RelationListProvider` class.
