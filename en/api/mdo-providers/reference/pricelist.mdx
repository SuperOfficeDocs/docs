...
description: List of pricelists from the PriceList table. Supported values in the AdditionalInfo - all - true|false. If true is specified, inactive items is also returned. QuoteConnectionId - Gets items for a specific Quote connection. If not specified, only SuperOffice pricelists (where IsErpCopy=false) will be returned. Example AdditionalInfo string - "all=true&QuoteConnectionId=2"
generated: true
...
* **all**: true|false. If true is specified, inactive items is also returned.
* **QuoteConnectionId**: Gets items for a specific Quote connection. If not specified, only SuperOffice pricelists (where IsErpCopy=false) will be returned.

Example `AdditionalInfo` string: `"all=true&QuoteConnectionId=2"`
