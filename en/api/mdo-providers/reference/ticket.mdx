...
Sentry rights are honoured; if you do not have Read access the ticket will not be listed here. The one exception is an explicitly
requested history id (resolve-by-id of a stored value), which is listed as a masked "&lt;id&gt; - Access is denied" placeholder.

Implemented by the `SuperOffice.CRM.Lists.TicketListProvider` class.
