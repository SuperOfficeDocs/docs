...
recently visited entities written by the ViewState agent's `UpsertRecentNavigation` operation.



Items are returned most-recent first (descending `navigation_history.rank`). Keys of the form
`&lt;table&gt;|&lt;pkfield&gt;=&lt;id&gt;` (e.g. `person|person_id=123`) are resolved to display
names for known tables (currently contact, person, dashboard, email_flow, selection, project, sale,
ticket and the virtual mailitem, ticketselection, saleselection and projectselection); anything
else falls back to the raw key as name.
...
FIRST return field's table (`SoDataReader` checks `RF` on it), so every fetcher below puts
the entity's own primary key first. Reordering `ReturnFields` would move the sentry onto a
...
icon carries information of its own (the company's country flag §4.2) also returns an
`IconHint` that replaces the generic entity icon.

Implemented by the `SuperOffice.CRM.Lists.RecentNavigationListProvider` class.
