...
description: "This SoList provider generates the appointment/task type list for the Saint criteria. This is a simple, hard-coded list corresponding to the type field of the `DocTmpl` and `Task` tables. Tasks are both appointments and documents."
generated: true
...
to the type field of the `DocTmpl` and `Task` tables. Tasks are both
appointments and documents.

Implemented by the `SuperOffice.CRM.ArchiveLists.SaintActivityTypeProvider` class.
