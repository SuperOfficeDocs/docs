...
description: "Provides the same baseline functionality as `AllPersonListSearchProvider`, but additionally adds the associateId of the person to the extrainfo and changes the iconhint if the person is an associate. Uses freetext to perform a speedy search on the contact and person tables. Does not include sub-tables, like userdefined fields, text records etc."
generated: true
content_type: reference
envir: online
---

Provides the same baseline functionality as `AllPersonListSearchProvider`, but additionally adds the associateId of the person to the extrainfo
...
Implemented by the `SuperOffice.CRM.Lists.AllPersonWithAssociateListSearchProvider` class.
