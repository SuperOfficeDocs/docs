...
description: "Dynamic MDO provider that will create a list with the name udlist&lt;id&gt; for each user-defined list in the UDListDefinition table. The actual fetching is done by an inner `UdListMDOProvider` for whatever list is implicitly specified by the id suffix of the dynamic list name"
generated: true
...
The actual fetching is done by an inner `UdListMDOProvider` for whatever list is implicitly specified by the
id suffix of the dynamic list name

Implemented by the `SuperOffice.CRM.Lists.UdListIndividualProvider` class.
