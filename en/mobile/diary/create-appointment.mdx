...
keywords: ['new appointment', 'add appointment', 'New based on existing', 'create meeting', 'diary', 'repeat', 'recurring']
author: digitaldiina
date: 09.18.2026
so_version: 11.1.17
content_type: howto
...
related:
  - /en/mobile/diary/update-appointment
  - Recurrence in SuperOffice CRM: /en/diary/learn/recurrence/create
language: en
---
...
## Set a repeating follow-up {#repeat}

You can turn a follow-up into a recurring series without leaving Mobile CRM, the same way recurring follow-ups already work in SuperOffice CRM.

1. In the follow-up form, tap the **Repeat** field, directly below **Starts** and **Ends**. It's set to **Never** by default.
1. Select a frequency: **Every day**, **Every workday** (Monday to Friday only), **Every week**, **Every month**, or **Every year**.
1. Set an **End repeat** date or a **Number of repeats**. Whichever one you set, the other is calculated for you, so the series always has a defined end.
1. Tap **Save** to create the series.

![Follow-up screen showing the Repeat field with frequency options -app-screen][img23]

<Note>
Mobile CRM offers a fixed set of frequencies rather than SuperOffice CRM's full custom pattern builder, and every series must be bounded by an end date or a number of repeats.
</Note>

To turn recurrence off again, set **Repeat** back to **Never**. The **End repeat** and **Number of repeats** fields clear and disappear.
