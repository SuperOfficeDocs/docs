---
uid: crmscript.netserver.nscredentialtype
title: NSCredentialType
description: "Description of credential type"
keywords: ['CRMScript', 'NSCredentialType', 'API reference']
author: SuperOffice Product and Engineering
date: 01.08.2026
so_version: 11.7
generated: true
content_type: reference
language: en
---

Description of credential type

## Constructors

### NSCredentialType()

```crmscript
NSCredentialType
```

Initializes a new instance of the NSCredentialType class.

## Methods

## GetCanCreatePerson()

```crmscript
Bool GetCanCreatePerson()
```

**Returns:** [CRMScript.Global.Bool](/en/automation/crmscript/reference/CRMScript.Global.Bool)

**Example:**

```crmscript
NSCredentialType thing;
Bool canCreatePerson = thing.GetCanCreatePerson();
```

## GetDescription()

```crmscript
String GetDescription()
```

**Returns:** [CRMScript.Global.String](/en/automation/crmscript/reference/CRMScript.Global.String) - Description of the authentication type.

<Note>
This value is used in a list of authentication providers that can be chosen by an end user and should be a fairly descriptive text. This will typically be SuperOffice is responsible for username and password for a password scenario.
</Note>

**Example:**

```crmscript
NSCredentialType thing;
String description = thing.GetDescription();
```

## GetDisplayType()

```crmscript
String GetDisplayType()
```

**Returns:** [CRMScript.Global.String](/en/automation/crmscript/reference/CRMScript.Global.String) - The name of the credentials to be displayed in the user interface.

<Note>
This will typically be Password for user-name/password scenarios and Active Directory User for AD integration.
</Note>

**Example:**

```crmscript
NSCredentialType thing;
String displayType = thing.GetDisplayType();
```

## GetIsUserNameSupported()

```crmscript
Bool GetIsUserNameSupported()
```

**Returns:** [CRMScript.Global.Bool](/en/automation/crmscript/reference/CRMScript.Global.Bool)

**Example:**

```crmscript
NSCredentialType thing;
Bool isUserNameSupported = thing.GetIsUserNameSupported();
```

## GetType()

```crmscript
String GetType()
```

**Returns:** [CRMScript.Global.String](/en/automation/crmscript/reference/CRMScript.Global.String) - Type of credentials, corresponding to name of plugin and type in the credentials table.

**Example:**

```crmscript
NSCredentialType thing;
String type = thing.GetType();
```

## GetValueControl()

```crmscript
Integer GetValueControl()
```

**Returns:** [CRMScript.Global.Integer](/en/automation/crmscript/reference/CRMScript.Global.Integer) - Type of control used to display the value. See &lt;xref href="CRMScript.NetServer.CredentialControlType" data-throw-if-not-resolved="false"&gt;&lt;/xref&gt;.

**Example:**

```crmscript
NSCredentialType thing;
Integer valueControl = thing.GetValueControl();
```

## SetCanCreatePerson(Bool)

```crmscript
Void SetCanCreatePerson(Bool canCreatePerson)
```

**Returns:** [CRMScript.Global.Void](/en/automation/crmscript/reference/CRMScript.Global.Void)

**Example:**

```crmscript
NSCredentialType thing;
Bool canCreatePerson;
thing.SetCanCreatePerson(canCreatePerson);
```

## SetDescription(String)

```crmscript
Void SetDescription(String description)
```

**Returns:** [CRMScript.Global.Void](/en/automation/crmscript/reference/CRMScript.Global.Void)

<Note>
This value is used in a list of authentication providers that can be chosen by an end user and should be a fairly descriptive text. This will typically be SuperOffice is responsible for username and password for a password scenario.
</Note>

**Example:**

```crmscript
NSCredentialType thing;
String description;
thing.SetDescription(description);
```

## SetDisplayType(String)

```crmscript
Void SetDisplayType(String displayType)
```

**Returns:** [CRMScript.Global.Void](/en/automation/crmscript/reference/CRMScript.Global.Void)

<Note>
This will typically be Password for user-name/password scenarios and Active Directory User for AD integration.
</Note>

**Example:**

```crmscript
NSCredentialType thing;
String displayType;
thing.SetDisplayType(displayType);
```

## SetIsUserNameSupported(Bool)

```crmscript
Void SetIsUserNameSupported(Bool isUserNameSupported)
```

**Returns:** [CRMScript.Global.Void](/en/automation/crmscript/reference/CRMScript.Global.Void)

**Example:**

```crmscript
NSCredentialType thing;
Bool isUserNameSupported;
thing.SetIsUserNameSupported(isUserNameSupported);
```

## SetType(String)

```crmscript
Void SetType(String type)
```

**Returns:** [CRMScript.Global.Void](/en/automation/crmscript/reference/CRMScript.Global.Void)

**Example:**

```crmscript
NSCredentialType thing;
String type;
thing.SetType(type);
```

## SetValueControl(Integer)

```crmscript
Void SetValueControl(Integer valueControl)
```

**Returns:** [CRMScript.Global.Void](/en/automation/crmscript/reference/CRMScript.Global.Void)

**Example:**

```crmscript
NSCredentialType thing;
Integer valueControl;
thing.SetValueControl(valueControl);
```
