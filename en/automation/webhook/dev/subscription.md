...
keywords: ['automation', 'NetServer']
author: AnthonyYates
date: 04.06.2018
content_type: howto
audience: api
audience_tooltip: SuperOffice APIs and database
redirect_from: /en/automation/webhook/subscription
language: en
---
...
1. `Save the webhook` using the POST `api/v1/Webhook` URL.

1. `Get the webhook ID` from the response.

1. `Create a new webhook subscription`, same as #1, but this time use the webhook ID to set the webhook state events.

1. `Save the webhook subscription` using the POST `api/v1/Webhook` URL.
...
The [HTTP RPC Agents API][2] contains all available `WebhookAgent` Methods.
