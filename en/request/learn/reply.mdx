...
related:
  - Forward request: /en/request/learn/forward
  - Close request: /en/request/learn/close
  - /en/request/learn/flag-message
  - Create reply template: /en/knowledge-base/learn/reply-templates/index
  - /en/knowledge-base/learn/reply-templates/new-language
  - Edit email signature: /en/learn/getting-started/edit-email-signature
language: en
---
...
## Quick reply from a preview {#quick-reply}

When you're working through a busy queue, you don't have to open a request in full just to answer it. Whenever you preview a request, from a request list or from a linked request list shown alongside a company, contact, or sale record, use **Quick reply** to write and send your answer without leaving the list.

![Request list with preview gives a quick reply options -screenshot][img8]

Selecting **Quick reply** opens a compact reply editor in place. A reply sent this way registers on the request exactly as one sent from the full request view does, including status updates, and the list refreshes automatically so you can move straight on to the next request.

<Tip>
If you cancel or select another request while your quick reply has unsent text, you're asked before it's discarded.
</Tip>

**Quick reply** isn't offered on requests marked as spam, deleted, or merged.

## Insert reply templates {#reply-templ}
...
## Insert earlier message {#insert-earlier-msg}
