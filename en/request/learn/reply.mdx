...
Answer a request directly from its preview, without opening it in full.

1. Preview a request from a request list, or from a linked request list shown alongside a company, contact, or sale record.
1. Click **Quick reply**.
1. Type your reply in the compact editor.
1. Click **Save** to send it.

    ![Request list with preview gives a quick reply options -screenshot][img8]

The reply registers on the request the same way a reply sent from the full request view does, including status updates, and the list refreshes automatically.

<Tip>
If you cancel or select another request while your quick reply has unsent text, you are asked before it is discarded.
</Tip>

**Quick reply** is not offered on requests marked as spam, deleted, or merged.
