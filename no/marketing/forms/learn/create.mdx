...
related:
  - /no/marketing/forms/learn/publish
language: no
---
...
## Opprette skjema med flere sider {#multi-page}
...
8. Du kan også redigere disse innstillingene (valgfritt):
    ...
    * Hvis felttypen støtter det, kan du klikke på **Legg til relatert felt** på et av alternativene eller verdiene for å legge til et betinget felt. Se [Legge til et betinget felt](#conditional) nedenfor.
...
### Legge til et betinget felt {#conditional}

Vis besøkende bare de spørsmålene som er relevante for dem. Et **betinget felt** (også kalt et relatert felt) viser et oppfølgingsspørsmål bare når et tidligere svar gjør det relevant, slik at hver besøkende bare ser det som er viktig for dem, samtidig som skjemaet fortsatt samler inn opplysningene som trengs fra alle andre. Feltet **Contact - Country** kan for eksempel vise feltet **Contact - Preferred language** bare når et bestemt land er valgt.

Dette fungerer på nedtrekksmenyer (inkludert standard listefelt), alternativknapper, avmerkingsbokser, vurderingsfelt (stjerner, smilefjes, terninger eller tomler), ekstrafelt av typen nedtrekksmeny eller boolsk, og interessefelt. Felttyper som støtter dette, får merket **Betinget** når du legger til et nytt element, slik at du ikke trenger å legge til feltet først for å finne det ut.

![Felt som støtter betingede felt, får merket Betinget i vinduet Legg til element -screenshot][img18]

1. Velg alternativet eller verdien på et felt som støtter dette, som skal utløse oppfølgingsspørsmålet.
1. Klikk på **Legg til relatert felt**.
1. [Legg til og konfigurer det nye feltet](#fields) på samme måte som ethvert annet felt.

Det overordnede feltet får nå merket **Har relaterte felter**, og det relaterte feltet vises rykket inn under det, med et banner som forklarer betingelsen i vanlig språk (for eksempel "Dette feltet vises når **Contact - Country** er **Sveits**"). Bruk redigeringsikonet på banneret for å endre betingelsen, eller fjern-ikonet for å slette det relaterte feltet. Hvis du velger flere utløsende verdier, kobler banneret dem sammen med "eller". Vurderingsfelt viser i stedet et intervall.

![Et felt med et relatert felt og betingelsesbanneret -screenshot][img19]

Du kan kjede relaterte felt flere nivåer dypt (et spørsmål kan utløse et oppfølgingsspørsmål, som igjen kan utløse et enda dypere oppfølgingsspørsmål), men hvert relaterte felt styres av nøyaktig én betingelse på ett felt.

<Note>
Et relatert felt er skjult som standard for den besøkende, vises umiddelbart når det utløsende svaret gis, og forsvinner og tømmes hvis svaret endres. Et skjult felt er aldri påkrevd, selv om det er merket som obligatorisk, og skjulte svar tas ikke med i innsendingen.
</Note>
...
#### Egendefinert utseende {#custom-style}
