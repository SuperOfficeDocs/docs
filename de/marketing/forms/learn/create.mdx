...
related:
  - /de/marketing/forms/learn/publish
language: de
---
...
## Mehrseitige Formulare erstellen {#multi-page}
...
## Formularfelder ausfüllen {#fields}
...
8. Sie können diese Einstellungen auch bearbeiten (optional):
    ...
    * Wenn der Feldtyp dies unterstützt, klicken Sie bei einer seiner Optionen oder Werte auf **Verwandtes Feld hinzufügen**, um ein bedingtes Feld hinzuzufügen. Siehe [Bedingtes Feld hinzufügen](#conditional) weiter unten.
...
### Bedingtes Feld hinzufügen {#conditional}

Zeigen Sie Besuchern nur die Fragen, die für sie relevant sind. Ein **bedingtes Feld** (auch verwandtes Feld genannt) zeigt eine Folgefrage nur an, wenn eine vorherige Antwort sie relevant macht, sodass jeder Besucher nur das sieht, was für ihn wichtig ist, während das Formular weiterhin die von allen anderen benötigten Angaben erfasst. Ein Feld **Contact - Country** kann beispielsweise ein Feld **Contact - Preferred language** nur anzeigen, wenn ein bestimmtes Land ausgewählt ist.

Dies funktioniert bei Dropdown-Feldern (einschließlich Standardlistenfeldern), Optionsfeldern, Kontrollkästchen, Bewertungsfeldern (Sterne, Smileys, Würfel oder Daumen), zusätzlichen Feldern vom Typ Dropdown oder Boolean sowie Interessenfeldern. Feldtypen, die dies unterstützen, erhalten beim Hinzufügen eines neuen Elements ein **Bedingt**-Abzeichen, sodass Sie das Feld nicht erst hinzufügen müssen, um das herauszufinden.

![Geeignete Felder erhalten im Fenster "Element hinzufügen" ein Bedingt-Abzeichen -screenshot][img18]

1. Wählen Sie bei einem geeigneten Feld die Option oder den Wert aus, der die Folgefrage auslösen soll.
1. Klicken Sie auf **Verwandtes Feld hinzufügen**.
1. [Fügen Sie das neue Feld hinzu und konfigurieren Sie es](#fields) wie jedes andere Feld.

Das übergeordnete Feld erhält nun das Abzeichen **Hat verwandte Felder**, und das verwandte Feld wird eingerückt darunter angezeigt, mit einem Banner, das die Bedingung in einfacher Sprache erklärt (zum Beispiel "Dieses Feld wird angezeigt, wenn **Contact - Country** gleich **Schweiz** ist"). Verwenden Sie das Bearbeiten-Symbol des Banners, um die Bedingung zu ändern, oder das Entfernen-Symbol, um das verwandte Feld zu löschen. Wenn Sie mehrere auslösende Werte auswählen, verknüpft das Banner sie mit "oder". Bewertungsfelder zeigen stattdessen einen Bereich anstelle eines einzelnen Werts an.

![Ein Feld mit einem verwandten Feld und seinem Bedingungsbanner -screenshot][img19]

Sie können verwandte Felder mehrere Ebenen tief verketten (eine Frage kann eine Folgefrage auslösen, die wiederum eine weitere Folgefrage auslösen kann), wobei jedes verwandte Feld durch genau eine Bedingung auf einem Feld gesteuert wird.

<Note>
Ein verwandtes Feld ist für den Besucher standardmäßig ausgeblendet, erscheint sofort bei der auslösenden Antwort und wird ausgeblendet und geleert, wenn sich die Antwort ändert. Ein ausgeblendetes Feld ist nie erforderlich, selbst wenn es als Pflichtfeld markiert ist, und ausgeblendete Antworten werden nicht in die Übermittlung aufgenommen.
</Note>
...
#### Benutzerdefinierte Designs {#custom-style}
