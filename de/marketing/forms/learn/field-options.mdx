...
* **Benutzerdefiniertes Kontrollkästchen:** Ein boolesches Zusatzfeld (nur Ja/Nein), das beim Laden des Formulars aktiviert oder deaktiviert starten kann. Die Option **Standardmäßig aktiviert** finden Sie in den Feldeinstellungen — sie ist standardmäßig deaktiviert. Wenn dem Feld über eine Bedingung [ein verwandtes Feld][6] zugeordnet ist, wird der Standardstatus so behandelt, als hätte der Besucher ihn selbst festgelegt, sodass das verwandte Feld beim Laden entsprechend angezeigt wird oder ausgeblendet bleibt.
...
* **Bedingte (verwandte) Felder:** Optionsfelder, Kontrollkästchen, Dropdown-Felder und Bewertungsfelder können [ein verwandtes Feld auslösen][6], das nur angezeigt wird, wenn eine bestimmte Option oder ein bestimmter Wert ausgewählt wird.
