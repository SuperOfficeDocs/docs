---
uid: gmail-link-install
title: Enable SuperOffice Gmail Link
description: Learn how to install the browser extension for Gmail in this how-to guide.
keywords: ['enable Gmail Link', 'add Gmail Link', 'install Gmail Link']
author: SuperOffice Product and Engineering
date: 02.14.2023
content_type: howto
category: integration
topic: Gmail Link
audience: person
audience_tooltip: SuperOffice CRM
related:
  - /integrations/gmail-link/settings
  - /en/learn/getting-started/connect-your-accounts
language: en
redirect_from:
  - /en/email/gmail-link/learn/install
  - /da/email/gmail-link/learn/install
  - /de/email/gmail-link/learn/install
  - /nl/email/gmail-link/learn/install
  - /no/email/gmail-link/learn/install
  - /sv/email/gmail-link/learn/install
userflow_index: true
---

<Frame caption="Watch this video to learn how to set up your Gmail Link and connect your Gmail account to SuperOffice (video length - 2:53) or follow the steps below">
  <iframe
    width="100%"
    height="420"
    src="https://www.youtube-nocookie.com/embed/kiQyO9KwiEs"
    title="Watch this video to learn how to set up your Gmail Link and connect your Gmail account to SuperOffice (video length - 2:53) or follow the steps below"
    frameborder="0"
    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share"
    referrerpolicy="strict-origin-when-cross-origin"
    allowfullscreen
  ></iframe>
</Frame>

## Steps

1. Go to the app entry for Gmail Link in the [SuperOffice App Store][1].

1. Click **Install** and choose which browser you want to install the extension for.

    **Chrome:** If you choose Chrome, you will be taken to the Chrome Web Store. There, you click the **Add to Chrome** button and confirm that you want to add this extension.
    * **Firefox**: If you choose Firefox, the Firefox browser installer will open and ask you to allow the browser plugin for Firefox. Confirm by clicking **Continue to installation**.

1. Log in to your SuperOffice CRM account. SuperOffice and Gmail will automatically detect that the extension was added.

[1]: http://online.superoffice.com/appstore/superoffice-as/superoffice-gmail-link
