...
1. Update the release-notes pointer in *docs.json*'s `markdown.instructions` to the new major version.
...
Put the feature heading text first, the module `<Badge>` after it, so the heading text gets visual focus instead of being pushed right.
...
```mdx
### Feature title <Badge icon="gear" color="orange">Settings and maintenance</Badge> {#12.1-admin-1}
```
