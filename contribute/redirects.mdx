...
<Note>
**Don't self-redirect a `snippets/` file.** Files under a `snippets/` folder are MDX-imported into other pages, not standalone routable pages themselves - Mintlify never renders them at their own URL, so a `.html` self-redirect for one has no real destination to land on. A first pass at the "current pages" self-redirect backfill (#341) mistakenly included these before the mistake was caught and reverted (528 entries).
</Note>
...
**Why this exists ([#424][6]):** `.../index.html` requests only resolved correctly for `en`, our default language - every other language 404'd. Originally suspected as a #341 regression, it wasn't; the real cause, confirmed by Mintlify support, is a caching side effect - `en` pages get visited often enough to stay cached, and a cached page quietly gets served for the `.html` form too, while less-visited languages fall through to the origin and 404. Mintlify's platform has no general "strip an arbitrary `.html` suffix" rule (see [External destinations need `.html`][1] above and the `.html`-suffix-duplicates section) - but every affected URL here ends in `/index.html`, so this one rule, added first, covers all of them.
...
[6]: https://github.com/SuperOfficeDocs/docs/issues/424
