...
keywords: ['redirect_from', 'redirects', 'config/redirects.json', 'docs.json', 'wildcard', 'permanent redirect', 'moved page', 'deleted page', 'URL migration', '.html suffix']
author: digitaldiina
date: 09.08.2026
...
### External destinations need `.html` {#external-destinations-need-html}
...
### Every current source needs a `.html` duplicate {#html-suffix-duplicates}

The old DocFx site served every page with a literal `.html` extension. Mintlify's redirect matcher treats `.html` as part of the literal source string, not something it strips - a source written as `/foo/bar` never matches a request for `/foo/bar.html`.

Any page reachable today needs two redirect entries for its `.html` form - one for pages that moved (the bare source pointing at the new destination, plus its `.html` duplicate), and one for pages that never moved but still need a **self-redirect** (`<path>.html` -> `<path>`, the page's own current URL) so a request for the old DocFx-style URL doesn't 404.

A moved, renamed, or deleted page needs **two** entries pointing at the same destination - the bare source, and a second entry with `.html` appended:

```json
{ "source": "/da/admin/erp/learn/erp-connect", "destination": "/da/erp/learn/connect" },
{ "source": "/da/admin/erp/learn/erp-connect.html", "destination": "/da/erp/learn/connect" }
```

A page that never moved needs a self-redirect instead - `source` and `destination` differ only by the `.html` suffix:

```json
{ "source": "/en/request/learn/create.html", "destination": "/en/request/learn/create" }
```

This is expected and safe, not the `/index` self-redirect loop covered in the danger callout above - that loop only happens when a source ending in `/index` (or a trailing slash) points at its own bare parent path. A plain `.html`-suffixed source pointing at the same page without `/index` is a different shape and doesn't trigger it.

<Note>
**Don't self-redirect a `snippets/` file.** Files under a `snippets/` folder are MDX-imported into other pages, not standalone routable pages themselves - Mintlify never renders them at their own URL, so a `.html` self-redirect for one has no real destination to land on.
</Note>

Three shapes are already covered and don't need a duplicate:

* **Wildcard sources** (`/prefix/*`) - `*` already matches the `.html` form too.
* **Sources ending in `/index` or a trailing slash** - covered by the repo-wide `/:path*/index.html` rule below, not a per-page duplicate.
* **A source that already ends in `.html`** - nothing to duplicate.

`tools/check-redirects-live.py` requests both forms for every plain entry, so a missing duplicate shows up as a live-check failure rather than a broken link a visitor happens to hit first.

### The `/:path*/index.html` rule must stay first {#index-html-rule-must-stay-first}

<Danger>
**`config/redirects.json`'s first array entry must always be:**
...
**Position matters, not just presence.** Mintlify evaluates redirects in array order, and this rule needs to strip `.html` off an `index.html` request before any other wildcard rule gets a chance to match the same URL first. If it ever ends up anywhere but first, treat that as a bug, not a cosmetic ordering issue.

**Why this exists:** `.../index.html` requests only resolved correctly for `en`, our default language - every other language 404'd. The cause, confirmed by Mintlify support, is a caching side effect: `en` pages get visited often enough to stay cached, and a cached page quietly gets served for the `.html` form too, while less-visited languages fall through to the origin and 404. Mintlify's platform has no general "strip an arbitrary `.html` suffix" rule (see [External destinations need `.html`][1] above and the `.html`-suffix-duplicates section) - but every affected URL here ends in `/index.html`, so this one rule, added first, covers all of them.

**`tools/sort-redirects.py` will never move this entry.** It's listed in that script's `PINNED_FIRST_SOURCES` and excluded from the alphabetical sort entirely, always re-emitted at the front. If you add another rule with the same "must run before anything else" requirement, add its source to that list too, in the order the rules must apply - don't rely on alphabetical position or manual care on every future sort run.
</Danger>

`tools/compact-redirects.py` doesn't need the same guard - it only ever touches entries under a source prefix explicitly passed to `--apply`, and never reorders the array wholesale. `tools/check-redirects-live.py` already classifies any `*`-containing source as a wildcard entry (sanity-checked by destination prefix only, not a literal fetch), so it needed no changes either.
