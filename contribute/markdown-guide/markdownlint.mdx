...
`.markdownlint.yaml` disables `MD041` (require a top-level heading as the first line) and sets a generous limit for `MD013` (line length): page titles come from frontmatter rather than an in-body H1, and prose here isn't hard-wrapped to a fixed column width.
