...
| 7 | `so_version` | Handcrafted docs | Product version: see defaults below; omit in generated files. Named `so_version`, not `version`: Mintlify's search back end treats `version` as a reserved filter field and 400s on non-string values |
