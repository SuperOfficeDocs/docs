...
| 22 | `icon` | If present | Font Awesome icon name shown next to the page's sidebar entry. On a subcategory map page (see the subcategory-landing-page blueprint template), always `"map"`, paired with `iconType` |
| 23 | `iconType` | If present | Font Awesome style for `icon`: `regular`, `solid`, `light`, `thin`, `sharp-solid`, `duotone`, or `brands`. Subcategory map pages use `"regular"` |
