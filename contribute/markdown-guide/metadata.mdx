...
<Note>
`index.mdx` (the site homepage, `mode: "custom"`) carries one extra, homepage-only property: `recent_pages`, a YAML block sequence of `{path, pinned?, since?, title?}` entries driving the "What's new" list rendered by `<RecentPages>` (`components/recent-pages.jsx`). `path` is a root-relative URL; `pinned: true` exempts an entry from the 120-day age-out but it still counts against the 7-entry cap; `since` is this repo's usual `MM.DD.YYYY` date; `title` is an optional override shown instead of the target page's own frontmatter title (e.g. "Marketing landing page" for a page whose real title is just "Marketing" — clearer out of context on a homepage list). Don't hand-edit the resolved `<RecentPages items={[...]} />` call in the page body — `tools/ci/sync-recent-pages.py` (run automatically by `.github/workflows/recent-pages-sync.yml` on every PR touching `en/`/`integrations/`/`index.mdx`) regenerates it from the `recent_pages` frontmatter and will overwrite manual edits to that call on its next run. See [Automated tests][5] for what the workflow actually does.
</Note>
