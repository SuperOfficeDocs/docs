...
1. Push the branch into `SuperOfficeDocs/docs` itself. Mintlify's GitHub App only resolves branches it already has access to, so a fork's branch needs pushing in first, for example `git fetch origin pull/<PR#>/head:tmp-branch && git push origin tmp-branch`.
2. In the Mintlify dashboard, go to **Previews** → **Create custom preview**, enter the branch name, and create it.
...
<Note>
Requesting a manual preview costs the docs lead's time and is not instant - it depends on their availability. If you are iterating on a fork PR, mark it **draft**, make the full round of changes, then undraft and `@`-mention the docs lead to request **one** preview, not a fresh request after every small fix.
</Note>
