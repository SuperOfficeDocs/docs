...
1. Launch Git Bash. Type `pwd` at the $ prompt to determine the current directory.

1. Change directory (cd) into the folder that you created for hosting the repository locally. For example, `cd /c/code/docs/`.
...
1. If you are prompted, select your GitHub account tile as the destination where the fork should be created.
...
1. The clone command runs and downloads a copy of the repository files from GitHub into a new folder on the local disk. This may take a few minutes, depending on the repository size.
...
Want to dive right in? Check out our [Git cheat sheet][19].

## Use AI tools

If you use an AI coding assistant such as Claude Code or Cursor to help draft or review content, install the SuperOffice CRM skill so your assistant understands the product you're documenting.
