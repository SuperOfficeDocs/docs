...
* [Style guide][2]
* Automatic tests should pass - see [Automated tests][9] for the full list (markdownlint, Vale, broken-link checks, the index-page relative-links guard) and where to read their results. Markdownlint and Vale are advisory only for now: a flagged issue is worth a look, but it does not block the merge on its own.
...
* If a touched page has no `description`, or a `keywords` entry that only repeats words already in its own `title` (see [Metadata][10] for what these fields actually do), fix it while you're there rather than leaving it for a future pass. Skip this for the generated reference trees (`database/tables`, `api/archive-providers`, `api/mdo-providers`, `api/webapi`, `automation/crmscript/reference`) — a fix there belongs in the generator, not a hand-edit.

[1]: /contribute/review/request-review
[2]: /contribute/style-guide/index
[3]: /contribute/markdown-guide/index
[4]: /contribute/review/check-changes
[5]: /contribute/review/incorporate-feedback
[6]: https://github.com/SuperOfficeDocs/docs/blob/main/.github/CODEOWNERS
[7]: /contribute/branch-strategy
[8]: /contribute/review/check-changes#preview
[9]: /contribute/automated-tests
[10]: /contribute/markdown-guide/metadata
