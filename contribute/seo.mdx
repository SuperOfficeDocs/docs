...
## On-site search relevance is a separate system

Everything above is about `seo`, external search engines, and social platforms. Mintlify's own on-site search box (the search bar in the header) is a different, unrelated system with its own two levers, `boost` and `searchable` — set in page frontmatter or at the `docs.json`/`config/nav-*.json` group level, never under `docs.json`'s `seo` key. Don't reach for `seo.indexing` or `noindex` to change how a page ranks in the site's own search results, and don't expect `boost` to affect Google or social-preview behavior — see [Metadata][1] for both properties.
