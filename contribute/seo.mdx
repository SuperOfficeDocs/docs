...
<Note>
This site's own `style.css` hides `description` from rendering under the page title, so it never shows up as on-page text here — its only jobs are the social-preview tags above and an external search-engine result snippet.
</Note>
...
## The search box is a separate system

Everything above is about `seo`, external search engines, and social platforms. The search bar in this site's own header is a different, unrelated system, not to be confused with the SuperOffice product's own "Onsite" (on-premise) deployment option. It has its own two levers, `boost` and `searchable`, set in page frontmatter or at the `docs.json`/`config/nav-*.json` group level, never under `docs.json`'s `seo` key. `seo.indexing` doesn't touch it either way. `noindex`, however, does reach it: the boolean frontmatter field removes a page from this search box's results entirely, not just from Google — it's one bundled switch covering external indexing, the sitemap, this site's own search, and the AI assistant's context all at once (see [Metadata][1]'s `noindex` row, and its note on the separate `robots` field for an external-only alternative). Don't expect `boost` to affect Google or social-preview behavior either way.
