...
description: How to capture a performance baseline for this site (page load, search latency, navigation responsiveness) and rerun the same methodology later so a before/after comparison stays sound.
...
A **Performance Verification Period (PVP)** checks that the site performs acceptably under real traffic after a platform change goes live. That kind of check needs something real to compare against - this page documents the benchmarking methodology, where the captured baseline lives, and how to rerun the same methodology later so a comparison stays sound.
...
A full PVP's scope (page load, search latency, navigation, CDN geography across the EU, concurrency/traffic-based performance, error rates under load) needs real production traffic and infrastructure that doesn't exist ahead of a launch. This page's baseline scopes that down to the three metrics that can be measured meaningfully without that infrastructure: **page load (p50/p95)**, **search latency**, and **navigation responsiveness**.
...
| Search latency (server-side) | Direct `fetch()` timing | Round-trip time against the search API directly - a diagnostic for the current back end, not comparable across a back-end change |
| Search latency (end-to-end) | Playwright | Perceived latency: fill + click Search to results rendered - the number that carries forward to a later comparison |
| Navigation responsiveness | Playwright | Click-to-load time per hop, walking representative nav paths |

Lighthouse CI was chosen over WebPageTest or the PageSpeed Insights API because CDN geography and multi-location testing are explicitly out of scope for this exercise - a single consistent vantage point is enough, and Lighthouse CI needs no signup or API key and stays fully checked into this repo. Lighthouse's default simulated-mobile throttling is left as-is deliberately - whatever settings are used for the baseline must stay identical for the post-cutover rerun, not tuned for "more realistic" numbers now.
...
1. `nav-paths.json`'s hop selectors match DocFx's literal `href` markup - they will need re-verifying against the equivalent Mintlify pages first, since Mintlify's markup won't match.

1. Run all four scripts per `tools/benchmarks/README.md` against the live Mintlify site.

1. Compare against `results/baseline/` (see above). Flag the server-side search-latency script's numbers as non-comparable (different back end) - only the end-to-end search number, the page-load metrics, and the navigation numbers are valid before/after comparisons.
