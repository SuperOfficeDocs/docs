...
description: The repeatable checklist to run before any refactor, design change, or site reorg - what's automated (link/redirect/image/accessibility/encoding checks, a scripted page sweep), what needs a manual look, and the known blind spots of each.
...
This is the checklist to run before any change big enough to risk breaking something across the site - a folder move, a nav overhaul, a design change, or a pre-launch pass like [issue #88][1]. It complements [Automated tests][2], which covers what runs automatically on every PR - everything here is run on demand, against a live deployment (the softlaunch site or a PR preview), never `mint dev`.
...
```sh
SMOKE_BASE_URL=https://superofficeas.mintlify.app RUN_LABEL=<label> node smoke-test-pages.mjs
```
...
[1]: https://github.com/SuperOfficeDocs/docs/issues/88
[2]: ./automated-tests
[3]: ./review/index
[4]: ./deployment
