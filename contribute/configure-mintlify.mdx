...
* `description` supports MDX, including components - this site uses it to show the Hugo mascot illustration (`images/404-hugo.svg`, see [Files and folders][4]) through an `<img>` tag rather than plain `![]()` markdown, for the `style`/`noZoom` control that gives.
