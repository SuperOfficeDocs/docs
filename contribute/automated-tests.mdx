...
keywords: ['ci', 'automated tests', 'markdownlint', 'vale', 'broken links', 'mint validate', 'automations', 'crmscript', 'stale pages', 'generated pages']
author: digitaldiina
date: 08.13.2026
...
| Legacy `version` frontmatter guard | GitHub Action (`.github/workflows/version-frontmatter-guard.yml`) | No - advisory (warns only) | This repo, `tools/check-legacy-version-property.py` |
...
| CRMScript reference drift | GitHub Action (`.github/workflows/crmscript-drift.yml`) | Yes - fails the check | This repo, `tools/check-crmscript-drift.ps1` |
| Stale generated pages | GitHub Action (`.github/workflows/stale-generated-pages.yml`) | Yes - fails the check | This repo, `tools/find-stale-generated-pages.py` |
...
### Legacy version frontmatter guard

Checks only the `.md`/`.mdx` files changed in a PR (or pushed directly to `main`) for a `version:` frontmatter key. Mintlify's search backend treats `version` as a reserved search-filter field and 400s on the non-string value our own `version` property produced, which was one of the causes behind broken site search - so the property was renamed to `so_version` repo-wide (see [issue #254][9]). This check exists so the old key does not quietly come back (a copy-pasted frontmatter block from a stale branch, or muscle memory). It never fails the check - a hit shows up as a warning annotation on the relevant line in the PR's Files Changed tab, suggesting `so_version` instead.
