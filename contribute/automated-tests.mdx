...
| WebAPI bare-relative links auto-fix | GitHub Action (`.github/workflows/webapi-bare-relative-links-guard.yml`) | No - auto-fixes and pushes a commit | This repo, `tools/ci/fix-webapi-bare-relative-links.py` |
...
### WebAPI bare-relative links auto-fix

Triggers on a PR touching `en/api/reference/webapi/**`. This tree is generated wholesale by an external ADO pipeline that has no awareness of Mintlify's routing: every internal link is emitted as a bare same-folder filename (`SuperOffice.WebApi.Agents.AgentBase.md`, optionally with an escaped anchor), which resolves fine against the folder on disk but 404s live, since Mintlify serves this tree at `/en/api/reference/webapi/` and drops the `.md` extension entirely. [Issue #400][24] found and fixed this repo-wide (2,518 files, confirmed against [PR #383][25], Mintlify's own broken-link-scan bot) - this check exists so the next ADO content drop that reintroduces the same bare-relative shape gets it rewritten automatically rather than shipping another few thousand dead links.

This is an auto-fix rather than a blocking check, for the same reason as the reserved-`version`-frontmatter check above: the generator itself is the thing that needs fixing (a bug report was filed against that pipeline separately), and failing every future content drop until that lands would leave the build permanently red for a problem this script can close out on its own.

Unlike the other changed-files-scoped auto-fixes on this page, each run rescans the *whole* tree rather than just this PR's changed files - a giant regenerated-content drop can touch every file in the tree at once (this repo's own fix PR did, and the first version of this workflow broke trying to pass that many changed-file paths through a workflow environment variable), and since the script is idempotent, a full scan of this (small, ~2,500-file) tree costs a few seconds regardless. Each run rewrites any bare-relative destination to a root-relative path (`Some.Class.md` -> `/en/api/reference/webapi/Some.Class`, unescaping `\_` in both the filename and any anchor), skipping fenced code/inline code spans so a documentation example quoting this exact link syntax is never rewritten. A destination that carries a path segment - never observed in this tree - is left untouched and reported rather than guessed at. If any rewrite happens, it's pushed back onto the PR branch as a fix commit, same as the other auto-fix checks on this page. Idempotent, and skipped entirely for PRs from a fork, since the default token can't push back to a fork's branch.
...
`<see cref="T:Namespace.Type">` is .NET XML-doc-comment syntax, not HTML - DocFX used to resolve it into a real hyperlink, but Mintlify's MDX renderer doesn't, so it either breaks the build outright or silently drops the link and leaves only the bare type name visible. A repo-wide audit ([issue #407][22]) found 797 files / 1,307 pre-existing occurrences, concentrated in the generated `archive-providers`/`mdo-providers` reference trees and predating any single PR - fixing all of it needs a real resolve-or-plain-text conversion script, tracked in #407 as its own piece of work. This guard is the stopgap in the meantime: it stops the count from growing while that fix is designed, without attempting to touch any of the pre-existing files itself.
