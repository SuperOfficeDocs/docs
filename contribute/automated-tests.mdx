...
| DocFX see-cref guard | GitHub Action (`.github/workflows/docfx-see-cref-guard.yml`) | **Yes** - fails the check | This repo, `tools/ci/check-no-new-docfx-see-cref.py` |
| DocFX xref guard | GitHub Action (`.github/workflows/docfx-xref-guard.yml`) | **Yes** - fails the check | This repo, `tools/ci/check-no-new-docfx-xref.py` |
...
### DocFX see-cref guard

Checks every PR (and direct push to `main`) for a newly *added* `<see cref="T:...">` (or its HTML-escaped form) occurrence in any `.md`/`.mdx` file, via the same base-ref `git diff` approach as the guard above - but at line granularity, not whole-file: only lines the PR's own diff actually adds are checked, so an unrelated edit to a file that already has the pattern isn't re-flagged. This is the second of three **blocking** exceptions in the guard family.

`<see cref="T:Namespace.Type">` is .NET XML-doc-comment syntax, not HTML - DocFX used to resolve it into a real hyperlink, but Mintlify's MDX renderer doesn't, so it either breaks the build outright or silently drops the link and leaves only the bare type name visible. A repo-wide audit ([issue #407][22]) found 1,315 pre-existing occurrences across 796 files (once the HTML-escaped duplicates the issue's own first-pass count missed are included), concentrated in the generated `archive-providers`/`mdo-providers`/`webapi` reference trees and predating any single PR. #407's real fix - a resolve-or-plain-text conversion, now run automatically by the Generated-reference auto-fix above - keeps those three trees clean going forward; this guard stays in place as the safety net against a *new*, hand-authored occurrence anywhere else in the repo, which the auto-fixer never touches.

### DocFX xref guard

Checks every PR (and direct push to `main`) for a newly *added* `<xref:some.uid>` or `<xref href="some.uid" data-throw-if-not-resolved="false"></xref>` occurrence (or either's HTML-escaped form) in any `.md`/`.mdx` file, at the same added-line granularity as the see-cref guard above. This is the third **blocking** exception in the guard family.

Mintlify's MDX renderer doesn't resolve DocFX's `xref` syntax in either form. #312/PR #313 did a repo-wide sweep converting ~90 known occurrences to real links, but that sweep alone isn't durable: [issue #404][23] found two of #313's own already-fixed lines had been silently reintroduced by a later ADO/generator content drop regenerating a page from its source, with no awareness of the Mintlify-side fix. This guard exists so a regenerated page - or a hand-authored edit - can't quietly reintroduce an occurrence the same way again; it doesn't touch any pre-existing occurrence itself.
