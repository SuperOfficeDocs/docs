...
This page is the single reference for every automatic check that runs against this repo, whether it is triggered by a pull request or runs on its own schedule. It does not duplicate the general mechanics already covered in [Mintlify deployment and CI/CD](./deployment) - only what each check covers and how to act on it. An annotation on **Files changed** can lag behind the PR's latest commit. See [Reviewing a PR][1] for how to confirm the current state before trusting one you see while scrolling a multi-commit diff.
...
| Boost changes guard | GitHub Action (`.github/workflows/boost-changes-guard.yml`) | No - advisory (warns only, comments on the PR) | This repo, `tools/ci/check-boost-changes.py` |
...
| Build validation | Mintlify preview deployment | Yes - a failed build shows as a red "Mintlify Deployment" check | Mintlify, automatic on every PR (see [Deploy trigger mechanics](./deployment#deploy-trigger-mechanics)) |
...
Runs on every PR (`fetch-depth: 0`, needed to diff against `origin/main`). Detects any `.md`/`.mdx` file this PR renamed, moved, or deleted (via git's own rename detection, not a plain add/delete pair), excluding `snippets/` and the machine-generated reference trees - those aren't standalone routable pages, so a missing redirect for one isn't a real gap. For each one, checks whether `config/redirects.json` already covers the old URL, including the `.html`-suffixed duplicate form documented in [Redirects](./redirects). Separately, for a renamed `en/` page, checks whether its translations were moved to the same relative location too - matched by path (swapping the language segment), not by `uid`. A translation left on its old path after the English page moves is a quieter version of the same failure mode.

A third, narrower check covers a gap neither of the above can: a page added to `config/nav-learn.json` without a rename ever being involved, but with no matching entry added to `config/nav-da.json`, `nav-de.json`, `nav-nl.json`, `nav-no.json`, or `nav-sv.json` (see issue #496, whose `marketing/forms/learn/recaptcha` entry was the first real instance of this). It flattens `config/nav-learn.json`'s nested `pages`/`groups`/`root` structure, finds any `en/learn/**` or `en/admin/**` page present now but absent at the PR's base commit, and checks the same language set for a mirrored path, skipping the same permanently English-only trees (`reference/`, `dev/`, `mobile/`, `en/admin/user-preferences/`) the manual translation-parity audits already exclude. It only watches for a newly added entry losing parity, not full historical completeness across the whole nav.

Never fails the check - a gap shows up as a warning annotation on the PR's Files Changed tab, and adds the `mintlify/nav-redirects` label (or `translation`, for a mirroring gap, from either the rename check or the nav-parity check) to the PR so it's visible without opening the Actions tab; the label is removed automatically once a later push resolves the gap.

### Boost changes guard

Runs on every PR (`fetch-depth: 0`, needed to diff against the PR's base commit). Detects any *added* `boost:`/`"boost":` line - page frontmatter or a `docs.json`/`config/nav-*.json` navigation-group key - by scanning only the `+` lines of a `git diff` against the base, not a file's current content, so an unrelated edit to an already-boosted page isn't re-flagged. `boost` is Mintlify's own search-box relevance multiplier: unlike every other check on this page, it has no wrong-or-right shape to lint for, only a value that's easy to get wrong at scale (Mintlify's own guidance warns an unreviewed large multiplier can make a barely-relevant page dominate results). So instead of a warning annotation, this is the first guard in the family to post an actual PR comment listing every changed value. The reviewer it notifies comes from a `BOOST_GUARD_MENTION` repository variable rather than a name written into this file, since a public `@`-mention in tracked source is an open invitation for anyone to message that person directly. See [Metadata](./markdown-guide/metadata) for the full guidance table before approving one. Never fails the check.
...
* [Mintlify deployment and CI/CD](./deployment) - the deploy pipeline and PR preview mechanics these checks build on.
* [markdownlint](./markdown-guide/markdownlint) - rule details and local usage.
* [Reviewing a PR](./review/index) - the review checklist that links back here.
* [Update the CRMScript reference](./how-to-update-crmscript-reference) - the regeneration workflow the drift check enforces.
* [Acceptance testing](./acceptance-testing) - the on-demand checklist to run after a refactor, design change, or site reorg, rather than every PR.

[1]: ./review/check-changes#stale-annotations
