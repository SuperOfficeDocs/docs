...
description: Every check that runs against a PR or on a schedule for this repo - markdownlint, Vale, build validation, broken-link checking, and the CRMScript reference drift check - what each covers, whether it blocks a merge, and where to look when one flags something.
keywords: ['ci', 'automated tests', 'markdownlint', 'vale', 'broken links', 'mint validate', 'automations', 'crmscript']
author: digitaldiina
date: 08.12.2026
