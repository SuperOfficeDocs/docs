...
related:
  - /sv/marketing/forms/learn/publish
language: sv
---
...
## Skapa flersidiga formulär {#multi-page}
...
8. Du kan också redigera dessa inställningar (valfritt):
    ...
    * Om fälttypen stöder det kan du klicka på **Lägg till relaterat fält** på ett av dess alternativ eller värden för att lägga till ett villkorligt fält. Se [Lägg till ett villkorligt fält](#conditional) nedan.
...
### Lägg till ett villkorligt fält {#conditional}

Visa bara besökarna de frågor som är relevanta för dem. Ett **villkorligt fält** (även kallat ett relaterat fält) visar en följdfråga bara när ett tidigare svar gör den relevant, så att varje besökare bara ser det som är viktigt för dem, samtidigt som formuläret ändå samlar in de uppgifter som behövs från alla andra. Fältet **Contact - Country** kan till exempel visa fältet **Contact - Preferred language** bara när ett visst land har valts.

Det här fungerar för listrutor (inklusive standardlistfält), alternativknappar, kryssrutor, bedömningsfält (stjärnor, smilisar, tärningar eller tummen upp/ner), extrafält av typen listruta eller booleskt, samt intressefält. Fälttyper som stöder det här får ett **Villkorad**-märke när du lägger till ett nytt element, så att du inte behöver lägga till fältet först för att ta reda på det.

![Fält som stöder villkorliga fält får märket Villkorad i fönstret Lägg till post -screenshot][img18]

1. Välj alternativet eller värdet i ett fält som stöder det här, som ska utlösa följdfrågan.
1. Klicka på **Lägg till relaterat fält**.
1. [Lägg till och konfigurera det nya fältet](#fields) på samma sätt som för alla andra fält.

Det överordnade fältet får nu märket **Har relaterade fält**, och det relaterade fältet visas indraget under det, med en banderoll som förklarar villkoret i vardagligt språk (till exempel "Det här fältet visas när **Contact - Country** är **Schweiz**"). Använd banderollens redigeringsikon för att ändra villkoret, eller borttagningsikonen för att ta bort det relaterade fältet. Om du väljer flera utlösande värden binder banderollen ihop dem med "eller". Bedömningsfält visar i stället ett intervall.

![Ett fält med ett relaterat fält och dess villkorsbanderoll -screenshot][img19]

Du kan kedja relaterade fält flera nivåer djupt (en fråga kan utlösa en följdfråga, som i sin tur kan utlösa en ännu djupare följdfråga), men varje relaterat fält styrs av exakt ett villkor på ett fält.

<Note>
Ett relaterat fält är dolt som standard för besökaren, visas direkt när det utlösande svaret ges, och försvinner och töms om svaret ändras. Ett dolt fält är aldrig obligatoriskt, även om det är markerat som obligatoriskt, och dolda svar tas inte med i inlämningen.
</Note>
...
#### Anpassad formatering {#custom-style}
