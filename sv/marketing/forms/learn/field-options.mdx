...
* **Tilläggsfält av typen kryssruta:** Ett booleskt tilläggsfält (endast ja/nej) som kan ställas in för att vara markerat eller omarkerat när formuläret läses in. Hitta **Markerad som standard** i fältets inställningar — det är avmarkerat som standard. Om fältet har [ett relaterat fält][6] kopplat via ett villkor behandlas standardmarkeringen som om besökaren själv hade angett den, så det relaterade fältet visas eller förblir dolt vid inläsning.
...
* **Villkorade (relaterade) fält:** Alternativknappar, kryssrutor, listrutor och bedömningsfält kan [utlösa ett relaterat fält][6] som bara visas när ett visst alternativ eller värde har valts.
