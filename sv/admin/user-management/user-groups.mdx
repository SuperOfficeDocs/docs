---
uid: help-sv-user-group-manage
title: Hantera användargrupper
description: Skapa och ta bort användargrupper
keywords: ['användargrupp']
author: digitaldiina
date: 05.04.2026
so_version: 11.13
content_type: howto
tier: starter
category: identity management
topic: user
audience: settings
audience_tooltip: Settings and maintenance
redirect_from: /sv/admin/user-management/learn/user-groups
userflow_index: true
related:
  - Lägga till SuperOffice-användare: /sv/admin/user-management/add-associate
  - Konfigurera vilken information användarna i företaget kan få tillgång till i SuperOffice CRM: /sv/admin/user-management/role/index
language: sv
---

import GotoSm from "../../learn/snippets/goto-sm.md";

För att gruppera kollegor tillsammans och ge dem åtkomst till CRM-data baserat på denna gruppering behöver du skapa en ny användargrupp i SuperOffice och sedan tilldela denna grupp till dina kollegor.

## Skapa en användargrupp {#create}

1. <GotoSm />

1. Klicka på <i class="ph ph-list-bullets" aria-hidden="true"></i> **Listor** i navigatorn.

    ![Skapa en användargrupp -screenshot][img3]

1. Välj **Allmänt - Användargrupp** i listan och klicka på **Ny**.

1. Ange information i dialogrutan. Valfritt, välj en eller flera [ärendekategorier][4] för den här gruppen.

    ![Skapa en användargrupp -screenshot][img4]

1. Klicka på **OK**. Nu är den nya användargruppen tillgänglig i Användar-dialogen.

    ![Användar-dialogen -screenshot][img5]

## Ta bort användargrupp {#delete}

1. Gå till listan **Allmänt - Användargrupp** enligt beskrivningen ovan.
1. Klicka på den aktuella användargruppens namn.
1. Klicka på knappen **Ta bort** längst ned på skärmen.

[4]: ../../request/admin/category/index
[img3]: ../../../media/loc/en/admin/add-user-group.png
[img4]: ../../../media/loc/en/admin/edit-list-item.png
[img5]: ../../../media/loc/en/admin/user-dialog.png
