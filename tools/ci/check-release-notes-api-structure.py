...
release-notes/{major}/api/ (for example release-notes/12/api/, release-notes/11/api/)
...
#150's own original "Required output" text (predating #223, corrected via a comment on #150 rather
than an edit to the issue body) called for a nested release-notes/{major}/api/{major.minor}/
subfolder instead. A first real generator PR built against that stale text landed exactly this
regression (see #291); this guard exists so the next one gets fast, visible feedback instead of
silently landing the same structural mistake again. Nothing else in this repo's checks would catch
it: nav wiring can point at the same file whether it's flat or nested, and `mint validate` has no
opinion on folder depth.

Detection is a bare path regex, not frontmatter or content: release-notes/(digits only, no
dot)/api/(anything)/... one directory level deeper than the flat convention allows. The digits-only
requirement is deliberate: it excludes this repo's historical release-notes/{major.minor}/api/ trees
(release-notes/10.1/api/, release-notes/9.2/api/, etc.), which are a different, frozen, pre-#219
structure and out of scope for this guard.

This is advisory only; it never fails the build. It emits a GitHub Actions warning annotation per hit
so it shows up on the PR's Files Changed tab without blocking anything.
...
def main():
    ...
    parser.add_argument("--path", help="Scope to one folder (for example release-notes) instead of an explicit file list")
