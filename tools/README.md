...
| `fix-webapi-bare-relative-links.py` | ci | Auto-fixes: rewrites bare-relative link destinations in `en/api/reference/webapi/` (generated by an external ADO pipeline) to root-relative paths, commits back to the PR — see #400 | Yes |
...
| `fix-docfx-xref.py` | ci | Auto-fixes: resolves dead DocFX `<xref href="...">`/`<xref:...>` cross-references in `archive-providers`/`mdo-providers`/`webapi` to a real link (`path#anchor` for member-level refs, `learn.microsoft.com` for `System.*`/`Microsoft.*` types), or falls back to plain text, commits back to the PR — see #404/#312 | Yes |
...
| `check-no-new-docfx-list.py` | ci | Fails the PR if it adds a new DocFX `<list type="table">`/`<list type="bullet">` block (added diff lines only, doesn't re-flag any pre-existing occurrence) — see #410 | Yes |
